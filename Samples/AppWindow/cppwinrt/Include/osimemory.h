...
public:
    ...
    void _Decref() noexcept // decrement use count
        ...
        if(f_Uses == 0)
            ...
            {
            if(m_MapPathName.length() > 0)
                {
                std::string f_PathName = m_MapPathName;
...
class shared_base
{
    shared_base(std::shared_ptr<T> f_Ptr)
        {
        m_Ptr = f_Ptr;
        m_Void_Ptr = std::static_pointer_cast<void>(f_Ptr);
        }

    std::shared_ptr<void> m_Void_Ptr;
