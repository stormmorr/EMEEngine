...
private:
    ...
    template <class _Ty0, class... _Types>
    friend shared_ptr<_Ty0> make_shared_reflect(_Types&&... _Args);
...
// OSI FUNCTION TEMPLATE make_shared_reflect
template <class _Ty, class... _Types>
_NODISCARD
#if _HAS_CXX20
enable_if_t<!is_array_v<_Ty>, shared_ptr<_Ty>>
...
shared_ptr<_Ty>
#endif // _HAS_CXX20
make_shared_reflect(_Types&&... _Args) // make a shared_ptr to non-array object
{
#ifndef OSI_MEM_APP
    std::string f_AOMStr = ag_AcceptObjectName();
    ...
    std::shared_ptr<ecom_base> f_ecomRefBase = _Ret;

     ///////////////////
    // Reference CHK
#ifdef ECOIN_DBG_BREAK
#if 0
    if(f_ecomRefBase->acEmptyBase() == true)
        {
        __debugbreak();
        }
#endif
    ...
    std::string f_TypeString = typeid(_Ty).name();

    std::string f_CountString = f_AOMStr + g_ref_Div + f_TypeString + g_ref_Div;
    ...
    ag_LockMutex();
     ...
     ////////////////////
    //Current Count UP//
    if(g_Map_ecomRefCount.empty() == false)
        {
        if(g_Map_ecomRefCount.find(f_CountString) == g_Map_ecomRefCount.end())
            {
            g_Map_ecomRefCount.emplace(f_CountString, 1);

            f_ecomRefBase->m_StackCount = 0;
            }
        else
            {
            f_ecomRefBase->m_StackCount = g_Map_ecomRefCount[f_CountString];

            g_Map_ecomRefCount[f_CountString]++;
            }
        }
    else
        {
    ...
    std::string f_StaticStringBuilder = f_CountString;

    char* f_char = ag_ClearChar(256);

    sprintf(f_char, "%i%s", f_ecomRefBase->m_StackCount, g_ref_Div.c_str());

    f_CountString += f_char;

    delete f_char;

    if(g_Map_ecomRefBase.empty() == false)
        {
        if(g_Map_ecomRefBase.find(f_CountString) == g_Map_ecomRefBase.end())
            {
            g_Map_ecomRefBase.emplace(f_CountString, f_ecomRefBase);

            f_ecomRefBase->m_StackCount = 0;
            f_ecomRefBase->m_PathName = f_CountString;
            }
        else  //win conditions! \\\/\/\/\/\\///
            {
            int f_XY = f_ecomRefBase->m_StackCount + 1;

            while(g_Map_ecomRefBase.find(f_CountString) != g_Map_ecomRefBase.end())
                {
                char* f_charA = ag_ClearChar(256);

                sprintf(f_charA, "%i%s", f_XY, g_ref_Div.c_str());

                f_CountString = f_StaticStringBuilder + f_charA;
            ...
            f_ecomRefBase->m_StackCount = f_XY - 1;
    ...
    f_ecomRefBase->m_Status = 1;

    _Ret._Rep->m_MapPathName = f_ecomRefBase->m_PathName;
...
public:
	...
	unsigned int _Use_count() const noexcept
		{
        return m_RefCount;
		}
};
...
class _Smart_count_object : public Smart_Class
{
public:
    template <class... _Types>
    explicit _Smart_count_object(_Types... _Args) : Smart_Class()
        {
		_Storage = new _RType(forward<_Types>(_Args)...);
        }

    ~_Smart_count_object() noexcept
		{
		//...
		}
...
public:
	unsigned int use_count() const noexcept
		{
		return this->_Rep->_Use_count();
		}

	Smart_Resource(const Smart_Resource&) = delete;
	Smart_Resource& operator=(const Smart_Resource&) = delete;
...
protected:
	constexpr Smart_Resource() noexcept = default;

	~Smart_Resource() = default;

	template <class _Ty>
	void _Copy_construct_from(const Smart_Ptr<_Ty>& _Other) noexcept
		{
		_Ptr = _Other._Ptr;
		_Rep = _Other._Rep;

		// implement Smart_Ptr (converting) copy ctor
		_Incref();
		}
	...
	void _Decref() noexcept
	 ...
	 void _Swap(Smart_Resource& _Right) noexcept
		{
		// swap pointers
...
public:
	Smart_Class* _Rep{ nullptr };
	_RType* _Ptr{ nullptr };
};

template<class _RType>
class Smart_Ptr : public Smart_Resource<_RType> {
private:
	using _Mybase = Smart_Resource<_RType>;

public:
	constexpr Smart_Ptr() noexcept = default;

	constexpr Smart_Ptr(nullptr_t) noexcept {}

	Smart_Ptr(const Smart_Ptr& _Other) : Smart_Resource<_RType>()
		{
		this->_Copy_construct_from(_Other);
		}
	...
	Smart_Ptr& operator=(Smart_Ptr& _Right) noexcept
		{
	...
	Smart_Ptr& operator=(Smart_Ptr&& _Right) noexcept
		{
		Smart_Ptr(move(_Right)).swap(*this);

        return *this;
		}

	Smart_Ptr<_RType> Nullify(void)
		{
		Smart_Ptr<_RType> f_DefaultConstructor;

		return f_DefaultConstructor;
		}
	...
	bool unique() const noexcept
		{
		// return true if no other shared_ptr object owns this resource
		return (this->_Use_count() == 1) ? true : false;
		}

	template<class _Ux>
	void _Set_ptr_rep_and_enable_shared(_Ux* const _Px, Smart_Class* const _Rx) noexcept
		{
		this->_Ptr = _Px;
		this->_Rep = _Rx;
		}
...
template<class _RType, class... _CArgs>
Smart_Ptr<_RType> ag_Make_Smart_Pointer(_CArgs... args)
{
	//Initialise Controllers
	const auto _Px = new _Smart_count_object<_RType>(forward<_CArgs>(args)...);

	Smart_Ptr<_RType> _Ret;

	_Ret._Set_ptr_rep_and_enable_shared(_Px->_Storage, _Px);

	return _Ret;
}

template <class _Ty1, class _Ty2>
bool operator==(const Smart_Ptr<_Ty1>& _Left, const Smart_Ptr<_Ty2>& _Right) noexcept
{
	return _Left.get() == _Right.get();
}
...
template <class _Ty>
bool operator==(nullptr_t, const Smart_Ptr<_Ty>& _Right) noexcept
{
	return nullptr == _Right.get();
}

template <class _Ty>
bool operator!=(Smart_Ptr<_Ty> _Left, nullptr_t) noexcept
{
	return _Left.get() != nullptr;
}

template <class _Ty>
bool operator!=(nullptr_t, Smart_Ptr<_Ty> _Right) noexcept
{
	return nullptr != _Right.get();
}
...
private:
    _NODISCARD static auto _Get_ptr_from_smart(_SmartPtr& _Smart_ptr) noexcept requires is_pointer_v<_SmartPtr> {
        return _Smart_ptr;
    }
...
#ifdef ECOIN_OSI_MEMORY
    using _STD make_shared_reflect;
#endif
