...
void ag_Calc_Light(BiVector f_CalcPos)
{
	...
	for(int f_F = 0; f_F < 10; f_F++)
		...
		if(f_Any)
			...
			g_LightPos[(f_F * 4) + 3] = g_Light[f_LightIndex[f_Shortest]].m_Radius * 3.0f;
				...
				if(f_i < f_LightNom)
					...
					if(uLightPos[f_i].w > 0.0)
						...
						/*if(f_constant_dist < 1450.0)
							{
							vec4 f_CalcVec = (Idiffi * (f_LightRadius / 40.0)) * (1.0 - (f_constant_dist / 1450.0)); // 750.0 // 654.0 // 850.0 // 55.0
						...
						vec4 f_CalcVec = clamp((Idiffi * (f_LightRadius / 40.0)), 0.0, 1.0) * (1.0 - clamp((f_constant_dist / 1450.0), 0.0, 1.0)); // 750.0 // 654.0 // 850.0 // 55.0
