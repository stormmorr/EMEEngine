...
void ag_Calc_Light(BiVector f_CalcPos)
{
	...
	for(int f_F = 0; f_F < 10; f_F++)
		...
		if(f_Any)
			...
			g_LightPos[(f_F * 4) + 3] = g_Light[f_LightIndex[f_Shortest]].m_Radius * 5.0f;
...
bool unRenderer::load_programs()
{
	...
	const std::string vs = STRING
		...
		void main()
			...
			vPosition = f_interim.xyz + uCam; //\/wk + uCam ?
    ...
    const std::string fs = STRING
        ...
        void main()
			...
			// sun effect
			vec3 f_SunLightPos = vec3(5050.0, 5050.0, 5850.0);
			...
			vec4 sunIdiff = vec4(f_SunLightColor.rgb * (max(dot(f_vNormal, f_L), 0.0) + 0.09), 1.0);

			sunIdiff = clamp(sunIdiff, 0.0, 1.0);

			sunIdiff *= 0.07; // *= 0.03

			vec4 f_AccumulationBuffer = sunIdiff * 5.75;
			...
			for(int f_i = 0; f_i < 10; f_i++)
				...
				if(f_i < f_LightNom)
					...
					if(uLightPos[f_i].w > 0.0)
						...
						vec4 Idiffi = vec4(uLightColor[f_i].rgb * (max(dot(f_vNormal, to_light_norm), 0.0) + 0.04), 1.0);
						...
						f_AccumulationBuffer += (Idiffi * (f_LightRadius / 40.0)) * (1.0 - (f_constant_dist / 950.0)); //wk // 55.0
			...
			// set pixel color
			gl_FragColor = ((f_AmbientColor * f_Result) * 0.85) + (uTeam * 0.26); //wk 0.44 // 0.34
			}
		);
#endif
...
void unRenderer::draw_material_avatars(bool relief)
{
	...
	for(unsigned int f_CountPlayer = 0; f_CountPlayer < g_PlayerNom; f_CountPlayer++)
		...
		if(1 || f_View.is_inside(pVector(playerAV[g_Player]->Xform.Translation.x, playerAV[g_Player]->Xform.Translation.y, playerAV[g_Player]->Xform.Translation.z)) && (playerAV[g_Player]->Avatar.Mesh->m_rebuild == 0))
			{
			  /////////////////
			 // Q_Light
			//
