...
void ag_Calc_Light(BiVector f_CalcPos)
{
	...
	int f_New_LightNom = f_CountIndex;

	int f_Max_Lights = 10;

	if(f_New_LightNom < 10)
		{
		f_Max_Lights = f_New_LightNom;
		}

	for(int f_F = 0; f_F < f_Max_Lights; f_F++)
		{
		float f_Max = FLT_MAX;
		int f_Shortest = 0;
		bool f_Any = false;

		for(int f_L = 0; f_L < f_New_LightNom; f_L++)
			{
			if(f_LightTest[f_L] == true)
				{
		...
		if(f_Any)
			...
			g_LightPos[(f_F * 4) + 3] = g_Light[f_LightIndex[f_Shortest]].m_Radius * 1.095f; // * 5.0f //// 1.995f

			//g_Light[f_LightIndex[f_Shortest]].m_Radius -= 50.0f;

			g_LightColor[f_F * 4] = g_Light[f_LightIndex[f_Shortest]].m_Color.red / 255.0f;
			g_LightColor[(f_F * 4) + 1] = g_Light[f_LightIndex[f_Shortest]].m_Color.green / 255.0f;
			g_LightColor[(f_F * 4) + 2] = g_Light[f_LightIndex[f_Shortest]].m_Color.blue / 255.0f;
			g_LightColor[(f_F * 4) + 3] = g_Light[f_LightIndex[f_Shortest]].m_Color.alpha / 255.0f;
	...
	// Global Time Calc
	//g_TimeDiff += 0.007f * _->TimeScale; //swk
	g_TimeDiff += 0.01f * _->TimeScale; //swk 0.001f

	if(g_TimeDiff >= (PI * 2.0f))
		{
		g_TimeDiff -= (PI * 2.0f);
		}
}
...
bool unRenderer::load_programs()
{
	...
	const std::string vs = STRING
		...
		void main()
			...
			vec3 f_AvatarTransV3 = avatarTrans.xyz;

			f_AvatarTransV3.z *= -1.0;

			vec4 f_interim = uModelMatrix * pos;
			vec4 f_interim_normal = uModelMatrix * vec4(normal, 1.0);

			// interim translation
			f_interim.xyz += f_AvatarTransV3;
			...
			vPosition = f_interim.xyz + uCam; //\/wk
			vPosition.z *= -1.0;
    ...
    const std::string fs = STRING
        ...
        void main()
			...
			// call for fragment
			float f_Frl = rand() * 0.5;
			float f_Byl = rand() * f_Frl;
			float f_qsX = ((rand() - 0.25) * 2.0) * f_Byl;
			float f_qsY = ((rand() - 0.25) * 2.0) * f_Byl;
			float f_qsZ = ((rand() - 0.25) * 2.0) * f_Byl;
			...
			vec3 f_SunLightPos = al_OrbitalSunPos(uTime, vec3(-1000.0, 0.0, 0.0));
			vec4 f_SunLightColor = vec4(0.96, 0.90, 0.79, 1.0);

			vec3 f_MoonLightPos = al_OrbitalMoonPos(uTime, vec3(0.0, 0.0, 1000.0));
			...
			vec3 f_to_Sun = f_SunLightPos - vPosition;
			...
			//- quantum moon effect -//
			vec3 f_to_Mn = f_MoonLightPos - vPosition;
				...
				if(f_i < f_LightNom)
					...
					if(uLightPos[f_i].w > 0.0)
						...
						float f_constant_dist = sqrt(to_light.x * to_light.x + to_light.y * to_light.y + to_light.z * to_light.z);
							...
							//(7%) OF (0.0 -> 1.0)     \/wk????     33.0
						vec4 f_CalcVec = (Idiffi * (f_LightRadius / 24.4)) * (1.0 - clamp((f_constant_dist / 1450.0), 0.0, 1.0)); // 750.0 // 654.0 // 850.0 // 55.0 - 1450.0
			...
			// set pixel color
			gl_FragColor = (f_Result * 0.9) + (uTeam * 0.2); //wk 0.44 // 0.34 // 0.20 // 0.24 //// 0.87 0.23
			}
		);
#endif
#if 0
...
void unRenderer::draw_material_avatars(bool relief)
{
	...
	// Wire Camera Position
	glUniform3f(mCamUniformLocation, playerAV[0]->Xform.Translation.x, playerAV[0]->Xform.Translation.y, playerAV[0]->Xform.Translation.z);
	...
	for(unsigned int f_CountPlayer = 0; f_CountPlayer < g_PlayerNom; f_CountPlayer++)
		...
		if(f_View.is_inside(pVector(playerAV[g_Player]->Xform.Translation.x, playerAV[g_Player]->Xform.Translation.y, playerAV[g_Player]->Xform.Translation.z)) && (playerAV[g_Player]->Avatar.Mesh->m_rebuild == 0))
			...
			if(g_Player == 0)
				...
				f_Transport[2] = playerAV[g_Player]->Xform.Translation.z - playerAV[0]->Xform.Translation.z;
				}
			...
			if(player[g_Player]->ShadowsOn == 1)
				...
				for(SCRAM = 1; SCRAM < 4; SCRAM++)
					{
					ag_Calc_Light(Cube::BiVector(player[g_Player]->Shadows[SCRAM].Xform.Translation.x, player[g_Player]->Shadows[SCRAM].Xform.Translation.y, player[g_Player]->Shadows[SCRAM].Xform.Translation.z));
					...
					if(g_Player == 0)
						...
						{
						f_Transport[0] = player[g_Player]->Shadows[SCRAM].Xform.Translation.x - playerAV[0]->Xform.Translation.x;
						f_Transport[1] = player[g_Player]->Shadows[SCRAM].Xform.Translation.y - playerAV[0]->Xform.Translation.y;
						f_Transport[2] = player[g_Player]->Shadows[SCRAM].Xform.Translation.z - playerAV[0]->Xform.Translation.z;
						}
	...
	for(int i = 0; i < _->NomAI; i++)
		...
		if((f_View.is_inside(pVector(Q_Forge->Forge[i].AV.Xform.Translation.x, Q_Forge->Forge[i].AV.Xform.Translation.y, Q_Forge->Forge[i].AV.Xform.Translation.z))) && (Q_Forge->Forge[i].HB_DeadTimer > 0) && (Q_Forge->Forge[i].LIVE == 1) && (Q_Forge->Forge[i].AV.Avatar.Mesh->m_rebuild == 0))
			...
			if(g_ZoomFactor == 0 || 1)
				...
				f_Transport[2] = Q_Forge->Forge[i].AV.Xform.Translation.z - playerAV[0]->Xform.Translation.z;
				}
			else
				{
			...
			if(Q_Forge->Forge[i].ShadowsOn == 1)
				{
				glBlendFunc(GL_SRC_ALPHA, GL_ONE);
				glEnable(GL_BLEND);
				...
				for(SCRAM = 1; SCRAM < 4; SCRAM++)
					{
					ag_Calc_Light(Cube::BiVector(Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.x, Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.y, Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.z));
					...
					if(g_ZoomFactor == 0 || 1)
						{
						f_Transport[0] = Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.x - playerAV[0]->Xform.Translation.x;
						f_Transport[1] = Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.y - playerAV[0]->Xform.Translation.y;
						f_Transport[2] = Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.z - playerAV[0]->Xform.Translation.z;
						}
					else
						{
						f_Transport[0] = Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.x - (playerAV[g_Player]->Xform.Translation.x * g_ZoomFactor);
						f_Transport[1] = (Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.z + Q_Forge->Forge[i].Shadows[SCRAM].Xform.Translation.y) - ((playerAV[g_Player]->Xform.Translation.z + playerAV[g_Player]->Xform.Translation.y) * g_ZoomFactor);
						f_Transport[2] = 0.0f;
						}
...
void unRenderer::draw_geometry(pMesh *m,bool relief)
{
	...
	if(KEY_PRESS_DOWN(QIN_F9) && (_->NomPntSprite < Q_PntSprite->m_ArrayMx))
		{
...
if(KEY_PRESS_DOWN(QIN_KP4))
	...
	//*if(KEY_PRESS_DOWN(QIN_u)) UNMessageCore->acPrintName(0);
#endif
...
#if 0
	if(KEY_PRESS_DOWN(QIN_g)) QActiveFrameWork->f_TrrBank->acAddInstance(0, playerAV[g_Player]->Xform.Translation.x + ((((float)rand() / 32767) * 300)-150), playerAV[g_Player]->Xform.Translation.y + ((((float)rand() / 32767) * 300)-150), playerAV[g_Player]->Xform.Translation.z + ((((float)rand() / 32767) * 300)-150));
#endif
	...
	if(KEY_PRESS_DOWN(QIN_F7))	// Exper
		{
		if(!vecvarDpress[7])
			{
			vecvarDpress[7] = true;
	...
	//if(KEY_PRESS_DOWN(QIN_0)) QActiveFrameWork->DigiMass->acSimulate();
	/*if(KEY_PRESS_DOWN(QIN_9))*/ //QActiveFrameWork->DigiMass->acRender(true);
	//glTranslatef(0, 356, 0);

	//Item Positioning
	/*if(KEY_PRESS_DOWN(QIN_d))
		{
		if(!vecvarDpress[16])
			{
			4++;
	...
	if(KEY_PRESS_DOWN(QIN_b)) player[g_Player]->HB_Energy = player[g_Player]->HB_MaxEnergy;
...
#if 0
	Qpo->Transform_AvatarGetLeft(&playerAV[g_Player]->Xform, /*ORIG GetUp*/&playerAV[g_Player]->Up);
	Qpo->Camera_Set();
...
#if 0
	if(KEY_PRESS_DOWN(QIN_UP)) playerAV[g_Player]->Avatar.Mesh->m_ClothAttachment[0]->getNxCloth()->addForceAtPos(PxVec3(0,-250,0), PxVec3(1,0,0), 500);
#endif
...
#if 0
	if(KEY_PRESS_DOWN(QIN_RBUTTON) && !KEY_PRESS_DOWN(QIN_SHIFT) && !g_render->fullscreen)
		{
		if(!vecvarDpress[4])
			{
			vecvarDpress[4] = true;
