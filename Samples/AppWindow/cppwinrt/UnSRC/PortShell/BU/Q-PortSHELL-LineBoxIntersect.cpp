...
bool Q_PORTShell::Collision_RayTest_Avatar(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Front, QpoVec3d *Rear, int UserFlags, bool Collision, bool *Context, QpoCollision *Col, int f_AvID, bool f_Impactor)
	...
	PxVec3 f_Impact;
	bool f_Result = false;
	bool f_PLResult = false;
	if(f_Impactor && 0) f_Result = QActiveFrameWork->DigiMass->acRayColliderImpact(Col, PxVec3(Front->x,Front->y,Front->z), PxVec3(Rear->x,Rear->y,Rear->z), f_Impact, distance);
	else f_Result = QActiveFrameWork->DigiMass->acRayCollider(Col, PxVec3(Front->x,Front->y,Front->z), PxVec3(Rear->x,Rear->y,Rear->z), f_Impact, distance);
	QpoVec3d* Hit = new QpoVec3d();
	...
	if(f_AvID != -1)
		{
		QpoVec3d B1;
		B1.x = player->Mins.x + playerAV->Xform.Translation.x;
		B1.y = player->Mins.y + playerAV->Xform.Translation.y;
		B1.z = player->Mins.z + playerAV->Xform.Translation.z;
		QpoVec3d B2;
		B2.x = player->Maxs.x + playerAV->Xform.Translation.x;
		B2.y = player->Maxs.y + playerAV->Xform.Translation.y;
		B2.z = player->Maxs.z + playerAV->Xform.Translation.z;
		f_PLResult = CheckLineBox(B1, B2, *Front, *Rear, Hit);
		...
		for(int hellyjet = 0; hellyjet < _->NomAI; ++hellyjet)
			{
			if(Q_Forge->Forge[hellyjet].LIVE == 1 && hellyjet != f_AvID)
				{
				QpoVec3d B1;
				B1.x = Q_Forge->Forge[hellyjet].Mins.x + Q_Forge->Forge[hellyjet].AV.Xform.Translation.x;
				B1.y = Q_Forge->Forge[hellyjet].Mins.y + Q_Forge->Forge[hellyjet].AV.Xform.Translation.y;
				B1.z = Q_Forge->Forge[hellyjet].Mins.z + Q_Forge->Forge[hellyjet].AV.Xform.Translation.z;
				QpoVec3d B2;
				B2.x = Q_Forge->Forge[hellyjet].Maxs.x + Q_Forge->Forge[hellyjet].AV.Xform.Translation.x;
				B2.y = Q_Forge->Forge[hellyjet].Maxs.y + Q_Forge->Forge[hellyjet].AV.Xform.Translation.y;
				B2.z = Q_Forge->Forge[hellyjet].Maxs.z + Q_Forge->Forge[hellyjet].AV.Xform.Translation.z;
				f_PLResult = CheckLineBox(B1, B2, *Front, *Rear, Hit);
				...
				if(f_PLResult)
					...
					Col->m_HitAvatar = true;
					Col->Avatar = Q_Forge->Forge[hellyjet].AV.Avatar;
					Col->m_AvatarID = hellyjet;
	...
	if(f_Result == true)
		...
		Col->Impact = QpoVec3d(f_Impact.x, f_Impact.y, f_Impact.z);
...
void Q_PORTShell::Avatar_GetBoneTransformMP(QpoAvatarFlags f_AV, const char * String, QpoTransform * Bone)
	...
	m_CC.pResult.x += m_CC.pResult.x * (f_AV.Avatar.Scale.x * SCALE_IMP);
	m_CC.pResult.y += m_CC.pResult.y * (f_AV.Avatar.Scale.y * SCALE_IMP);
	m_CC.pResult.z += m_CC.pResult.z * (f_AV.Avatar.Scale.z * SCALE_IMP);
...
void Q_PORTShell::Avatar_GetTransformAvatar(QpoAvatarFlags * f_AV, const char * String, QpoTransform * Bone, int AIcounters)
	...
	bonevector.x += bonevector.x * (f_AV->Avatar.Scale.x * SCALE_IMP);
	bonevector.y += bonevector.y * (f_AV->Avatar.Scale.y * SCALE_IMP);
	bonevector.z += bonevector.z * (f_AV->Avatar.Scale.z * SCALE_IMP);
...
void Q_PORTShell::Avatar_GetWeaponTransform_s(QpoAvatarFlags * i_AV, const char * String, QpoTransform * Bone)
	...
	bonevector.x += bonevector.x * (playerAV->Avatar.Scale.x * SCALE_IMP);
	bonevector.y += bonevector.y * (playerAV->Avatar.Scale.y * SCALE_IMP);
	bonevector.z += bonevector.z * (playerAV->Avatar.Scale.z * SCALE_IMP);
...
#pragma message("FIXME:::Rotate Matrix and transform by player matrix")
	...
	m_CC.pResult.x += m_CC.pResult.x * (i_AV->Avatar.Scale.x * SCALE_IMP);
	m_CC.pResult.y += m_CC.pResult.y * (i_AV->Avatar.Scale.y * SCALE_IMP);
	m_CC.pResult.z += m_CC.pResult.z * (i_AV->Avatar.Scale.z * SCALE_IMP);
