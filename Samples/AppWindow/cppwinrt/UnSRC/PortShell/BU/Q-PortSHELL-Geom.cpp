...
bool Q_PORTShell::Collision_RayTest_Avatar(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Front, QpoVec3d *Rear, int UserFlags, bool Collision, bool *Context, QpoCollision *Col, int f_AvID, bool f_Impactor)
	...
	QpoVec3d f_Dir;
	Qpo->Vec3d_Subtract(Front, Rear, &f_Dir);
	float f_Length = Qpo->Vec3d_Length(&f_Dir);
	Qpo->Vec3d_Normalize(&f_Dir);

	//###-- AI detect Player --###
	if(f_AvID != -1)
		{
		f_Center = QpoVec3d(playerAV->Xform.Translation.x, playerAV->Xform.Translation.y + (45 + (40*(((player->BFO.CharDATA[player->BFO.CurrCHAR].BodySKILL.Height/100)-0.5)*2))), playerAV->Xform.Translation.z);
		radius2 = (8 + (5 * (((player->BFO.CharDATA[player->BFO.CurrCHAR].BodySKILL.Height/100)-0.5)*2)));

		// geometric solution
		QpoVec3d L;
		L.x = f_Center.x - Front->x;
		L.y = f_Center.y - Front->y;
		L.z = f_Center.z - Front->z;

		float tca;
		QpoVec3d f_Loop;
		Qpo->Vec3d_AddScaled(Front, Qpo->Vec3d_Length(&L), &f_Dir, &f_Loop);
		tca = Qpo->Vec3d_Length(&f_Loop);

		if(tca >= 0)
			{
			float d2 = Qpo->Vec3d_DotProduct(&L, &L) - tca * tca;
			if(d2 <= radius2)
				{
				...
				if(t0 <= f_Length)
					...
					Qpo->Vec3d_AddScaled(Front, t0, &f_Dir, &Col->Impact);
		...
		for(int hellyjet = 0; hellyjet < _->NomAI; ++hellyjet)
			...
			if(Q_Forge->Forge[hellyjet].LIVE == 1 && hellyjet != f_AvID)
				{
				f_Center = QpoVec3d(Q_Forge->Forge[hellyjet].AV.Xform.Translation.x, Q_Forge->Forge[hellyjet].AV.Xform.Translation.y + (45 + (40*(((Q_Forge->Forge[hellyjet].CharDATA.BodySKILL.Height/100)-0.5)*2))), Q_Forge->Forge[hellyjet].AV.Xform.Translation.z);
				radius2 = (8 + (5 * (((Q_Forge->Forge[hellyjet].CharDATA.BodySKILL.Height/100)-0.5)*2)));
...
void Q_PORTShell::Avatar_GetWeaponTransform_s(QpoAvatarFlags * i_AV, const char * String, QpoTransform * Bone, bool f_IsPlayer)
	...
	bonevector.y *= playerAV->Avatar.Scale.y;
