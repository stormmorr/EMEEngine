...
bool Q_PORTShell::Sound_Init(void)
	...
	//Generate Sources
	alGenSources(32, &_->SoundSource[0]);
	...
	if (ALFWIsEFXSupported())
		...
		if (CreateAuxEffectSlot(&uiEffectSlot))
			...
			{
			printf("Failed to generate an Auxilary Effect Slot\n");
			}

		if (bEffectCreated)
			{
			printf("Playing Source with Send to 'Hangar' Reverb Effect\n");
			...
			printf("Playing Source with Send to 'Bathroom' Reverb Effect\n");
		...
		{
		printf("AL Device does not support EFX\n");
		}
#endif
	...
	// of BUFFERSIZE Samples (== BUFFERSIZE * 2 bytes)			//22050
	pCaptureDevice = alcCaptureOpenDevice(szDefaultCaptureDevice, 22050, AL_FORMAT_MONO16, QPO_CAPTURE_RATE);
	if (pCaptureDevice)
		{
	...
	if(Mix_OpenAudio(22050, MIX_DEFAULT_FORMAT, 16, 4096) == -1)
		{
...
#pragma message("Server engine excludes")
	...
	if(!ALFWLoadWaveToBuffer((char*)File, uiBuffer))
		{
...
#pragma message("Server engine excludes")
	...
	for(int i_Iter = 0; i_Iter < 32; i_Iter++)
		{
		alGetSourcei(_->SoundSource[i_Iter], AL_SOURCE_STATE, &iState);
		if(iState == AL_PLAYING) _->SoundQueue[i_Iter] = 1;
		else _->SoundQueue[i_Iter] = 0;
		}

	float vec[6];

	Qpo->Transform_GetRight(&player->Camrot,&player->In);

	vec[0] = player->In.x;
	vec[1] = player->In.y;
	vec[2] = player->In.z;

	Qpo->Transform_GetUp(&player->Camrot,&player->In);

	vec[3] = player->In.x;
	vec[4] = player->In.y;
	vec[5] = player->In.z;

	alListener3f(AL_POSITION, g_render->camera.pos.x, g_render->camera.pos.y, g_render->camera.pos.z);
...
#pragma message("Server engine excludes")
#if defined(WIN32)
#if defined(OPENAL)
	int i_SourceIdent = Qpo->Sound_FindSource();
	if(i_SourceIdent == -1) return -1;

	ALuint i_Source = _->SoundSource[i_SourceIdent];

	// Attach Buffer to Source
	alSourcei(i_Source, AL_BUFFER, SoundDef);
	...
	alSource3f(i_Source, AL_VELOCITY, 0, 0, 0);

	alSourcef(i_Source, AL_MAX_DISTANCE, 1000.0f);

	alSourcef(i_Source, AL_MIN_GAIN, 1.0f);

	alSourcef(i_Source, AL_ROLLOFF_FACTOR, 1.0f);

	alSourcePlay(i_Source);

	return i_SourceIdent;
#else
	QpoVec3d f_Aerials = playerAV->Xform.Translation;
	QpoVec3d f_VecTo;
	float f_LengthVecTo;

	f_Aerials.y += (75 + (30*(((player->CDATA.CharDATA[player->CDATA.CurrCHAR].BodySKILL.Height/100)-0.5)*2)));

	Qpo->Vec3d_Subtract(Position, &f_Aerials, &f_VecTo);
	f_LengthVecTo = Qpo->Vec3d_Length(&f_VecTo);

	if(f_LengthVecTo > 255) f_LengthVecTo = 255;
	if(f_LengthVecTo < 0) f_LengthVecTo = 0;

	if(Mix_SetPosition(m_ChannelPNT, player->Angles.y, f_LengthVecTo) == -1)
		{
	...
	m_ChannelPNT++;
	if(m_ChannelPNT >= 16) m_ChannelPNT = 0;
#endif
#endif
	}
...
#pragma message("Server engine excludes")
	...
	Sound_SetEffect(i_Source, i_RefreshEFX);
...
#if defined(WIN32)
#if OPENAL
	_->SoundQueue[Sound] = 0;
	alSourceStop(_->SoundSource[Sound]);
#else
...
#if defined(WIN32)
#if OPENAL
	alDeleteSources(32, &_->SoundSource[0]);
...
void Q_PORTShell::Tex_Blit(unsigned int i_texture, QpoRect *Brushrect, int x, int y)
	...
	glVertex2d(x, y);
	glTexCoord2i(0, 0);
	glVertex2d(x + Brushrect->xmax, y);
	glTexCoord2i(0, 1);
	glVertex2d(x + Brushrect->xmax, y + Brushrect->ymax);
	glTexCoord2i(1, 1);
	glVertex2d(x, y + Brushrect->ymax);
	glEnd();
...
bool Q_PORTShell::Collision_RayTest(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Front, QpoVec3d *Back, int UserFlags, bool Collision, bool *Context, QpoCollision *Col)
	{
#if 0
	pVector front(Front->x, Front->y, Front->z);
	pVector back(Back->x, Back->y, Back->z);
	...
	Vec3d_Subtract(Back, Front, &i_ray);
...
#pragma message("FIXME optimise ray picker for try concave selection")
	...
	float distance;
	PxVec3 i_impact;
	bool i_result = Digi::DigiMassCore::instance()->acRayCollider(Col, PxVec3(Front->x,Front->y,Front->z), PxVec3(Back->x,Back->y,Back->z), i_impact, distance);
...
bool Q_PORTShell::Collision_RayTest_Avatar(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Front, QpoVec3d *Back, int UserFlags, bool Collision, bool *Context, QpoCollision *Col)
	{
...
#pragma message("FIXME optimise ray picker for try concave selection")
	...
	float distance;
	PxVec3 i_impact;
	int i_result = Digi::DigiMassCore::instance()->acRayCollider(Col, PxVec3(Front->x,Front->y,Front->z), PxVec3(Back->x,Back->y,Back->z), i_impact, distance);
#endif
...
#pragma message("FIXME optimise ray picker for try concave selection")
...
#if 0
	Col->m_HitAvatar = false;
	if(!(((playerAV->Xform.Translation.x + playerAV->Mins.x) > (Col->Impact.x + 25)) || ((playerAV->Xform.Translation.x + playerAV->Maxs.x) < (Col->Impact.x - 25))) &&
	!(((playerAV->Xform.Translation.y + playerAV->Mins.y) > (Col->Impact.y + 25)) || ((playerAV->Xform.Translation.y + playerAV->Maxs.y) < (Col->Impact.y - 25))) &&
	!(((playerAV->Xform.Translation.z + playerAV->Mins.z) > (Col->Impact.z + 25)) || ((playerAV->Xform.Translation.z + playerAV->Maxs.z) < (Col->Impact.z - 25))))
		{
		Col->Avatar = playerAV->Avatar;
		Col->m_HitAvatar = true;
		}
	...
	while(SCRAM < _->NomAI)
		...
		if(Q_Forge->Forge[SCRAM].LIVE == 1)
			{
			if(!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.x + Q_Forge->Forge[SCRAM].AV.Mins.x) > (Col->Impact.x + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.x + Q_Forge->Forge[SCRAM].AV.Maxs.x) < (Col->Impact.x - 25))) &&
			!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.y + Q_Forge->Forge[SCRAM].AV.Mins.y) > (Col->Impact.y + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.y + Q_Forge->Forge[SCRAM].AV.Maxs.y) < (Col->Impact.y - 25))) &&
			!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Mins.z) > (Col->Impact.z + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Maxs.z) < (Col->Impact.z - 25))))
				{
				Col->Avatar = Q_Forge->Forge[SCRAM].AV.Avatar;
				Col->m_HitAvatar = true;
				}
...
bool Q_PORTShell::Collision_RayTest_Projectile(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Front, QpoVec3d *Back, int UserFlags, bool Collision, bool *Context, QpoCollision *Col)
	{
#if defined(ENABLE_PX_PHYSICS)
#if 1
	float distance;
...
#pragma message("Server Engine excludes")
	...
	g_render->camera.mat_t.m[3][3]=1;

	//g_render->camera.mat = playerAV->Xform.Matrix;
	g_render->camera.pos.x = Position.x;
	g_render->camera.pos.y = Position.y;
	g_render->camera.pos.z = Position.z;
