...
#pragma message("FIXME optimise ray picker for try concave selection")
	...
	PxVec3 i_impact;
	bool i_result = false;
	i_result = QActiveFrameWork->DigiMass->acRayCollider(Col, PxVec3(Front->x,Front->y,Front->z), PxVec3(Rear->x,Rear->y,Rear->z), i_impact, distance);
#else
	bool i_result;
	float distance;
	pVector i_impact;
	...
	if(!i_result || 1)
		...
		if(f_AvID >= 0)
			{
			QpoVec3d RayFull;
			Qpo->Vec3d_Subtract(Front, Rear, &RayFull);

			float f_RFLen = Qpo->Vec3d_Length(&RayFull);
			if(f_RFLen < 0.01) return false;

			QpoVec3d VecTo = RayFull;
			Qpo->Vec3d_Normalize(&VecTo);

			QpoVec3d xyzlength;
			Qpo->Vec3d_Subtract(Mins, Maxs, &xyzlength);

			float chklength = 0;
			bool f_BusyScan = true;
			while(f_BusyScan)
				{
				chklength += 3;
				if(chklength > f_RFLen) f_BusyScan = false;
				...
				RayMover = *Rear;
				Qpo->Vec3d_AddScaled(Rear, chklength, &VecTo, &RayMover);
				Qpo->Vec3d_Subtract(&RayMover, Rear, &RayRemains);

				float f_Length = Qpo->Vec3d_Length(&RayRemains);
			
				if(f_Length < f_RFLen && f_Length < 8000)
					{
					if(!(((playerAV->Xform.Translation.x + playerAV->Mins.x) > (RayMover.x + 25)) || ((playerAV->Xform.Translation.x + playerAV->Maxs.x) < (RayMover.x - 25))) &&
					!(((playerAV->Xform.Translation.y + playerAV->Mins.y) > (RayMover.y + 25)) || ((playerAV->Xform.Translation.y + playerAV->Maxs.y) < (RayMover.y - 25))) &&
					!(((playerAV->Xform.Translation.z + playerAV->Mins.z) > (RayMover.z + 25)) || ((playerAV->Xform.Translation.z + playerAV->Maxs.z) < (RayMover.z - 25))))
						{
						Col->m_AvatarID = -1;
						Col->Avatar.IDflag = -1;
						Col->m_HitAvatar = true;

						Col->Impact.x = RayMover.x;
						Col->Impact.y = RayMover.y;
						Col->Impact.z = RayMover.z;

						Col->Plane.Dist = 0;
						Col->Plane.Normal = VecTo;
						Qpo->Vec3d_Inverse(&Col->Plane.Normal);
						return true;
						}
					...
					while(SCRAM < _->NomAI)
						{
						if(Q_Forge->Forge[SCRAM].LIVE == 1 && SCRAM != f_AvID)
							{
							if(!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.x + Q_Forge->Forge[SCRAM].AV.Mins.x) > (RayMover.x + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.x + Q_Forge->Forge[SCRAM].AV.Maxs.x) < (RayMover.x - 25))) &&
								!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.y + Q_Forge->Forge[SCRAM].AV.Mins.y) > (RayMover.y + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.y + Q_Forge->Forge[SCRAM].AV.Maxs.y) < (RayMover.y - 25))) &&
								!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Mins.z) > (RayMover.z + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Maxs.z) < (RayMover.z - 25))))
								{
								Col->m_AvatarID = SCRAM;
								Col->Avatar.IDflag = SCRAM;
								Col->m_HitAvatar = true;
					...
					//***- Construct Eight Star Spygram Vector Positions -***
					QpoVec3d f_EightStarSGV[8];
					f_EightStarSGV[0].acSet(RayMover.x - ((Maxs->x - Mins->x) * 0.5), RayMover.y + ((Maxs->y - Mins->y) * 0.5), RayMover.z - ((Maxs->z - Mins->z) * 0.5));
					f_EightStarSGV[1].acSet(RayMover.x + ((Maxs->x - Mins->x) * 0.5), RayMover.y + ((Maxs->y - Mins->y) * 0.5), RayMover.z - ((Maxs->z - Mins->z) * 0.5));
					f_EightStarSGV[2].acSet(RayMover.x - ((Maxs->x - Mins->x) * 0.5), RayMover.y + ((Maxs->y - Mins->y) * 0.5), RayMover.z + ((Maxs->z - Mins->z) * 0.5));
					f_EightStarSGV[3].acSet(RayMover.x + ((Maxs->x - Mins->x) * 0.5), RayMover.y + ((Maxs->y - Mins->y) * 0.5), RayMover.z + ((Maxs->z - Mins->z) * 0.5));
					f_EightStarSGV[4].acSet(RayMover.x - ((Maxs->x - Mins->x) * 0.5), RayMover.y - ((Maxs->y - Mins->y) * 0.5), RayMover.z - ((Maxs->z - Mins->z) * 0.5));
					f_EightStarSGV[5].acSet(RayMover.x + ((Maxs->x - Mins->x) * 0.5), RayMover.y - ((Maxs->y - Mins->y) * 0.5), RayMover.z - ((Maxs->z - Mins->z) * 0.5));
					f_EightStarSGV[6].acSet(RayMover.x - ((Maxs->x - Mins->x) * 0.5), RayMover.y - ((Maxs->y - Mins->y) * 0.5), RayMover.z + ((Maxs->z - Mins->z) * 0.5));
					f_EightStarSGV[7].acSet(RayMover.x + ((Maxs->x - Mins->x) * 0.5), RayMover.y - ((Maxs->y - Mins->y) * 0.5), RayMover.z + ((Maxs->z - Mins->z) * 0.5));

#if 1
					QpoCollision f_QpoTaiPei;
					float distanceTaiPei;
					PxVec3 i_impactTaiPei;
					bool i_resultTaiPei = false;
					for(int f_Count = 0; f_Count < 8; f_Count++)
						{
						i_resultTaiPei = QActiveFrameWork->DigiMass->acRayCollider(&f_QpoTaiPei, PxVec3(RayMover.x, RayMover.y, RayMover.z), PxVec3(f_EightStarSGV[f_Count].x, f_EightStarSGV[f_Count].y, f_EightStarSGV[f_Count].z), i_impactTaiPei, distanceTaiPei);
						}
#else
					bool i_result;
					float distance;
					...
					if(i_resultTaiPei == true)
						...
						if(i_resultTaiPei == true)
							...
							Qpo->Vec3d_Subtract(Rear, &RayMover, &i_ray);
							Qpo->Vec3d_Subtract(&Col->Impact, &RayMover, &i_dest);
	...
	while(SCRAM < _->NomAI)
		...
		if(Q_Forge->Forge[SCRAM].LIVE == 1)
			...
			!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Mins.z) > (Front->z + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Maxs.z) < (Front->z - 25))))
				{
				//Col->Avatar = Q_Forge->Forge[SCRAM].AV.Avatar;
				Col->m_AvatarID = SCRAM;
				Col->Avatar.IDflag = SCRAM;
				Col->m_HitAvatar = true;

				Col->Impact.x = Front->x;
				Col->Impact.y = Front->y;
				Col->Impact.z = Front->z;
				//Col->Avatar = 0;
		...
		while(SCRAM < _->NomAI)
			...
			if(Q_Forge->Forge[SCRAM].LIVE == 1 && SCRAM != f_AvID)
				{
				float xlength = Q_Forge->Forge[SCRAM].AV.Maxs.x - Q_Forge->Forge[SCRAM].AV.Mins.x;
				float ylength = Q_Forge->Forge[SCRAM].AV.Maxs.y - Q_Forge->Forge[SCRAM].AV.Mins.y;
				float zlength = Q_Forge->Forge[SCRAM].AV.Maxs.z - Q_Forge->Forge[SCRAM].AV.Mins.z;
				float chklength = 0;
				int lengthdivisor;
				...
				if(f_RFLen < 0.01) return false;
				if(chklength > f_RFLen) chklength = f_RFLen / 8;
				QpoVec3d RayRemains;
				QpoVec3d VecTo;
				VecTo = RayFull;
				Qpo->Vec3d_Normalize(&VecTo);
				Qpo->Vec3d_Subtract(&RayMover, Rear, &RayRemains);
				float f_Length = Qpo->Vec3d_Length(&RayRemains);
				//printf(":: f_Length %f < %f ::: f_RFLen %f + chklength %f :", f_Length, f_RFLen + chklength,  f_RFLen, chklength);
				while(f_Length < f_RFLen + chklength)
					{
					if(!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.x + Q_Forge->Forge[SCRAM].AV.Mins.x) > (RayMover.x + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.x + Q_Forge->Forge[SCRAM].AV.Maxs.x) < (RayMover.x - 25))) &&
					!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.y + Q_Forge->Forge[SCRAM].AV.Mins.y) > (RayMover.y + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.y + Q_Forge->Forge[SCRAM].AV.Maxs.y) < (RayMover.y - 25))) &&
					!(((Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Mins.z) > (RayMover.z + 25)) || ((Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Maxs.z) < (RayMover.z - 25))))
					...
					//VecTo
					Qpo->Vec3d_AddScaled(&RayMover, chklength, &VecTo, &RayMover);
					Qpo->Vec3d_Subtract(&RayMover, Rear, &RayRemains);
					f_Length = Qpo->Vec3d_Length(&RayRemains);
					//printf("--: f_Length %f < %f ::: RFLen %f + chk %f : ", f_Length, f_RFLen + chklength,  f_RFLen, chklength);
					}
				}
		...
		while(SCRAM < _->NomAI)
			...
			if(Q_Forge->Forge[SCRAM].LIVE == 1)
				{
				pBoundBox bbox;
				bbox.min.x = Q_Forge->Forge[SCRAM].AV.Xform.Translation.x + Q_Forge->Forge[SCRAM].AV.Mins.x;
				bbox.min.y = Q_Forge->Forge[SCRAM].AV.Xform.Translation.y + 45 + Q_Forge->Forge[SCRAM].AV.Mins.y;
				bbox.min.z = Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Mins.z;
				bbox.max.x = Q_Forge->Forge[SCRAM].AV.Xform.Translation.x + Q_Forge->Forge[SCRAM].AV.Maxs.x;
				bbox.max.y = Q_Forge->Forge[SCRAM].AV.Xform.Translation.y + 45 + Q_Forge->Forge[SCRAM].AV.Maxs.y;
				bbox.max.z = Q_Forge->Forge[SCRAM].AV.Xform.Translation.z + Q_Forge->Forge[SCRAM].AV.Maxs.z;
				pVector ro,rd;
				ro.x = Rear->x;
				ro.y = Rear->y;
				ro.z = Rear->z;
				...
				if(bbox.ray_intersect(ro, rd, pvnear, pvfar) != -1)
					{
					//Col->Avatar = Q_Forge->Forge[SCRAM].AV.Avatar;
					Col->m_AvatarID = SCRAM;
					Col->Avatar.IDflag = SCRAM;
					...
					QpoVec3d Dir;
					Qpo->Vec3d_Subtract(Front, Rear, &Dir);
					Qpo->Vec3d_Normalize(&Dir);
					Qpo->Vec3d_Inverse(&Dir);

					Col->Plane.Dist = 0;
					Col->Plane.Normal = Dir;
