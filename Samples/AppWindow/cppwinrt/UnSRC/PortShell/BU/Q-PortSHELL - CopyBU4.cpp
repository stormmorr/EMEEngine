		...
		if(f_AvID >= 0)
			...
			while(f_BusyScan)
				...
				if(f_Length < f_RFLen && f_Length < 8000)
					...
					float distance8;
					PxVec3 i_impact8;
					bool i_result8 = false;
					for(int f_Count = 0; f_Count < 8; f_Count++)
						{
						if(f_Impact && 0) i_result8 = QActiveFrameWork->DigiMass->acRayColliderImpact(&f_Qpo8, PxVec3(RayMover.x, RayMover.y, RayMover.z), PxVec3(Rear->x,Rear->y,Rear->z), i_impact8, distance8);
						else i_result8 = QActiveFrameWork->DigiMass->acRayCollider(&f_Qpo8, PxVec3(RayMover.x, RayMover.y, RayMover.z), PxVec3(f_EightStarSGV[f_Count].x, f_EightStarSGV[f_Count].y, f_EightStarSGV[f_Count].z), i_impact8, distance8);
						}
#else
					bool i_result;
					float distance;
