...
QpoPolygon * Q_PORTShell::Poly_Add(QpoVertex *Verts, int NumVerts, unsigned int Bitmap, Uint8 TypeFLAGS, Uint8 RendFLAGS, float Scale)
	{
#if 0
	//##==-- POLY
...
#pragma message("Server engine excludes")
#if defined(WIN32)
#if 0
	int nl = mesh.nlight - 1;
	int i_counter = 0;

	pLight *tmp = new pLight[nl];

	for(int i=0; i < mesh.nlight && i < nl; i++)
		{
		if(i_counter != light)
			{
			tmp[i_counter] = mesh.light[i];
			i_counter++;
			}
	...
	mesh.light[(int)light].m_Live = false;
#endif
	}
...
#pragma message("Server engine excludes")
#if defined(WIN32)
	for(int k=0; k < mesh.nlight; k++)
		{
		if(!mesh.light[k].m_Live)
			{
			mesh.light[k].m_Live = true;
			mesh.light[k].pos.x = pos.x;
			mesh.light[k].pos.y = pos.y;
			mesh.light[k].pos.z = pos.z;
			mesh.light[k].pos.w = 1;

			mesh.light[k].color.x = 1;
			mesh.light[k].color.y = 1;
			mesh.light[k].color.z = 1;

#if 0
			mesh.light[k].color.x = 0.175;
			mesh.light[k].color.y = 0.175;
			mesh.light[k].color.z = 0.175;
#endif

			mesh.light[k].flags = 3;
	...
	pLight *tmp=0;
	tmp=new pLight[nl];
	/*if (mesh.light)*/ for(int i=0; i<mesh.nlight&&i<nl ;i++) tmp[i] = mesh.light[i];

	delete[] mesh.light;
	mesh.light=tmp;
	mesh.nlight=nl;

	pLight *i_Light = &mesh.light[mesh.nlight - 1];
...
#pragma message("Server engine excludes")
#if defined(WIN32)
	if(Light < mesh.nlight)
		{
		pLight *i_Light = &mesh.light[(int)Light];
...
bool Q_PORTShell::Collision_RayTest(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Front, QpoVec3d *Back, int UserFlags, bool Collision, bool *Context, QpoCollision *Col)
	...
	float distance;
	pVector i_impact;
	int ret = mesh.ray_intersect(front, back, i_impact, distance);
...
bool Q_PORTShell::Collision_RayTest_Avatar(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Front, QpoVec3d *Back, int UserFlags, bool Collision, bool *Context, QpoCollision *Col, int f_AvID)
	{
#if defined(ENABLE_PX_PHYSICS)
#if 0
	pVector front(Front->x, Front->y, Front->z);
...
void Q_PORTShell::Avatar_GetBoneTransform(QpoAvatarFlags * i_AV, const char * String, QpoTransform * Bone)
	{
	CalBone *i_bone = i_AV->Avatar.Mesh->m_calModel->getSkeleton()->getBone(m_mapSkeletonA[String]);
...
void Q_PORTShell::AvatarView_GetBoneTransform(QpoAvatarFlags * i_AV, const char * String, QpoTransform * Bone)
	{
	CalBone *f_bone = i_AV->Avatar.Mesh->m_calModel->getSkeleton()->getBone(m_mapSkeletonA[String]);
...
void Q_PORTShell::Avatar_WeaponView_GetBoneTransform(int i_AvatarIndex, const char * String, QpoTransform * Bone)
	{
	CalBone *i_bone = Q_Forge->Forge[i_AvatarIndex].AV.Avatar.Mesh->m_calModel->getSkeleton()->getBone(m_mapSkeletonA[String]);
	CalQuaternion boneort = i_bone->getRotationAbsolute();
	CalVector bonetrans = i_bone->getTranslationAbsolute();
...
void Q_PORTShell::Player_GetBoneBoundingBox(const char *BoneName, QpoVector *Corner, QpoVec3d *DX, QpoVec3d *DY, QpoVec3d *DZ)
	{
	CalBone *i_bone = playerAV->Avatar.Mesh->m_calModel->getSkeleton()->getBone(m_mapSkeletonA[BoneName]);
...
void Q_PORTShell::Avatar_GetBoneBoundingBox(unsigned int i_AVIndex, const char *BoneName, QpoVector *Corner, QpoVec3d *DX, QpoVec3d *DY, QpoVec3d *DZ)
	{
	CalBone *i_bone = Q_Forge->Forge[i_AVIndex].AV.Avatar.Mesh->m_calModel->getSkeleton()->getBone(m_mapSkeletonA[BoneName]);
	i_bone->calculateBoundingBox();
	CalBoundingBox & i_BoneBox = i_bone->getBoundingBox();
