...
bool Q_PORTShell::Collision_RayTest(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Back, QpoVec3d *Front, int UserFlags, bool Collision, bool *Context, QpoCollision *Col)
{
	...
	std::shared_ptr<VoxelGrid> f_backGrid = nullptr;
	int f_backint = -1;
	...
	for(int i = 0; i < f_i; i++)
		...
		if((back.m_X >= ((float)f_Grid->m_gridX * GRIDSIZE)) && 
			(back.m_X < (((float)f_Grid->m_gridX + 1) * GRIDSIZE)) && 
			(back.m_Y >= ((float)f_Grid->m_gridY * GRIDSIZE)) && 
			(back.m_Y < (((float)f_Grid->m_gridY + 1) * GRIDSIZE)) && 
			(back.m_Z >= ((float)(f_Grid->m_gridZ * -1) * GRIDSIZE)) && 
			(back.m_Z < (((float)(f_Grid->m_gridZ * -1) + 1) * GRIDSIZE)))
			{
			f_backGrid = f_Grid;
			f_backint = i;
			}
	...
	std::shared_ptr<VoxelGrid> f_frontGrid = nullptr;
	int f_frontint = -1;
	...
	if((f_backint >= 0) && (f_frontint >= 0))
		...
		if(f_backint == f_frontint)
			{ //both the same
			f_Collide = f_frontGrid->CollideMarchPoint(front, back, Col);

			if(f_Collide)
				{
				Col->m_VXGrid = f_frontGrid;
			...
			f_Collide = f_backGrid->CollideMarchPoint(front, back, Col);

			if(f_Collide)
				{
				Col->m_VXGrid = f_backGrid;
				Col->Plane.Normal.acSet(Col->Normal.m_X, Col->Normal.m_Y, Col->Normal.m_Z);
