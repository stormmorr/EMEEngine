...
bool ag_PortShell_Initialisation(void)
{
	Qpo = new Q_PORTShell();
	...
	// sets up a transform that rotates RadianAngle about X axis
{
	...
	Qpo->m_CC.Cos = (float)cos(RadianAngle);
	Qpo->m_CC.Sin = (float)sin(RadianAngle);

#if 0
	transform->Matrix.m[1][1] =  Qpo->m_CC.Cos;
	transform->Matrix.m[1][2] = -Qpo->m_CC.Sin;
	transform->Matrix.m[2][1] =  Qpo->m_CC.Sin;
	transform->Matrix.m[2][2] =  Qpo->m_CC.Cos;
	...
	// sets up a transform that rotates RadianAngle about Y axis
{
	...
	transform->Matrix.m[0][0] =  Qpo->m_CC.Cos;
	transform->Matrix.m[0][2] =  Qpo->m_CC.Sin;
	transform->Matrix.m[2][0] = -Qpo->m_CC.Sin;
	transform->Matrix.m[2][2] =  Qpo->m_CC.Cos;
	...
	// sets up a transform that rotates RadianAngle about Z axis
{
	...
	transform->Matrix.m[0][0] =  Qpo->m_CC.Cos;
	transform->Matrix.m[0][1] = -Qpo->m_CC.Sin;
	transform->Matrix.m[1][0] =  Qpo->m_CC.Sin;
	transform->Matrix.m[1][1] =  Qpo->m_CC.Cos;
...
void Q_PORTShell::Transform_GetIn(QpoTransform * transform, QpoVec3d * result)
	...
	{
	result->x = -transform->Matrix.m[0][2];
	result->y = -transform->Matrix.m[1][2];
	result->z = -transform->Matrix.m[2][2];
	}
...
void Q_PORTShell::Transform_GetLeft(QpoTransform * transform, QpoVec3d * result)
	...
	result->y = -transform->Matrix.m[1][0];
	result->z = -transform->Matrix.m[2][0];
	}
...
void Q_PORTShell::Transform_GetUp(QpoTransform * transform, QpoVec3d * result)
	// Gets a vector that is 'up' in the frame of reference of transform->Matrix.getMatrix() (facing -Z)
	{
	result->x = transform->Matrix.m[2][1];
	result->y = transform->Matrix.m[1][1];
	result->z = transform->Matrix.m[0][1];

	//before not used
	//result->x = transform->Matrix.m[2][0];
	//result->y = transform->Matrix.m[1][0];
	//result->z = transform->Matrix.m[0][0];
	}

//###- Avatar Specific Transform Local Axis -###//

void Q_PORTShell::Transform_AvatarGetIn(QpoTransform * transform, QpoVec3d * result)
	// Gets a vector that is 'in' in the frame of reference of transform->Matrix.getMatrix() (facing -Z)
	{
	//////result->x = -transform->Matrix.m[1][2];
	//////result->y = -transform->Matrix.m[2][2];
...
void Q_PORTShell::Transform_AvatarGetLeft(QpoTransform * transform, QpoVec3d * result)
	// Gets a vector that is 'left' in the frame of reference of transform->Matrix.getMatrix() (facing -Z)
	{
	////result->x = -transform->Matrix.m[0][1];
	////result->y = -transform->Matrix.m[1][1];
	////result->z = -transform->Matrix.m[2][1];
...
void Q_PORTShell::Transform_AvatarGetRight(QpoTransform * transform, QpoVec3d * result)
	// Gets a vector that is 'left' in the frame of reference of transform->Matrix.getMatrix() (facing -Z)
	{
	/////result->x = transform->Matrix.m[0][1];
	/////result->y = transform->Matrix.m[1][1];
...
void Q_PORTShell::Transform_AvatarGetUp(QpoTransform * transform, QpoVec3d * result)
	// Gets a vector that is 'up' in the frame of reference of transform->Matrix.getMatrix() (facing -Z)
	{
	/////result->x = transform->Matrix.m[1][0];
	/////result->y = transform->Matrix.m[2][0];
...
void Q_PORTShell::Transform_GetIn(pMatrix * transform, QpoVec3d * result)
	...
	result->y = -transform->m[1][2];
	result->z = -transform->m[2][2];
	}
...
void Q_PORTShell::Transform_GetUp(pMatrix * transform, QpoVec3d * result)
	// Gets a vector that is 'up' in the frame of reference of Matrix.getMatrix() (facing -Z)
	{
	result->x = transform->m[0][1];
	result->y = transform->m[1][1];
	result->z = transform->m[2][1];
	}
#endif
...
void Q_PORTShell::Transform_RotateVector(QpoTransform * transform, QpoVec3d * vec, QpoVec3d * result)
	...
	Qpo->m_CC.pVector1.x = vec->x;
	Qpo->m_CC.pVector1.y = vec->y;
	Qpo->m_CC.pVector1.z = vec->z;

	Qpo->m_CC.pResult = Qpo->m_CC.pVector1 * transform->Matrix;

	result->x = Qpo->m_CC.pResult.x;
	result->y = Qpo->m_CC.pResult.y;
	result->z = Qpo->m_CC.pResult.z;
...
bool Q_PORTShell::Poly_AddOnce(QpoVertex* Verts, int NumVerts, unsigned int Bitmap, UInt8 TypeFLAGS, UInt8 RendFLAGS, float Scale)
{
	...
	float f_Length = Qpo->Vec3d_Length(&f_VecTo);
	...
	if(f_Length <= 1200.0f)
		...
		if(TypeFLAGS == QPOLY_TYPE_PNT)
			{
			g_SimpleRenderer->DrawPoint(Verts[0].x, Verts[0].y, Verts[0].z, Verts[0].r, Verts[0].g, Verts[0].b, Verts[0].a, Bitmap, Scale);
			}
		else if(TypeFLAGS == QPOLY_TYPE_TRI)
			{
			if(NumVerts == 3)
				{
				g_SimpleRenderer->DrawTri(Verts, Bitmap);
				}
			else if(NumVerts == 4)
				{
				g_SimpleRenderer->DrawQuad(Verts, Bitmap);
				}
			else
				{
				for(int f_Jet = 0; f_Jet < NumVerts; f_Jet++)
					{
					g_SimpleRenderer->DrawTri(&Verts[f_Jet * 3], Bitmap);
					}
				}
			}
		else if(TypeFLAGS == QPOLY_TYPE_GOR)
			{
			if(NumVerts == 3)
				{
				g_SimpleRenderer->DrawTriGor(Verts);
				}
			else if(NumVerts == 4)
				{
				g_SimpleRenderer->DrawQuadGor(Verts);
				}
			else
				{
				for(int f_Jet = 0; f_Jet < NumVerts; f_Jet++)
					{
					g_SimpleRenderer->DrawTriGor(&Verts[f_Jet * 3]);
					}
				}
			}
...
{
	Qpo->m_CC.setExecMode(CCX_MODE_DRAW);
}
...
void Q_PORTShell::Light_Setup(void)
{
	if(m_Light_Setup == false)
		{
		QpoVec3d f_Origin(0.0f, 0.0f, 0.0f);

		for(int f_LCnt = 0; f_LCnt < NOM_LIGHT; f_LCnt++)
			{
			g_Light[f_LCnt].m_ON = false;
			g_Light[f_LCnt].m_Pos = f_Origin;
			g_Light[f_LCnt].m_Radius = 0.0f;
			g_Light[f_LCnt].m_Color.alpha = 0.0f;
			g_Light[f_LCnt].m_Color.red = 0.0f;
			g_Light[f_LCnt].m_Color.green = 0.0f;
			g_Light[f_LCnt].m_Color.blue = 0.0f;
			}
...
void Q_PORTShell::Light_Remove(QpoLight light)
{
	...
	g_Light[(int)light].m_Radius = 0.0f;
}
...
QpoLight Q_PORTShell::Light_AddtoWorld(QpoVec3d pos)
{
	...
	while(f_Scan)
		...
		if(f_Return < 0)
			{
			if((g_Light[f_Count].m_ON == false) || (g_Light[f_Count].m_Radius <= 0.0f))
				{
			...
			if(f_Count >= Q_Nom_Light)
				...
				{
				if((g_Light[f_Count].m_ON == true) && (g_Light[f_Count].m_Radius > 0.0f))
					{
					f_QLightNom++;
					f_QLightProgNom = f_Count + 1;
					}
...
QpoSound Q_PORTShell::Sound_Play(QpoSoundDef SoundDef, float Volume, float Pan, float Pitch, bool Loop, QpoVec3d * Position)
	{
	std::string f_ref_CurrentPath = g_ref_global_start + "Q_PORTShell::Sound_Play";

	QpoVec3d f_Aerials = playerAV[0]->Xform.Translation;
	QpoVec3d f_VecTo;
	float f_LengthVecTo = 0;
	...
	float f_SetVolume = (1 - (f_LengthVecTo / 650));
	...
	if(f_SetVolume < 0.0f)
		...
		ag_StatusLive(f_ref_CurrentPath, "f_SoundEffect");
		std::shared_ptr<SoundEffect> f_SoundEffect = std::make_shared_reflect<SoundEffect>();

		f_SoundEffect->m_Position = *(Position);
		f_SoundEffect->m_Loop = Loop;

		f_SoundEffect->Initialize(g_Audio->MusicEngine(), g_StreamWaveForm[SoundDef], g_Stream[SoundDef], f_SoundEffect);
		...
		f_SoundEffect->m_AuPlus = _->m_vec_SoundEffect.size();

		_->m_vec_SoundEffect.push_back(f_SoundEffect);

		g_Audio_mutex.unlock();

		return f_SoundEffect;
		}
...
QpoSound Q_PORTShell::Sound_Play_Default(QpoSoundDef SoundDef, float Volume)
	{
	std::string f_ref_CurrentPath = g_ref_global_start + "Q_PORTShell::Sound_Play_Default";
...
void Q_PORTShell::Sound_Modify(QpoSound Sound, float Volume, float Pan, float Pitch, QpoVec3d * Position, QpoVec3d * Velocity)
	...
	QpoVec3d f_Aerials = playerAV[0]->Xform.Translation;
	QpoVec3d f_VecTo;
	float f_LengthVecTo = 0.0f;

	f_Aerials.y += (75 + (30*(((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Height/100)-0.5)*2)));

	Qpo->Vec3d_Subtract(&Sound->m_Position, &f_Aerials, &f_VecTo);
...
bool Q_PORTShell::Collision_Make_DMG_Sphere(QpoVec3d *Center, float f_Radius, float f_Power)
{
	Cube::BiVector f_Center(Center->x, Center->y, Center->z);

	f_Power *= WORLD_DMG_DEFAULT;
	...
	for(int i = 0; i < f_i; i++)
		...
		*(f_IntZ) = 0;
		...
		f_Collide = f_Grid->Make_ApplyDamageSphere(f_Center, f_Radius * 6.5f, (f_Power * 3.45f) + 10.0f, f_Grid->m_gridX, f_Grid->m_gridX, f_Grid->m_gridZ, f_IntX, f_IntY, f_IntZ);
...
void Q_PORTShell::Collision_Make_Impact_Grid_Select(Cube::BiVector f_Center, Cube::BiVector f_In, Cube::BiVector f_DistCenter, BlockMark f_BeginXY, BlockMark f_EndXY, float f_Power, float f_ExplosiveForce, float f_HeatPower)
{
	const float GRIDSIZE = (Cube::VoxelGrid::VOXEL_GRID_SIZE * (VoxelGrid::BLOCK_RENDER_SIZE * 2.0f));

	// Increase with World Damage Default
	f_Power *= WORLD_DMG_DEFAULT;
	f_ExplosiveForce *= WORLD_DMG_DEFAULT;
	f_HeatPower *= WORLD_DMG_DEFAULT;
	...
	for(int i = 0; i < f_i; i++)
		...
		if((f_Grid->m_rebuild == 0) || (f_Grid->m_setup == true))
			{
			if( (f_Center.m_X >= (f_Grid->m_gridX * GRIDSIZE)) &&
				(f_Center.m_X < ((f_Grid->m_gridX + 1) * GRIDSIZE)) &&
				(f_Center.m_Y >= (f_Grid->m_gridY * GRIDSIZE)) &&
				(f_Center.m_Y < ((f_Grid->m_gridY + 1) * GRIDSIZE)) &&
				(f_Center.m_Z >= ((f_Grid->m_gridZ * -1) * GRIDSIZE)) &&
				(f_Center.m_Z < (((f_Grid->m_gridZ * -1) + 1) * GRIDSIZE)))
				{
				int f_DiffX = abs(f_EndXY.m_VecX) + abs(f_BeginXY.m_VecX); //optimize here
				int f_DiffY = abs(f_EndXY.m_VecY) + abs(f_BeginXY.m_VecY);

				if((f_DiffX != 0) && (f_DiffY != 0))
					{
					int f_accum_Bluntness = (f_DiffX + f_DiffY) * (VoxelGrid::BLOCK_RENDER_SIZE * 2.0f);

					int f_Exprad = (f_DiffX * (VoxelGrid::BLOCK_RENDER_SIZE * 2.0f)) / (f_DiffY * (VoxelGrid::BLOCK_RENDER_SIZE * 2.0f));

					// DMG Mask
					Qpo->Collision_Make_DMG_Mask(f_Center, f_In, f_BeginXY, f_EndXY, 1.0f, 1.0f, 0.0f, 0.0f, (f_Power * f_HeatPower), f_accum_Bluntness); //swk

					float f_AyaFac = f_Power * f_ExplosiveForce * min(((f_HeatPower - WORLD_HEAT_ROOM_TEMP) / WORLD_HEAT_REDUCE), 0.9f);

					if(f_AyaFac > WORLD_EXPLOSIVE_FORCE_THRESH)
						{
						// Explosion Propagation
						Qpo->Collision_Make_DMG_Blast_Mask(f_DistCenter, f_AyaFac, WORLD_REDUCE_AMT, f_Exprad);
						}
					}
				}
			}
...
void Q_PORTShell::Collision_Make_Impact_Grid(QpoVec3d* Center, QpoVec3d* In, QpoVec3d* DistCenter, std::shared_ptr<VoxelGrid> f_Grid, BlockMark f_BeginXY, BlockMark f_EndXY, float f_Power, float f_ExplosiveForce, float f_HeatPower)
{
	Cube::BiVector f_DistCenter(DistCenter->x, DistCenter->y, DistCenter->z);
	Cube::BiVector f_Center(Center->x, Center->y, Center->z);
	Cube::BiVector f_In(In->x, In->y, In->z);

	Collision_Make_Impact_Grid(f_Center, f_In, f_DistCenter, f_Grid, f_BeginXY, f_EndXY, f_Power, f_ExplosiveForce, f_HeatPower);
}

// note : impact verified
void Q_PORTShell::Collision_Make_Impact_Grid(Cube::BiVector f_Center, Cube::BiVector f_In, Cube::BiVector f_DistCenter, std::shared_ptr<VoxelGrid> f_Grid, BlockMark f_BeginXY, BlockMark f_EndXY, float f_Power, float f_ExplosiveForce, float f_HeatPower)
{
	const float GRIDSIZE = (Cube::VoxelGrid::VOXEL_GRID_SIZE * (VoxelGrid::BLOCK_RENDER_SIZE * 2.0f));
...
// note : impact verified
void Q_PORTShell::Collision_Make_Impact(QpoVec3d* Center, QpoVec3d* In, QpoVec3d* DistCenter, float f_Power, float f_ExplosiveForce, float f_HeatPower)
{
	const float GRIDSIZE = (Cube::VoxelGrid::VOXEL_GRID_SIZE * (VoxelGrid::BLOCK_RENDER_SIZE * 2.0f));
	...
	for(int i = 0; i < f_i; i++)
		...
		if((f_Grid->m_rebuild == 0) || (f_Grid->m_setup == true))
			...
			if( (f_Center.m_X >= (f_Grid->m_gridX * GRIDSIZE)) &&
				...
				(f_Center.m_Z < (((f_Grid->m_gridZ * -1) + 1) * GRIDSIZE)))
				{
				BlockMark f_BeginXY = BlockMark(-10, -10);
				BlockMark f_EndXY = BlockMark(10, 10);
				...
				float f_AyaFac = f_Power * f_ExplosiveForce * f_HeatPower;
...
void Q_PORTShell::Collision_Make_DMG_Mask(VoxelBlock f_OriginBlock, BiVector f_ImpactPoint, BiVector f_ImpactIn, BlockMark f_Begin, BlockMark f_Finish, float f_StepX, float f_StepY, float f_ChangeX, float f_ChangeY, float f_FullEdgePower, float f_PropagationReductionPerc)
{
	std::shared_ptr<VoxelDamageMask> f_DmgMask = std::make_shared<VoxelDamageMask>(f_OriginBlock, f_ImpactIn, f_ImpactPoint);

	std::shared_ptr<MarkLayer> f_MarkLayer = std::make_shared<MarkLayer>(f_Begin, f_Finish, f_StepX, f_StepY, f_ChangeX, f_ChangeY, f_FullEdgePower, f_PropagationReductionPerc);

	f_MarkLayer->acSetSpeed(f_DmgMask->m_SingleUnit);

	f_DmgMask->m_vec_MarkLayer.push_back(f_MarkLayer);

	Q_VoxelDamage->m_vec_VoxelDamageMask.push_back(f_DmgMask);
}

void Q_PORTShell::Collision_Make_DMG_Mask(BiVector f_ImpactPoint, BiVector f_ImpactIn, BlockMark f_Begin, BlockMark f_Finish, float f_StepX, float f_StepY, float f_ChangeX, float f_ChangeY, float f_FullEdgePower, float f_PropagationReductionPerc)
{
	std::shared_ptr<VoxelDamageMask> f_DmgMask = std::make_shared<VoxelDamageMask>(f_ImpactIn, f_ImpactPoint);
...
void Q_PORTShell::Collision_Make_DMG_Blast_Mask(BiVector f_ImpactPoint, float f_FullEdgePower, float f_PropagationReductionPerc, int f_StartExplosionRadius)
{
	std::shared_ptr<VoxelDamageMask> f_DmgMask = std::make_shared<VoxelDamageMask>(f_ImpactPoint, f_StartExplosionRadius);

	std::shared_ptr<MarkLayer> f_Blast_MarkLayer = std::make_shared<MarkLayer>(f_FullEdgePower, f_PropagationReductionPerc);

	f_DmgMask->m_vec_MarkLayer.push_back(f_Blast_MarkLayer);
...
{
	const float GRIDSIZE = (Cube::VoxelGrid::VOXEL_GRID_SIZE * (Cube::VoxelGrid::BLOCK_RENDER_SIZE * 2.0f));
	...
	Cube::BiVector f_Vec3(Front->x, Front->y, Front->z);
	Cube::BiVector f_MotionVec = front - back;
	...
	// setup front back of ray
	Col->Front = front;
	Col->Rear = back;

	float fm_Length = f_MotionVec.acLength();
	...
	for(int i = 0; i < f_i; i++)
		...
		if((f_Vec3.m_X >= ((float)f_Grid->m_gridX * GRIDSIZE)) && 
			(f_Vec3.m_X < (((float)f_Grid->m_gridX + 1) * GRIDSIZE)) &&
			(f_Vec3.m_Y >= ((float)f_Grid->m_gridY * GRIDSIZE)) &&
			(f_Vec3.m_Y < (((float)f_Grid->m_gridY + 1) * GRIDSIZE)) &&
			(f_Vec3.m_Z >= ((float)(f_Grid->m_gridZ * -1) * GRIDSIZE)) &&
			(f_Vec3.m_Z < (((float)(f_Grid->m_gridZ * -1) + 1) * GRIDSIZE)))
			{
			if(i != f_GridIndex)
				{
				f_GridIndex = i;

				f_Collide = f_Grid->CollideMarchPoint(Col);

				if(fm_Length > 0.0f)
					{
					if(f_Collide == false)
						{
						f_Vec3 = f_MotionVec * GRIDSIZE;

						fm_Length -= GRIDSIZE;

						i = 0; //reset loop rotation
						}
					else
						{
						Col->m_VXGrid = f_Grid;
						Col->Plane.Normal.acSet(Col->Normal.m_X, Col->Normal.m_Y, Col->Normal.m_Z);
	...
	return false; //Default only fail
}
...
bool Q_PORTShell::Collision_RayTest_Projectile(QpoVec3d *Mins, QpoVec3d *Maxs, QpoVec3d *Front, QpoVec3d *Rear, int UserFlags, bool Collision, bool *Context, QpoCollision *Col)
{
	...
	Col->Impact.m_X = i_impact.x;
	Col->Impact.m_Y = i_impact.y;
	Col->Impact.m_Z = i_impact.z;
	//Col->Avatar = 0;
...
void Q_PORTShell::Avatar_GetBoneTransformMP(QpoAvatarFlags f_AV, const char * String, QpoTransform * Bone)
	...
	Qpo->m_CC.pVector1.x = bonetrans.x;
	Qpo->m_CC.pVector1.y = bonetrans.y;
	Qpo->m_CC.pVector1.z = bonetrans.z;
	Qpo->m_CC.pResult = Qpo->m_CC.pVector1 * f_AV.Xform.Matrix;

	Qpo->m_CC.pResult.x *= f_AV.Avatar.Scale.x;
	Qpo->m_CC.pResult.y *= f_AV.Avatar.Scale.y;
	Qpo->m_CC.pResult.z *= f_AV.Avatar.Scale.z;

	Bone->Translation.x = Qpo->m_CC.pResult.x + f_AV.Xform.Translation.x;
	Bone->Translation.y = Qpo->m_CC.pResult.y + f_AV.Xform.Translation.y;
	Bone->Translation.z = Qpo->m_CC.pResult.z + f_AV.Xform.Translation.z;
...
#pragma message("FIXME:::Rotate Matrix and transform by player matrix")
	...
	Qpo->m_CC.pVector1.y = bonetrans.y;
	Qpo->m_CC.pVector1.z = bonetrans.z;
	Qpo->m_CC.pResult = Qpo->m_CC.pVector1 * i_AV->Viewform;
	
	Qpo->m_CC.pResult.y *= i_AV->Avatar.Scale.y;

	Bone->Translation.x = Qpo->m_CC.pResult.x + i_AV->Xform.Translation.x;
	Bone->Translation.y = Qpo->m_CC.pResult.y + i_AV->Xform.Translation.y;
	Bone->Translation.z = Qpo->m_CC.pResult.z + i_AV->Xform.Translation.z;
	}
...
void Q_PORTShell::WeaponView_GetBoneTransform(QpoAvatarFlags * i_AV, const char * String, QpoTransform * Bone)
	...
	Qpo->m_CC.pVector1.y = bonetrans.y;
	Qpo->m_CC.pVector1.z = bonetrans.z;
	Qpo->m_CC.pResult = Qpo->m_CC.pVector1 * i_AV->Weaponform;
...
#if 0
	Qpo->m_CC.pResult = Qpo->m_CC.pVector1 * Q_Forge->Forge[i_AvatarIndex].AV.Weaponform;
#else
	Qpo->m_CC.pResult = Qpo->m_CC.pVector1 * Q_Forge->Forge[i_AvatarIndex].AV.Viewform;
#endif

	Bone->Translation.x = Qpo->m_CC.pResult.x + Q_Forge->Forge[i_AvatarIndex].AV.Xform.Translation.x;
	Bone->Translation.y = Qpo->m_CC.pResult.y + Q_Forge->Forge[i_AvatarIndex].AV.Xform.Translation.y;
	Bone->Translation.z = Qpo->m_CC.pResult.z + Q_Forge->Forge[i_AvatarIndex].AV.Xform.Translation.z;
	}
...
void Q_PORTShell::Player_World_intersect(void)
	{
	  ///////////////
	 // MakeDMG
	//
	if(playerAV[g_Player]->m_ImpactCount < 12)
		{
		QpoCollision f_Collision;

		bool f_SweepTestCollide = Collision_RayTest(NULL, NULL, &player[g_Player]->OAttPos.Translation, &player[g_Player]->AttPos.Translation, 0xffffffff, NULL, NULL, &f_Collision);

		if(f_SweepTestCollide == true)
			{
			BiVector f_inImpactCenter(player[g_Player]->AttPos.Translation.x, player[g_Player]->AttPos.Translation.y, player[g_Player]->AttPos.Translation.z);
			BiVector f_inOldCenter(player[g_Player]->OAttPos.Translation.x, player[g_Player]->OAttPos.Translation.y, player[g_Player]->OAttPos.Translation.z);
			BiVector f_ImpactIn = f_inImpactCenter - f_inOldCenter;
			float f_mull = min(f_ImpactIn.acLength(), 1.0f);
			f_ImpactIn.acNormalize();
			BiVector f_ImpactDistCenter = f_Collision.Impact - (f_ImpactIn * 30.0f);

			int f_X = (_->Box.xScale * 1.34f) / 2.0f;
			int f_Y = (_->Box.yScale * 1.34f) / 2.0f;

			Qpo->Collision_Make_Impact_Grid(f_Collision.Impact, f_ImpactIn, f_ImpactDistCenter, f_Collision.m_VXGrid, BlockMark(f_X * -1, f_Y * -1), BlockMark(f_X, f_Y), player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.ATTACKPOWER * f_mull, (player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.ATTACKPOWER + 10.5f) * f_mull);

			playerAV[g_Player]->m_ImpactCount += 3;
			}
		}
	else
		{
		playerAV[g_Player]->m_ImpactCount--;
		}
	}

void Q_PORTShell::Avatar_World_intersect(int f_AIflag)
	{
	  ///////////////
	 // MakeDMG
	//
	if(Q_Forge->Forge[f_AIflag].AV.m_ImpactCount < 12)
		{
		QpoCollision f_Collision;

		bool f_SweepTestCollide = Collision_RayTest(NULL, NULL, &Q_Forge->Forge[f_AIflag].OAttPos.Translation, &Q_Forge->Forge[f_AIflag].AttPos.Translation, 0xffffffff, NULL, NULL, &f_Collision);

		if(f_SweepTestCollide == true)
			{
			BiVector f_ImpactCenter(Q_Forge->Forge[f_AIflag].AttPos.Translation.x, Q_Forge->Forge[f_AIflag].AttPos.Translation.y, Q_Forge->Forge[f_AIflag].AttPos.Translation.z);
			BiVector f_OldCenter(Q_Forge->Forge[f_AIflag].OAttPos.Translation.x, Q_Forge->Forge[f_AIflag].OAttPos.Translation.y, Q_Forge->Forge[f_AIflag].OAttPos.Translation.z);
			BiVector f_ImpactIn = f_ImpactCenter - f_OldCenter;
			float f_mull = min(f_ImpactIn.acLength(), 1.0f);
			f_ImpactIn.acNormalize();
			BiVector f_ImpactDistCenter = f_ImpactCenter - (f_ImpactIn * 30.0f);

			int f_X = (_->Box.xScale * 6.0f) / 2.0f;
			int f_Y = (_->Box.yScale * 6.0f) / 2.0f;

			Qpo->Collision_Make_Impact_Grid(f_ImpactCenter, f_ImpactIn, f_ImpactDistCenter, f_Collision.m_VXGrid, BlockMark(f_X * -1, f_Y * -1), BlockMark(f_X, f_Y), player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.ATTACKPOWER * f_mull, (player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.ATTACKPOWER + 10.5f) * f_mull);

			Q_Forge->Forge[f_AIflag].AV.m_ImpactCount += 3;
			}
		}
	else
		{
		Q_Forge->Forge[f_AIflag].AV.m_ImpactCount--;
		}
	}
...
bool ClassCache::setExecMode(int i_ExecMode)
	{
