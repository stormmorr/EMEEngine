...
extern UN::unMessageCore *unMessageCore;
...
OsirusCore::OsirusCore()
{
	...
	acLoadGlobalAPI("Osirus\\DigiMassCore.exl", "DigiMassCore");
	acLoadGlobalAPI("Osirus\\GL.exl", "OpenGL");
	acLoadGlobalAPI("Osirus\\Tochnog.exl", "Tochnog");
...
void OsirusCore::acLoadGlobalAPI(const char* i_Filename, const char* i_APIName)
{
	FILE			*i_stream;
...
void OsirusCore::acSpeechEvent(CSpeechEvent *i_Event)
{
...
void OsirusCore::acAPISpeechEvent(CSpeechEvent *i_Event)
{
	OSI::Class *i_Class = 0;
	OSI::Function i_Function;
...
#endif
#if 0
			i_CSpeechDlg->acSpeak(unMessageCore->acGenName(0));
#endif
...
void OsirusCore::acGlobalAPISpeechEvent(CSpeechEvent *i_Event, bool i_SpkClsName)
{
	OSI::Class *i_Class = 0;
	OSI::Function i_Function;
	...
	while(i_Iter < i_Event->m_Word.size())
		...
		/*if(!i_Function)*/ i_Function = acFindFunctionGlobal(i_Event->m_Word[i_Iter]);

		if(m_FoundFunc)
			{
			printf("%s->", i_Function.m_Class->m_Name.c_str());
			fprintf(m_logFile, "%s->", i_Function.m_Class->m_Name.c_str());

			if(i_SpkClsName)
				{
				i_CSpeechDlg->acSpeak(i_Function.m_Class->m_Name.c_str());
				}
...
void OsirusCore::acGlobalAPISpeechEventOrder(CSpeechEvent *i_Event, bool i_SpkClsName)
{
	OSI::Class *i_Class = 0;
	OSI::Function i_Function;
	...
	for(unsigned int i_Count = 0; i_Count < vec_Class.size(); i_Count++)
		{
		i_Class = vec_Class[i_Count];

		if(i_Class)
			{
			printf("%s->", i_Class->m_Name.c_str());
			fprintf(m_logFile, "%s->", i_Class->m_Name.c_str());
...
Function OsirusCore::acFindFunctionGlobal(std::string *i_Word)
{
	Function i_resultFunction;
	m_FoundFunc = false;
	...
	for(unsigned int i_Count = 0; i_Count < vec_Class.size(); i_Count++)
		{
		for(unsigned int i_iter = 0; i_iter < vec_Class[i_Count]->vec_Function.size(); i_iter++)
			{
			for(unsigned int i_iterword = 0; i_iterword < i_Word->length(); i_iterword++)
				{
				for(unsigned int i_iterkey = 0; i_iterkey < vec_Class[i_Count]->vec_Function[i_iter].m_Name.m_Text.length(); i_iterkey++)
					{
					if(i_Word->at(i_iterword) == vec_Class[i_Count]->vec_Function[i_iter].m_Name.m_Text[i_iterkey])
						{
						i_wordindex.push_back(i_iterword);
						i_keyindex.push_back(i_iterkey);
						}
...
#pragma message("FIXME:::Test in the future");
#if 0
			i_sequencepoints /= i_Word->length();
			i_separationpoints /= i_Word->length();
#endif
			...
			if(i_sequencepoints > 0)	// Character in the same pattern
				...
				&& i_separationpoints <= m_separationpoints)
					{
					i_resultFunction = vec_Class[i_Count]->vec_Function[i_iter];
					m_FoundFunc = true;
