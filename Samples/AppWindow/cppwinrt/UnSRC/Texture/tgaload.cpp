...
void tgaUploadImage ( image_t *p, tgaFLAG mode )
{
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, internal_format, p->info.width, p->info.height, 0, p->info.tgaColourType, GL_UNSIGNED_BYTE, p->data);  
}
...
void tgaChecker ( image_t *p )
{
  ...
  glTexImage2D ( GL_TEXTURE_2D, 0, GL_RGBA4, p->info.width,
...
{
    tgaLoadTheImage ( file_name, p, mode );
...
unsigned int unTgaLoad( const char *file_name, tgaFLAG mode )
{
