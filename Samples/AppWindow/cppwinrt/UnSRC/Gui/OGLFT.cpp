...
namespace OGLFT {
  ...
  void Face::draw ( const char* s )
  {
	...
	character_display_list++;
      ...
      draw( c );
		...
		glGetIntegerv(GL_CURRENT_RASTER_POSITION, i_pos);
		
		//glTranslatef( i_pos[0] + 5, 0, 0 );
		//glColor4f( foreground_color_[R], foreground_color_[G], foreground_color_[B], foreground_color_[A] );

		glRasterPos3i( i_pos[0] + 5, 0, 0 );
		}
    }
  }
#ifndef OGLFT_NO_QT
  ...
  void Filled::init ( void )
  {
    ...
    gluTessCallback( tess_obj_, GLU_TESS_VERTEX, (void (__stdcall *)(void))vertexCallback );
    gluTessCallback( tess_obj_, GLU_TESS_BEGIN, (void (__stdcall *)(void))beginCallback );
    gluTessCallback( tess_obj_, GLU_TESS_END, (void (__stdcall *)(void))endCallback );
