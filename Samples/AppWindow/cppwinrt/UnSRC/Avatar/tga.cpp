...
char ac_sgetc(std::string f_String, int* f_Count)
{
	char f_Result = 'a';

	f_Result = f_String.at(*(f_Count));
	*(f_Count) += 1;

	return f_Result;
}

int CTga::ReadFile(std::string str)
{
	unsigned char l,r,g,b,a;
    unsigned char *tmp;
	...
	*(f_Count) = 0;
	...
	TgaHeader.idlength=ac_sgetc(str, f_Count);
	TgaHeader.colourmaptype=ac_sgetc(str, f_Count);
	TgaHeader.datatypecode=ac_sgetc(str, f_Count);
	TgaHeader.colourmaporigin=ac_sgetc(str, f_Count);
	TgaHeader.colourmaporigin+=ac_sgetc(str, f_Count)<<8;
	TgaHeader.colourmaplength=ac_sgetc(str, f_Count);
	TgaHeader.colourmaplength+=ac_sgetc(str, f_Count)<<8; 
	TgaHeader.colourmapdepth=ac_sgetc(str, f_Count);
	TgaHeader.x_origin=ac_sgetc(str, f_Count); 
	TgaHeader.x_origin+=ac_sgetc(str, f_Count)<<8; 
	TgaHeader.y_origin=ac_sgetc(str, f_Count);
	TgaHeader.y_origin+=ac_sgetc(str, f_Count)<<8;
	TgaHeader.SizeX=ac_sgetc(str, f_Count);
	TgaHeader.SizeX+=ac_sgetc(str, f_Count)<<8;
	TgaHeader.SizeY=ac_sgetc(str, f_Count);
	TgaHeader.SizeY+=ac_sgetc(str, f_Count)<<8;
	TgaHeader.Bpp=ac_sgetc(str, f_Count);
	TgaHeader.imagedescriptor=ac_sgetc(str, f_Count);
    ...
    if(TgaHeader.datatypecode==TGA_TYPE_COLOR || TgaHeader.datatypecode==TGA_TYPE_GRAY)
    {
        for(int y=0;y<TgaHeader.SizeY;y++)
            for(int x=0;x<TgaHeader.SizeX;x++)
            {
                if(TgaHeader.Bpp==8)
                {
                    r=ac_sgetc(str, f_Count);
                    m_dest[x+y*TgaHeader.SizeX]=r;
                }
                if(TgaHeader.Bpp==24 || TgaHeader.Bpp==32)
                {
                    b=ac_sgetc(str, f_Count);
                    g=ac_sgetc(str, f_Count);
                    r=ac_sgetc(str, f_Count);
					if(TgaHeader.Bpp==24)
                        a=255;
                    else
                        a=ac_sgetc(str, f_Count);
