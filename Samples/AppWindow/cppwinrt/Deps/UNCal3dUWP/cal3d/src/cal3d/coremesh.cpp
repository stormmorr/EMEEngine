...
int QunCoreMesh::addCoreSubmesh(QunCoreSubmesh *pCoreSubmesh)
{
  // get next bone id
  int submeshId;
...
/** Removes a core submesh.
  ...
  *
  *****************************************************************************/
void QunCoreMesh::removeCoreSubmesh( int submeshID )
{
...
QunCoreSubmesh *QunCoreMesh::getCoreSubmesh(int id)
{
...
int QunCoreMesh::getCoreSubmeshCount() const
{
...
std::vector<QunCoreSubmesh *>& QunCoreMesh::getVectorCoreSubmesh()
{
...
/** Adds a core submesh.
  ...
  * It adds appropriate QunCoreSubMorphTargets to each of the core sub meshes.
  *
...
int QunCoreMesh::addAsMorphTarget(QunCoreMesh *pCoreMesh)
{
  //Check if the numbers of vertices allow a blending
  std::vector<QunCoreSubmesh *>& otherVectorCoreSubmesh = pCoreMesh->getVectorCoreSubmesh();
  if (m_vectorCoreSubmesh.size() != otherVectorCoreSubmesh.size())
  {
    QunError::setLastError(QunError::INTERNAL, __FILE__, __LINE__);
    return -1;
  }
  ...
  std::vector<QunCoreSubmesh *>::iterator iteratorCoreSubmesh = m_vectorCoreSubmesh.begin();
  std::vector<QunCoreSubmesh *>::iterator otherIteratorCoreSubmesh = otherVectorCoreSubmesh.begin();
  ...
  while(iteratorCoreSubmesh != m_vectorCoreSubmesh.end())
  {
    ...
    QunCoreSubMorphTarget *pQunCoreSubMorphTarget = new(std::nothrow) QunCoreSubMorphTarget();
    if(!pQunCoreSubMorphTarget->reserve(vertexCount)) return -1;
    std::vector<QunCoreSubmesh::Vertex>& vectorVertex = (*otherIteratorCoreSubmesh)->getVectorVertex();
    std::vector<QunCoreSubmesh::Vertex>::iterator iteratorVectorVertex = vectorVertex.begin();
    for(int i = 0;i<vertexCount;++i)
    {
      ...
      if(!pQunCoreSubMorphTarget->setBlendVertex(i,blendVertex)) return -1;
      ++iteratorVectorVertex;
    }
    (*iteratorCoreSubmesh)->addCoreSubMorphTarget(pQunCoreSubMorphTarget);
    ++iteratorCoreSubmesh;
    ++otherIteratorCoreSubmesh;
  }
...
void QunCoreMesh::scale(float factor)
{
	std::vector<QunCoreSubmesh *>::iterator iteratorCoreSubmesh;
...
void QunCoreMesh::setFilename(const std::string& filename)
{
  m_filename = filename;
}
...
const std::string& QunCoreMesh::getFilename(void) const
{
  return m_filename;
}
...
void QunCoreMesh::setName(const std::string& name)
{
  m_name = name;
}
...
const std::string& QunCoreMesh::getName(void) const
{
  return m_name;
}
