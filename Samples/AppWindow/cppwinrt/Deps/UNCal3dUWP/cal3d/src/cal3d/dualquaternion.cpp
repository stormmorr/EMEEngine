...
QunDualQuaternion::QunDualQuaternion(	const QunQuaternion& inRotation,
	...
	: nondual( inRotation )
{
	...
	*/
	dual = inRotation;
	dual *= 0.5f;
	dual *= QunQuaternion( inTranslation.x, inTranslation.y, inTranslation.z, 0.0f );
}

void	QunDualQuaternion::normalize()
{
	/*
...
void	QunDualQuaternion::transformPoint( const QunVector& inPt, QunVector& outPt )
{
	QunQuaternion	ptAsQuat( inPt.x, inPt.y, inPt.z, 0.0f );
	QunQuaternion	nonDualConj( nondual );
	nonDualConj.conjugate();
	QunQuaternion	a( nonDualConj );
	a *= dual;
	a *= 2.0f;
	
	QunQuaternion	b( nonDualConj );
	b *= ptAsQuat;
	b *= nondual;
