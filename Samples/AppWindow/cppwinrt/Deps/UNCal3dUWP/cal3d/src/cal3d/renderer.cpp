...
QunRenderer::QunRenderer(QunModel* pModel)
...
QunRenderer::QunRenderer(QunRenderer* pRenderer)
{
...
bool QunRenderer::beginRendering()
{
...
void QunRenderer::getAmbientColor(unsigned char *pColorBuffer)
{
  ...
  QunCoreMaterial::Color& color = pCoreMaterial->getAmbientColor();
...
void QunRenderer::getDiffuseColor(unsigned char *pColorBuffer)
{
  // get the core material
  QunCoreMaterial *pCoreMaterial;
  pCoreMaterial = m_pModel->getCoreModel()->getCoreMaterial(m_pSelectedSubmesh->getCoreMaterialId());
  ...
  QunCoreMaterial::Color& color = pCoreMaterial->getDiffuseColor();
...
int QunRenderer::getFaceCount() const
{
...
/** Provides access to the face data.
  ...
  *****************************************************************************/
int QunRenderer::getFaces(QunIndex *pFaceBuffer) const
{
...
int QunRenderer::getMapCount() const
{
  // get the core material
  QunCoreMaterial *pCoreMaterial;
  pCoreMaterial = m_pModel->getCoreModel()->getCoreMaterial(m_pSelectedSubmesh->getCoreMaterialId());
...
Qun::UserData QunRenderer::getMapUserData(int mapId)
{
  // get the core material
  QunCoreMaterial *pCoreMaterial;
  pCoreMaterial = m_pModel->getCoreModel()->getCoreMaterial(m_pSelectedSubmesh->getCoreMaterialId());
...
int QunRenderer::getMeshCount() const
{
  // get the attached meshes vector
  std::vector<QunMesh *>& vectorMesh = m_pModel->getVectorMesh();
...
/** Provides access to the tangent space data.
  ...
  *****************************************************************************/
int QunRenderer::getTangentSpaces(int mapId, float *pTangentSpaceBuffer, int stride)
{
  // get the texture coordinate vector vector
  std::vector<std::vector<QunCoreSubmesh::TangentSpace> >& vectorvectorTangentSpace = m_pSelectedSubmesh->getCoreSubmesh()->getVectorVectorTangentSpace();
  ...
  if(m_pSelectedSubmesh->hasInternalData())
  {
    ...
    std::vector<QunSubmesh::TangentSpace>& vectorTangentSpace = m_pSelectedSubmesh->getVectorVectorTangentSpace()[mapId];
	...
	if(stride == sizeof(QunCoreSubmesh::TangentSpace) || stride <= 0)
	{
		memcpy(pTangentSpaceBuffer, &vectorTangentSpace[0], tangentSpaceCount * sizeof(QunCoreSubmesh::TangentSpace));	
	}
	else
	{
		...
		{
			memcpy(&pBuffer[0], &vectorTangentSpace[i], sizeof(QunCoreSubmesh::TangentSpace));
			pBuffer+=stride;
		}
	}
...
int QunRenderer::getNormals(float *pNormalBuffer, int stride)
{
  // check if the submesh handles vertex data internally
  if(m_pSelectedSubmesh->hasInternalData())
  {
    // get the normal vector of the submesh
    std::vector<QunVector>& vectorNormal = m_pSelectedSubmesh->getVectorNormal();
	...
	if(stride == sizeof(QunVector) || stride <= 0)
	{
		memcpy(pNormalBuffer, &vectorNormal[0], normalCount * sizeof(QunVector));
	}
	else
	{
		...
		{
			memcpy(&pBuffer[0], &vectorNormal[i], sizeof(QunVector));
			pBuffer+=stride;
		}
...
float QunRenderer::getShininess() const
{
  // get the core material
  QunCoreMaterial *pCoreMaterial;
  pCoreMaterial = m_pModel->getCoreModel()->getCoreMaterial(m_pSelectedSubmesh->getCoreMaterialId());
...
void QunRenderer::getSpecularColor(unsigned char *pColorBuffer)
{
  // get the core material
  QunCoreMaterial *pCoreMaterial;
  pCoreMaterial = m_pModel->getCoreModel()->getCoreMaterial(m_pSelectedSubmesh->getCoreMaterialId());
  ...
  QunCoreMaterial::Color& color = pCoreMaterial->getSpecularColor();
...
int QunRenderer::getSubmeshCount(int meshId) const
{
  // get the attached meshes vector
  std::vector<QunMesh *>& vectorMesh = m_pModel->getVectorMesh();
...
int QunRenderer::getTextureCoordinates(int mapId, float *pTextureCoordinateBuffer, int stride)
{
  // get the texture coordinate vector vector
  std::vector<std::vector<QunCoreSubmesh::TextureCoordinate> >& vectorvectorTextureCoordinate = m_pSelectedSubmesh->getCoreSubmesh()->getVectorVectorTextureCoordinate();
  ...
  if(stride == sizeof(QunCoreSubmesh::TextureCoordinate) || stride <= 0)
  {
	  memcpy(pTextureCoordinateBuffer, &vectorvectorTextureCoordinate[mapId][0], textureCoordinateCount * sizeof(QunCoreSubmesh::TextureCoordinate));
  }
  else
  {
	  ...
	  {
		  memcpy(&pBuffer[0], &vectorvectorTextureCoordinate[mapId][i], sizeof(QunCoreSubmesh::TextureCoordinate));
		  pBuffer+=stride;
	  }
...
int QunRenderer::getVertexCount() const
{
...
bool QunRenderer::isTangentsEnabled(int mapId) const
{
...
int QunRenderer::getVertices(float *pVertexBuffer, int stride)
{
  // check if the submesh handles vertex data internally
  if(m_pSelectedSubmesh->hasInternalData())
  {
    // get the vertex vector of the submesh
    std::vector<QunVector>& vectorVertex = m_pSelectedSubmesh->getVectorVertex();
	...
	if(stride == sizeof(QunVector) || stride <= 0)
	{
		memcpy(pVertexBuffer, &vectorVertex[0], vertexCount * sizeof(QunVector));
	}
	else
	{
		...
		{
			memcpy(&pBuffer[0], &vectorVertex[i], sizeof(QunVector));
			pBuffer+=stride;
		}
...
int QunRenderer::getVerticesAndNormals(float *pVertexBuffer, int stride)
{
  // check if the submesh handles vertex data internally
  if(m_pSelectedSubmesh->hasInternalData())
  {
	...
	{
		memcpy(&pBuffer[0], &vectorVertex[i], sizeof(QunVector));		
		memcpy(&pBuffer[sizeof(QunVector)], &vectorNormal[i], sizeof(QunVector));
		pBuffer+=stride;
	}
...
int QunRenderer::getVerticesNormalsAndTexCoords(float *pVertexBuffer,int NumTexCoords)
{  
  ...
  if(m_pSelectedSubmesh->hasInternalData())
  {
	...
	if(TextureCoordinateCount==0)
	{
		...
		{
			memcpy(&pVertexBuffer[0], &vectorVertex[vertexId], sizeof(QunVector));
			memcpy(&pVertexBuffer[3], &vectorNormal[vertexId], sizeof(QunVector));
	...
	else
	{
		if(NumTexCoords==1)
		{
			for(int vertexId=0; vertexId < vertexCount; ++vertexId)
			{
				memcpy(&pVertexBuffer[0], &vectorVertex[vertexId], sizeof(QunVector));
				memcpy(&pVertexBuffer[3], &vectorNormal[vertexId], sizeof(QunVector));
				memcpy(&pVertexBuffer[6], &vectorvectorTextureCoordinate[0][vertexId], sizeof(QunCoreSubmesh::TextureCoordinate));
				pVertexBuffer+=8;
			}
		}
	    else
		{
			for(int vertexId=0; vertexId < vertexCount; ++vertexId)
			{
				...
				{
					memcpy(&pVertexBuffer[0], &vectorvectorTextureCoordinate[mapId][vertexId], sizeof(QunCoreSubmesh::TextureCoordinate));
					pVertexBuffer+=2;
				}
			}
		}		
...
bool QunRenderer::selectMeshSubmesh(int meshId, int submeshId)
{
  // get the attached meshes vector
  std::vector<QunMesh *>& vectorMesh = m_pModel->getVectorMesh();
...
void QunRenderer::setNormalization(bool normalize)
{ 
