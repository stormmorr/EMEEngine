...
QunSpringSystem::QunSpringSystem(QunModel* pModel)
{
  assert(pModel);
  m_pModel = pModel;
  
  m_vGravity = QunVector(0.0f, 0.0f, -98.1f);
  // We add this force to simulate some movement
  m_vForce = QunVector(0.0f, 0.5f, 0.0f);
  m_collision=false;
}


 /*****************************************************************************/
/** Qunculates the forces on each unbound vertex.
  *
...
void QunSpringSystem::calculateForces(QunSubmesh *pSubmesh, float deltaTime)
{
#pragma unused( deltaTime )
  // get the vertex vector of the submesh
  std::vector<QunVector>& vectorVertex = pSubmesh->getVectorVertex();

  // get the vertex vector of the submesh
  std::vector<QunSubmesh::PhysicalProperty>& vectorPhysicalProperty = pSubmesh->getVectorPhysicalProperty();

  // get the physical property vector of the core submesh
  std::vector<QunCoreSubmesh::PhysicalProperty>& vectorCorePhysicalProperty = pSubmesh->getCoreSubmesh()->getVectorPhysicalProperty();
  ...
  for(vertexId = 0; vertexId < (int)vectorVertex.size(); ++vertexId)
  {
    ...
    QunSubmesh::PhysicalProperty& physicalProperty = vectorPhysicalProperty[vertexId];

    // get the physical property of the core vertex
    QunCoreSubmesh::PhysicalProperty& corePhysicalProperty = vectorCorePhysicalProperty[vertexId];
 ...
 /*****************************************************************************/
/** Qunculates the vertices influenced by the spring system instance.
  *
...
void QunSpringSystem::calculateVertices(QunSubmesh *pSubmesh, float deltaTime)
{
  // get the vertex vector of the submesh
  std::vector<QunVector>& vectorVertex = pSubmesh->getVectorVertex();
  ...
  {
    // get the vertex
    QunVector& vertex = vectorVertex[vertexId];
    ...
    if(corePhysicalProperty.weight > 0.0f)
    {
      ...
      physicalProperty.position += (position - physicalProperty.positionOld) * 0.99f + physicalProperty.force / corePhysicalProperty.weight * deltaTime * deltaTime;
		...
		QunSkeleton *pSkeleton = m_pModel->getSkeleton();
		
		if(m_collision)
		{
			std::vector<QunBone *> &m_vectorbone =  pSkeleton->getVectorBone();
			
			for(size_t boneId=0; boneId < m_vectorbone.size(); boneId++)
			{
				QunBoundingBox p = m_vectorbone[boneId]->getBoundingBox();
				bool in=true;
				float min=1e10;
				int index=-1;
				...
				if(in && index!=-1)
				{
					QunVector normal = QunVector(p.plane[index].a,p.plane[index].b,normal.z = p.plane[index].c);
					normal.normalize();
  ...
  std::vector<QunCoreSubmesh::Spring>& vectorSpring = pSubmesh->getCoreSubmesh()->getVectorSpring();
...
#define ITERATION_COUNT 2
  for(iterationCount = 0; iterationCount < ITERATION_COUNT; ++iterationCount)
  {
    ...
    std::vector<QunCoreSubmesh::Spring>::iterator iteratorSpring;
    for(iteratorSpring = vectorSpring.begin(); iteratorSpring != vectorSpring.end(); ++iteratorSpring)
    {
      // get the spring
      QunCoreSubmesh::Spring& spring = *iteratorSpring;
...
/* DEBUG-CODE ********************
  QunVector spherePosition(Sphere.x, Sphere.y, Sphere.z);
...
void QunSpringSystem::resetPositions()
{
	QunPhysique*	thePhysique = m_pModel->getPhysique();
	...
	for(iteratorMesh = vectorMesh.begin(); iteratorMesh != vectorMesh.end(); ++iteratorMesh)
	{
		...
		{
			QunSubmesh*	theSubmesh = *iteratorSubmesh;
			...
			if (theSubmesh->getCoreSubmesh()->getSpringCount() > 0 && theSubmesh->hasInternalData())
			{
				...
				std::vector<QunVector>&		vectorVertex( theSubmesh->getVectorVertex() );
				vectorVertex.resize( kNumVert );
				thePhysique->calculateVertices( theSubmesh, &vectorVertex[0].x );
				
				std::vector<QunSubmesh::PhysicalProperty>&	vectorPhysProp(
...
void QunSpringSystem::update(float deltaTime)
{
  // get the attached meshes vector
  std::vector<QunMesh *>& vectorMesh = m_pModel->getVectorMesh();
...
QunVector & QunSpringSystem::getGravityVector()
{
	return m_vGravity;
}
...
QunVector & QunSpringSystem::getForceVector()
{
	return m_vForce;
}
...
void QunSpringSystem::setGravityVector(const QunVector & vGravity)
{
	m_vGravity = vGravity;
}
...
void QunSpringSystem::setForceVector(const QunVector & vForce)
{
	m_vForce = vForce;
}
...
void QunSpringSystem::setCollisionDetection(bool collision)
{
