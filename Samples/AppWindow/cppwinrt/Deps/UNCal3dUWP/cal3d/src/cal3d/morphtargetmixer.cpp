...
QunMorphTargetMixer::QunMorphTargetMixer(QunModel* pModel)
{
  assert(pModel);
  m_pModel = pModel;
...
/** Interpolates the weight of a morph target.
  ...
  *****************************************************************************/
bool QunMorphTargetMixer::blend(int id, float weight, float delay)
{
...
bool QunMorphTargetMixer::clear(int id, float delay)
{
  if((id < 0) || (id >= (int)m_vectorCurrentWeight.size()))
  {
    QunError::setLastError(QunError::INVALID_HANDLE, __FILE__, __LINE__);
...
/** Get the weight of a morph target.
  ...
  *****************************************************************************/
float QunMorphTargetMixer::getCurrentWeight(int id) const
{
  if((id < 0) || (id >= (int)m_vectorCurrentWeight.size()))
  {
    QunError::setLastError(QunError::INVALID_HANDLE, __FILE__, __LINE__);
...
/** Get the weight of the base vertices.
  ...
  *****************************************************************************/
float QunMorphTargetMixer::getCurrentWeightBase()
{
...
/** Copy data from one mixer (for the same core model) to another.
  ...
  *****************************************************************************/
bool QunMorphTargetMixer::copy( const QunMorphTargetMixer& inOther )
{
...
void QunMorphTargetMixer::update(float deltaTime)
{
  std::vector<float>::iterator iteratorCurrentWeight = m_vectorCurrentWeight.begin();
  std::vector<float>::iterator iteratorEndWeight = m_vectorEndWeight.begin();
  std::vector<float>::iterator iteratorDuration = m_vectorDuration.begin();
  ...
  {
    QunCoreMorphAnimation* pCoreMorphAnimation = 
    ...
    while(meshIterator<vectorCoreMeshID.size())
    {
    	...
    	QunMesh*	theMesh = m_pModel->getMesh( coreMeshID );
    	if (theMesh)
    	{
			std::vector<QunSubmesh *> &vectorSubmesh = 
...
int QunMorphTargetMixer::getMorphTargetCount() const
{
