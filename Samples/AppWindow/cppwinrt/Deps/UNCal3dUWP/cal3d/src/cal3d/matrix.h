...
public:
	inline QunMatrix()	: dxdx(0.0), dydx(0.0), dzdx(0.0),
		dxdy(0.0), dydy(0.0), dzdy(0.0),
		dxdz(0.0), dydz(0.0), dzdz(0.0)
	{
	}
	
	QunMatrix(const QunQuaternion &q);
	
	inline QunMatrix(float weight, const QunMatrix &m)
	{
		dxdx = m.dxdx*weight;
		dxdy = m.dxdy*weight;
		dxdz = m.dxdz*weight;
...
public:
	...
	inline void blend(float factor, const QunMatrix& m)
	{
		dxdx += m.dxdx*factor;
		dydx += m.dydx*factor;
		dzdx += m.dzdx*factor;
