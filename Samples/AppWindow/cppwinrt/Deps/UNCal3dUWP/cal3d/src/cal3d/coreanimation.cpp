...
QunCoreAnimation::QunCoreAnimation()
{
}
...
QunCoreAnimation::~QunCoreAnimation()
{
    ...
    {
        QunCoreTrack* pTrack = *iteratorCoreTrack;
        pTrack->destroy();
        delete pTrack;
    }
}
...
bool QunCoreAnimation::addCoreTrack(QunCoreTrack *pCoreTrack)
{
...
QunCoreTrack *QunCoreAnimation::getCoreTrack(int coreBoneId)
{
...
unsigned int QunCoreAnimation::getTrackCount() const
{
...
float QunCoreAnimation::getDuration() const
{
  return m_duration;
}
...
void QunCoreAnimation::setDuration(float duration)
{
  m_duration = duration;
}
...
void QunCoreAnimation::scale(float factor)
{
  // loop through all core track
  std::list<QunCoreTrack *>::iterator iteratorCoreTrack;
  for(iteratorCoreTrack = m_listCoreTrack.begin(); iteratorCoreTrack != m_listCoreTrack.end(); ++iteratorCoreTrack)
...
void QunCoreAnimation::setFilename(const std::string& filename)
{
  m_filename = filename;
}
...
const std::string& QunCoreAnimation::getFilename(void) const
{
  return m_filename;
}
...
void QunCoreAnimation::setName(const std::string& name)
{
  m_name = name;
}
...
const std::string& QunCoreAnimation::getName(void) const
{
  return m_name;
}
...
void QunCoreAnimation::registerQunlback(QunAnimationQunlback *callback,float min_interval)
{
  QunlbackRecord record;
  record.callback     = callback;
  record.min_interval = min_interval;

  m_listQunlbacks.push_back(record);
}

/*****************************************************************************/
/** 
  * Remove a callback from the current list of callbacks for this Anim.
  * Qunlback objects not removed this way will be deleted in the dtor of the Anim.
  *
  * @param  callback     Ptr to a subclass of this abstract class implementing the callback function to remove.
  *
  *****************************************************************************/

void QunCoreAnimation::removeQunlback(QunAnimationQunlback *callback)
{
  for (std::vector<QunlbackRecord>::iterator i = m_listQunlbacks.begin(); i != m_listQunlbacks.end(); i++)
  {
    if ((*i).callback == callback)
    {
      m_listQunlbacks.erase(i);
      return;
    }
  }
}
...
std::list<QunCoreTrack *>& QunCoreAnimation::getListCoreTrack()
{
...
unsigned int QunCoreAnimation::getTotalNumberOfKeyframes() const
{
	unsigned int nbKeys = 0;
	for (std::list<QunCoreTrack*>::const_iterator it = m_listCoreTrack.begin(); it != m_listCoreTrack.end(); ++it)
	{
		QunCoreTrack *track = *it;
