...
QunCoreSubmesh::~QunCoreSubmesh()
{
  ...
  std::vector<QunCoreSubMorphTarget *>::iterator iteratorCoreSubMorphTarget;
...
int QunCoreSubmesh::getCoreMaterialThreadId() const
{
...
int QunCoreSubmesh::getFaceCount() const
{
...
int QunCoreSubmesh::getLodCount() const
{
  return m_lodCount;
}
...
int QunCoreSubmesh::getSpringCount() const
{
...
bool QunCoreSubmesh::isTangentsEnabled(int mapId) const
{
...
void QunCoreSubmesh::UpdateTangentVector(int v0, int v1, int v2, int mapId)
{
  std::vector<QunCoreSubmesh::Vertex> &vvtx = getVectorVertex();
  std::vector<QunCoreSubmesh::TextureCoordinate> &vtex = m_vectorvectorTextureCoordinate[mapId];
  ...
  QunVector vec1 = vvtx[v1].position - vvtx[v0].position;
  QunVector vec2 = vvtx[v2].position - vvtx[v0].position;
  QunVector tangent = (vec1 * ((float)x)) + (vec2 * ((float)y));
...
bool QunCoreSubmesh::enableTangents(int mapId, bool enabled)
{
  if((mapId < 0) || (mapId >= (int)m_vectorTangentsEnabled.size())) return false;
  ...
  {
    m_vectorvectorTangentSpace[mapId][tangentId].tangent= QunVector(0.0f,0.0f,0.0f);
...
std::vector<QunCoreSubmesh::Face>& QunCoreSubmesh::getVectorFace()
{
  return m_vectorFace;
}
...
std::vector<QunCoreSubmesh::PhysicalProperty>& QunCoreSubmesh::getVectorPhysicalProperty()
{
...
std::vector<QunCoreSubmesh::Spring>& QunCoreSubmesh::getVectorSpring()
{
...
std::vector<std::vector<QunCoreSubmesh::TextureCoordinate> > & QunCoreSubmesh::getVectorVectorTextureCoordinate()
{
...
std::vector<std::vector<QunCoreSubmesh::TangentSpace> >& QunCoreSubmesh::getVectorVectorTangentSpace()
{
...
std::vector<QunCoreSubmesh::Vertex>& QunCoreSubmesh::getVectorVertex()
{
...
int QunCoreSubmesh::getVertexCount() const
{
...
bool QunCoreSubmesh::reserve(int vertexCount, int textureCoordinateCount, int faceCount, int springCount)
{
	bool	success = true;
	try
	{
...
void QunCoreSubmesh::setCoreMaterialThreadId(int coreMaterialThreadId)
{
...
bool QunCoreSubmesh::setFace(int faceId, const Face& face)
{
...
void QunCoreSubmesh::setLodCount(int lodCount)
{
  m_lodCount = lodCount;
}
...
bool QunCoreSubmesh::setTangentSpace(int vertexId, int textureCoordinateId, const QunVector& tangent, float crossFactor)
{
...
bool QunCoreSubmesh::setPhysicalProperty(int vertexId, const PhysicalProperty& physicalProperty)
{
...
bool QunCoreSubmesh::setSpring(int springId, const Spring& spring)
{
...
bool QunCoreSubmesh::setTextureCoordinate(int vertexId, int textureCoordinateId, const TextureCoordinate& textureCoordinate)
{
  if((textureCoordinateId < 0) || (textureCoordinateId >= (int)m_vectorvectorTextureCoordinate.size())) return false;
...
bool QunCoreSubmesh::setVertex(int vertexId, const Vertex& vertex)
{
  if((vertexId < 0) || (vertexId >= (int)m_vectorVertex.size())) return false;
...
int QunCoreSubmesh::addCoreSubMorphTarget(QunCoreSubMorphTarget *pCoreSubMorphTarget)
{
...
QunCoreSubMorphTarget *QunCoreSubmesh::getCoreSubMorphTarget(int id)
{
...
int QunCoreSubmesh::getCoreSubMorphTargetCount() const
{
...
std::vector<QunCoreSubMorphTarget *>& QunCoreSubmesh::getVectorCoreSubMorphTarget()
{
...
void QunCoreSubmesh::scale(float factor)
{
...
/*		
		...
		for(springId = 0; springId < m_vectorVertex.size() ; springId++)
		{
			...
			QunVector distance = m_vectorVertex[m_vectorSpring[springId].vertexId[1]].position - m_vectorVertex[m_vectorSpring[springId].vertexId[0]].position;
