/*
	...
	*/#include "pch.h"/*- Minor Component -Player Inputs-
...
void Q_COREFunctions::Input(void)
	{
	//**********************************************************************************************************
	//**********************************************************************************************************
...
#pragma message("Server engine is oblivious to rendering variables")
	...
	if(player[g_Player]->PlayerMode == 0)
		...
		if(_->mbutton[g_Player] == 1 && player[g_Player]->KL_RTFSpecLevel > 5 && player[g_Player]->EnergyArchitect.BuildMODEON == 0 && player[g_Player]->Mposestate != 16)	//Build mode activation
			{
			helly = 0;
		...
		if(_->lbutton[g_Player] == 1 && player[g_Player]->KL_LTFSpecLevel == 1 && player[g_Player]->WeaponStatus == 0 && player[g_Player]->EnergyArchitect.BuildMODEON == 1)	//****////BUILD\\\\****
			{
		...
		//#####	Energy Pylon ####//
		if(_->mbutton[g_Player] == 1 && player[g_Player]->KL_LTFSpecLevel == 1 && player[g_Player]->WeaponStatus == 0 && player[g_Player]->EnergyArchitect.BuildMODEON == 1)	//****////BUILD\\\\****
			{
			player[g_Player]->KL_LTFSpecLevel = 0;
			player[g_Player]->EnergyArchitect.BuildMODEON = 0;
		...
		if(_->lbutton[g_Player] == 1 && player[g_Player]->WeaponStatus == 0 && (player[g_Player]->Aposestate != 26 && player[g_Player]->Aposestate != 28 && player[g_Player]->Aposestate != 30 && player[g_Player]->Mposestate != 16) && player[g_Player]->EnergyArchitect.BuildMODEON == 0)	//****////PUNCHS\\\\****
			{
			if(player[g_Player]->astackcnt <= 6)
				...
				if(!KEY_PRESS_DOWN(QIN_z))
					...
					if(KEY_DOWN_s())	//BACKWARD ATTACKS
						{
						if(player[g_Player]->astackcntdir[0] > 6) player[g_Player]->astackcntdir[0] = 0;
		...
		if(_->rbutton[g_Player] == 1 && (player[g_Player]->Aposestate != 26 && player[g_Player]->Aposestate != 28 && player[g_Player]->Aposestate != 30 && player[g_Player]->Mposestate != 16))	//****////KICKS\\\\****
			{
			jet = 0;
			helly = 0;
			while(helly < 6)
			...
			if(jet == 1)
				{
				if(!KEY_PRESS_DOWN(QIN_z))	//Not Low
					{
					player[g_Player]->attackstack[helly] = 40;							//STILL ATTACKS
					if(KEY_DOWN_w()) player[g_Player]->attackstack[helly] = 79;		//FORWARD ATTACKS
					if(KEY_DOWN_s()) player[g_Player]->attackstack[helly] = 29;		//BACKWARD
					if(KEY_DOWN_a()) player[g_Player]->attackstack[helly] = 81;		//LEFT ATTACKS
					if(KEY_DOWN_d()) player[g_Player]->attackstack[helly] = 80;		//RIGHT ATTACKS
					}
				else
					{
			...
			if(player[g_Player]->astackcnt < 6)
				...
				if(player[g_Player]->WeaponStatus == 0)
					{
					if(!KEY_PRESS_DOWN(QIN_z))		//Check for low attack
						{
						noot=0;
		...
		if(_->mbutton[g_Player] == 1 && ((player[g_Player]->KL_RTFSpecLevel == 3 && player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].Class == 0 && player[g_Player]->WeaponStatus == 0) || (player[g_Player]->KL_RTFSpecLevel == 5 && player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].Class == 1 && player[g_Player]->WeaponStatus > 0)))	//Fighter Power Up Activation
			{
			if(player[g_Player]->Mposestate == 14)
				{
		...
		if(_->mbutton[g_Player] == 1 && (player[g_Player]->KL_RTFSpecLevel == 4 || player[g_Player]->KL_FTRSpecLevel == 4) && player[g_Player]->astackcnt < 6)	//ZONE AFFECT SWITCH
			{
			player[g_Player]->astackcnt = 0;
			player[g_Player]->astackrun = 0;
			player[g_Player]->astackcntdir[0] = 0;
		...
		if((player[g_Player]->WeaponStatus > 0) && ((_->lbutton[g_Player] == 1 || _->mbutton[g_Player] == 1) && (player[g_Player]->Aposestate != 26 && player[g_Player]->Aposestate != 28 && player[g_Player]->Aposestate != 30)))
			{
			//### Damage Upgrades
		...
		/*if(KEY_PRESS_DOWN(QIN_KP1)) _->WorldScale -= 0.1;
		if(KEY_PRESS_DOWN(QIN_KP2)) _->WorldScale += 0.1;
		if(KEY_PRESS_DOWN(QIN_KP3)) Q.RadialDamage(Q_Explosion->Explosion[0].Team, Q_Explosion->Explosion[0].AvID, Q_Explosion->Explosion[0].ExpSize/2.5, Q_Explosion->Explosion[0].ExpDMG, Q_Explosion->Explosion[0].Pos, 0, TempVec1);*/
		...
		//playerAV[g_Player]->NewPos.y += 3.5;
		//playerAV[g_Player]->OldPos.y += 60;

#if 0
		//if(Qpo->Collision_RayTest(&playerAV[g_Player]->Mins, &playerAV[g_Player]->Maxs, &playerAV[g_Player]->OldPos, &playerAV[g_Player]->NewPos, 0xffffffff, NULL, NULL, &playerAV[g_Player]->Collision))
		if(Qpo->Collision_RayTest(NULL, NULL, &playerAV[g_Player]->OldPos, &playerAV[g_Player]->NewPos, /*GE_CONTENTS_SOLID_CLIP, GE_COLLIDE_MODELS,*/ 0xffffffff, NULL, NULL, &playerAV[g_Player]->Collision))
			{
			printf("Qpo->Collision_RayTest(&playerAV[g_Player]->Mins TRUE\n");
			playerAV[g_Player]->Xform.Translation = playerAV[g_Player]->Collision.Impact;
			player[g_Player]->AirBorn = 0;
			player[g_Player]->JumpState = 0;

			//player[g_Player]->DownSpeed += (0.5 / player[g_Player]->GravityAlter) * _->TimeScale; //* (0.5 + (0.8 * (player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Weight / 500)));
		...
		playerAV[g_Player]->OldPos = playerAV[g_Player]->Xform.Translation;
		playerAV[g_Player]->OldPos.y += 75;
		playerAV[g_Player]->NewPos = playerAV[g_Player]->Xform.Translation;
		Qpo->Transform_GetIn(&playerAV[g_Player]->Xform,&playerAV[g_Player]->Up);
		Qpo->Vec3d_Inverse(&playerAV[g_Player]->Up);
		Qpo->Vec3d_AddScaled(&playerAV[g_Player]->NewPos, player[g_Player]->PlayerSpeed*_->TimeScale, &playerAV[g_Player]->Up, &playerAV[g_Player]->NewPos);*/

		//playerAV[g_Player]->OldPos.y += 2;
		//playerAV[g_Player]->NewPos.y += 2;

#if 1
	//Qpo->Camera_Set();
	//glColor3f(1.0f, 1.0f, 0.25f);
	////*glBegin(GL_LINES);
	//glVertex3f(playerAV[g_Player]->OldPos.x, playerAV[g_Player]->OldPos.y, playerAV[g_Player]->OldPos.z);
	//glVertex3f(playerAV[g_Player]->NewPos.x, playerAV[g_Player]->NewPos.y, playerAV[g_Player]->NewPos.z);
	////*glEnd();
#endif

#if 0
		if(Qpo->Collision_RayTest(NULL, NULL, &playerAV[g_Player]->OldPos, &playerAV[g_Player]->NewPos,/* GE_CONTENTS_CANNOT_OCCUPY //GE_COLLIDE_MODELS, */ 0xffffffff, NULL, NULL, &playerAV[g_Player]->Collision) || playerAV[g_Player]->NewPos.y < 0)
			{
			player[g_Player]->collide = 1;
			}
		else
		...
		if(player[g_Player]->collide == 1)// playerAV[g_Player]->Xform.Translation = playerAV[g_Player]->NewPos;
		//else
			{
			//playerAV[g_Player]->Xform.Translation = playerAV[g_Player]->NewPos;
			/*if(playerAV[g_Player]->CollisionCount < 500)
				{
				playerAV[g_Player]->Xform.Translation.y = 0;
				}
			else
				{*/
				if(playerAV[g_Player]->Collision.Impact.m_Y >= 0)
					{
					playerAV[g_Player]->Xform.Translation = playerAV[g_Player]->Collision.Impact;
					//playerAV[g_Player]->Xform.Translation.y += 3;
					}
				else
					{
					playerAV[g_Player]->Xform.Translation.y = 0;
			...
			player[g_Player]->AirBorn = 0;
			player[g_Player]->JumpState = 0;
			//player[g_Player]->DownSpeed = -10;
			}

		//Increment Collision Start Timer
		playerAV[g_Player]->CollisionCount++;
#endif
#endif

	#if 0
		if(playerAV[g_Player]->NewPos.y < 0/*grnd*/)
			{
			playerAV[g_Player]->Xform.Translation.y = 0/*grnd*/;
			player[g_Player]->AirBorn = 0;
			player[g_Player]->JumpState = 0;
			//player[g_Player]->DownSpeed += (0.5 / player[g_Player]->GravityAlter) * _->TimeScale; //* (0.5 + (0.8 * (player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Weight / 500)));
			}
		...
		//""""""""""""""""
		//playerAV[g_Player]->OldPos = player[g_Player]->finalcolldet_oldpos;
		playerAV[g_Player]->NewPos = playerAV[g_Player]->Xform.Translation;
		//""""""""""""""""
		...
		//Wind Resistance
		if(player[g_Player]->MoveReactSpeed - 0.05 *_->TimeScale > 0) player[g_Player]->MoveReactSpeed -= 0.05 * _->TimeScale;	//0.4
		else player[g_Player]->MoveReactSpeed = 0;
		//""""""""""""""""

		Qpo->Vec3d_AddScaled(&playerAV[g_Player]->NewPos, player[g_Player]->MoveReactSpeed * _->TimeScale*HITREACT_SCALE_IMP, &player[g_Player]->MoveReactVec, &playerAV[g_Player]->NewPos);

		if(player[g_Player]->HitShield == 1)
			{
			player[g_Player]->MoveReactSpeed = 0;
			player[g_Player]->HitShield = 2;
			}
		...
		QpoVec3d f_OldPo = playerAV[g_Player]->Xform.Translation;
		QpoVec3d f_NewPo = playerAV[g_Player]->Xform.Translation;

		f_OldPo.y += 41.5;
		
		// Height Test
		if(Qpo->Collision_RayTest(&playerAV[g_Player]->Mins, &playerAV[g_Player]->Maxs, &f_OldPo, &f_NewPo, 0xffffffff, NULL, NULL, &playerAV[g_Player]->Collision)/* && player[g_Player]->DownSpeed <= 0*/)
			{
			playerAV[g_Player]->NewPos.y = playerAV[g_Player]->Collision.Impact.m_Y;
			//playerAV[g_Player]->NewPos.y = playerAV[g_Player]->OldPos.y;
			player[g_Player]->AirBorn = 0;
			player[g_Player]->JumpState = 0;
			player[g_Player]->collide = 1;
			}
		...
		// Height test
		if(playerAV[g_Player]->NewPos.y < 0)
			{
			playerAV[g_Player]->NewPos.y = 0;
			player[g_Player]->AirBorn = 0;
			player[g_Player]->JumpState = 0;
			player[g_Player]->collide = 1;
			}
		...
		//X / Z Test
#if 0
		QpoVec3d f_OldPos = playerAV[g_Player]->OldPos;
		QpoVec3d f_NewPos = playerAV[g_Player]->NewPos;

		f_OldPos.y += 55;
		f_NewPos.y += 55;

		if(Qpo->Collision_RayTest(&playerAV[g_Player]->Mins, &playerAV[g_Player]->Maxs, &f_OldPos, &f_NewPos, 0xffffffff, NULL, NULL, &playerAV[g_Player]->Collision))
			{
			playerAV[g_Player]->NewPos.x = playerAV[g_Player]->Collision.Impact.m_X;
			playerAV[g_Player]->NewPos.z = playerAV[g_Player]->Collision.Impact.m_Z;
			}
#endif
							//1.5
							//0.5
							//0.7
		player[g_Player]->DownSpeed -= (0.7 / player[g_Player]->GravityAlter) * _->TimeScale; //* (0.5 + (0.8 * (player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Weight / 500)));

		if(player[g_Player]->DownSpeed <= 0 && player[g_Player]->collide == 1)	//## Check to determine if Avatar touching the Floor
			{
			if((player[g_Player]->DownSpeed < (((0.5 / player[g_Player]->GravityAlter) * _->TimeScale) * -15)) && player[g_Player]->Mposestate != 14 && player[g_Player]->Mposestate != 15 && player[g_Player]->Mposestate != 16 && player[g_Player]->Mposestate != 13)
				{
				Qpo->Sound_Play(Medias->S_Footsteps, _->Volume / 2.5f, _->Pan, _->S_TimeScale, false, &playerAV[g_Player]->Xform.Translation); //campos
				Qpo->Sound_Play(Medias->S_Footsteps, _->Volume / 2.5f, _->Pan, _->S_TimeScale, false, &playerAV[g_Player]->Xform.Translation); //campos
				}
			...
			if(playerAV[g_Player]->Collision.Plane.Normal.y >= 0.707f)
				{
				if(player[g_Player]->MoveReactSpeed - 0.15 * _->TimeScale * (0.8 * (player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Weight / 100)) > 0) Q_Forge->Forge[AIcount].MoveReactSpeed -= 0.15 * _->TimeScale * (0.8 * (1 - (player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Weight / 100)));
				if(player[g_Player]->MoveReactSpeed - 0.15 * _->TimeScale > 0) player[g_Player]->MoveReactSpeed -= 0.15 * _->TimeScale;
				else player[g_Player]->MoveReactSpeed = 0;
				player[g_Player]->MoveReactVec.y = 0;

				if(player[g_Player]->HB_RecoverMode == 1 && ((player[g_Player]->Aposestate == 30 && player[g_Player]->attposecnt > 2) || (player[g_Player]->Aposestate == 52 && player[g_Player]->attposecnt > 1))) player[g_Player]->HB_RecoverMode = 2;
				}

#pragma message("FIXME::Vehicle mounting excluded until avatar detection is added");
#if 0
				//### Player Mounts a Vehicle
			if(playerAV[g_Player]->Collision.Avatar != 0)
				{
				if(playerAV[g_Player]->Collision.Avatar.IDflag == player[g_Player]->Vehicle.Model.Avatar.IDflag)
					{
			...
			if((player[g_Player]->Aposestate == 51 || player[g_Player]->Aposestate == 52 || player[g_Player]->Aposestate == 26 && player[g_Player]->MoveReactSpeed > 0.5) || (player[g_Player]->Aposestate == 73))
				{
				Qpo->Avatar_GetBoneTransform(playerAV[g_Player].operator ->(), "BIP01 R FOOT", &player[g_Player]->RFootPos);
				Qpo->Avatar_GetBoneTransform(playerAV[g_Player].operator ->(), "BIP01 L FOOT", &player[g_Player]->LFootPos);

				jet=0;
				while(jet < 1)
					{
					Q_Cloud->Cloud[_->NomCloud].CloudType = 5;

					Q_Cloud->Cloud[_->NomCloud].CloudPos = player[g_Player]->LFootPos.Translation;

					Q_Cloud->Cloud[_->NomCloud].CloudAlpha = (((float)rand() / 32767)*100)+155;
					...
					Q_Cloud->Cloud[_->NomCloud].CloudSpeed = (((float)rand() / 32767)-0.5)*0.05;
				...
				while(jet < 1)
					...
					Q_Cloud->Cloud[_->NomCloud].CloudPos = player[g_Player]->RFootPos.Translation;
			...
			if(player[g_Player]->Aposestate == 73)
				...
				while(jet < 1)
					{
					Q_Cloud->Cloud[_->NomCloud].CloudType = 5;	//6

					Q_Cloud->Cloud[_->NomCloud].CloudPos = player[g_Player]->RFootPos.Translation;

					Q_Cloud->Cloud[_->NomCloud].CloudAlpha = (((float)rand() / 32767)*120)+80;
					Q_Cloud->Cloud[_->NomCloud].CloudScale = ((float)rand() / 32767)*0.12;

					if((((float)rand() / 32767)-0.5) > 0)
						{
						//RED CLOUD
						Q_Cloud->Cloud[_->NomCloud].CloudRed = (((float)rand() / 32767)*55) + 200;
						Q_Cloud->Cloud[_->NomCloud].CloudGreen = 120;
						Q_Cloud->Cloud[_->NomCloud].CloudBlue = 120;
						}
					else
						{
						//YELLOW CLOUD
						Q_Cloud->Cloud[_->NomCloud].CloudRed = (((float)rand() / 32767)*55) + 200;
						Q_Cloud->Cloud[_->NomCloud].CloudGreen = (((float)rand() / 32767)*55) + 200;
						Q_Cloud->Cloud[_->NomCloud].CloudBlue = 120;
						}
					...
					Q_Cloud->Cloud[_->NomCloud].CloudSpeed = (((float)rand() / 32767)-0.5)*0.3;
		...
		//else								//## Check to determine if Avatar touching the Ceiling
			//{
			//playerAV[g_Player]->OldPos.y += (100 + (50*(((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Weight/100)-0.5)*2)));
			//playerAV[g_Player]->NewPos.y += (100 + (50*(((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Weight/100)-0.5)*2)));
		...
		//##### New Hit Reaction Collision

	#if 0
		playerAV[g_Player]->OldPos.y += 60;	//## Check to determine if Avatar has hit a wall
		playerAV[g_Player]->NewPos.y += 60;
		...
		if(Qpo->Collision_RayTest(NULL, NULL, &playerAV[g_Player]->OldPos, &playerAV[g_Player]->NewPos, /* GE_CONTENTS_CANNOT_OCCUPY GE_COLLIDE_MODELS,*/ 0xffffffff, NULL, NULL, &playerAV[g_Player]->Collision))
			{
			player[g_Player]->collide = 1;	//# Hit Wall
			player[g_Player]->HitWall = 1;
			player[g_Player]->HitWallNormal = playerAV[g_Player]->Collision.Plane.Normal;

			player[g_Player]->MoveReactSpeed = 0;

			if(player[g_Player]->HB_RecoverMode == 1 && ((player[g_Player]->Aposestate == 30 && player[g_Player]->attposecnt > 2) || (player[g_Player]->Aposestate == 52 && player[g_Player]->attposecnt > 1))) player[g_Player]->HB_RecoverMode = 2;

			playerAV[g_Player]->NewPos = playerAV[g_Player]->Collision.Impact;
			Qpo->Vec3d_AddScaled(&playerAV[g_Player]->NewPos, 15 + (15*(((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Weight/100)-0.5)*2)), &playerAV[g_Player]->Collision.Plane.Normal, &playerAV[g_Player]->NewPos);
		...
		playerAV[g_Player]->Xform.Translation = playerAV[g_Player]->NewPos;
		//playerAV[g_Player]->Xform.Translation.y += 2;
		player[g_Player]->finalcolldet_oldpos = playerAV[g_Player]->Xform.Translation;

		//if(playerAV[g_Player]->Xform.Translation.y < 0/*grnd*/) playerAV[g_Player]->Xform.Translation.y = 0/*grnd*/;
		//playerAV[g_Player]->Xform.Translation.y = 0/*grnd*/;


		//##### Old Hit Reaction Coll detection

		/*playerAV[g_Player]->OldPos.y += 60;	//## Check to determine if Avatar has hit a wall
		playerAV[g_Player]->NewPos.y += 60;		// \/---- Orig Value 30
		player[g_Player]->Tempfloat1 = player[g_Player]->MoveReactSpeed;
		Qpo->Vec3d_AddScaled(&playerAV[g_Player]->NewPos, player[g_Player]->Tempfloat1, &player[g_Player]->MoveReactVec, &playerAV[g_Player]->NewPos);
																										//EDIT Removed Collision detection with actors
		if(Qpo->Collision_RayTest(NULL, NULL, &playerAV[g_Player]->OldPos, &playerAV[g_Player]->NewPos, GE_CONTENTS_SOLID_CLIP, GE_COLLIDE_MODELS , 0xffffffff, NULL, NULL, &playerAV[g_Player]->Collision))
			{
			player[g_Player]->MoveReactSpeed = 0;	//# Hit Wall
			player[g_Player]->HitWall = 1;
			player[g_Player]->HitWallNormal = playerAV[g_Player]->Collision.Plane.Normal;

			if(player[g_Player]->HB_RecoverMode == 1) player[g_Player]->HB_RecoverMode = 2;
			}
		...
		//##### Switch to jump motion if falling added Floor Distance Condition

		playerAV[g_Player]->OldPos = playerAV[g_Player]->Xform.Translation;
		playerAV[g_Player]->NewPos = playerAV[g_Player]->Xform.Translation;
		playerAV[g_Player]->NewPos.y -= 80;

	#if 1
							//-1.4
		if(player[g_Player]->DownSpeed < -2 && player[g_Player]->Mposestate != 10 && player[g_Player]->Mposestate != 11 && player[g_Player]->Mposestate != 12 && player[g_Player]->Mposestate != 13 && player[g_Player]->Mposestate != 14 && player[g_Player]->Mposestate != 15 && player[g_Player]->Mposestate != 9 && player[g_Player]->Mposestate != 8 && player[g_Player]->Aposestate != 57 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->attackon == 0 && (playerAV[g_Player]->NewPos.y > 0/*grnd*/) /*0 && Qpo->Collision_RayTest(NULL, NULL, &playerAV[g_Player]->OldPos, &playerAV[g_Player]->NewPos, /*GE_CONTENTS_SOLID_CLIP, GE_COLLIDE_MODELS,*//* 0xffffffff, NULL, NULL, &playerAV[g_Player]->Collision)*/)
			{
			player[g_Player]->Mposestate = 10;
			player[g_Player]->jumpposecnt = 0;
			player[g_Player]->blendamount[4] = 0;
			...
			{
			if(player[g_Player]->DownSpeed > -0.45 && player[g_Player]->Mposestate == 10)
				{
				player[g_Player]->Mposestate = 0;
				player[g_Player]->jumpposecnt = 0;
				player[g_Player]->AirBorn = 0;
		...
		if(player[g_Player]->AirBornSTR == 1 && player[g_Player]->AirBorn == 0 && player[g_Player]->Mposestate != 14 && player[g_Player]->Mposestate != 15 && player[g_Player]->Mposestate != 16)
			{
		...
		player[g_Player]->OldPos.y += 60;
		player[g_Player]->NewPos.y += 60;
		...
		//playerAV[g_Player]->Xform.Translation = player[g_Player]->NewPos;


		player[g_Player]->ForwardSpeed = 0;		// reset our forward speed

		if (KEY_DOWN_a() && player[g_Player]->attackon == 0 /*&& !KEY_PRESS_DOWN(QIN_MBUTTON)*/)
			{
			if (KEY_PRESS_DOWN(QIN_LSHIFT) || ((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].ItemDATA[player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].CurrWEAPON].ItemREF == 3 || player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].ItemDATA[player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].CurrWEAPON].ItemREF == 4) && player[g_Player]->WeaponStatus != 0) || ((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].Class == 2) && (player[g_Player]->Cleric.ShieldOn == 1))) player[g_Player]->ForwardSpeed = (player[g_Player]->PlayerSpeed/4)*_->TimeScale;		//Slow Sidestep Left
			else player[g_Player]->ForwardSpeed = (player[g_Player]->PlayerSpeed/1.5)*_->TimeScale;							//Sidestep Left

			if(((player[g_Player]->Mposestate != 25 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 9 && player[g_Player]->Mposestate != 5 && !KEY_DOWN_w() && !KEY_DOWN_s()) || (player[g_Player]->Mposestate != 5 && KEY_PRESS_DOWN(QIN_LSHIFT))) && (player[g_Player]->Mposestate != 10 && player[g_Player]->Mposestate != 11 && player[g_Player]->Mposestate != 12 && player[g_Player]->Mposestate != 13 && player[g_Player]->Mposestate != 14 && player[g_Player]->Mposestate != 15 && player[g_Player]->Mposestate != 16) && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->Mposestate != 8)	//Dependent Movement mode switch
				{
			...
			if(player[g_Player]->Ninja.SpeedACT == 1)
				{
				Qpo->Vec3d_AddScaled(&playerAV[g_Player]->OldPos, player[g_Player]->ForwardSpeed*4, &playerAV[g_Player]->Left, &playerAV[g_Player]->NewPos);
				}
			else
				{
				Qpo->Vec3d_AddScaled(&playerAV[g_Player]->OldPos, player[g_Player]->ForwardSpeed, &playerAV[g_Player]->Left, &playerAV[g_Player]->NewPos);
				}
		...
		if (KEY_DOWN_d() && player[g_Player]->attackon == 0 /*&& !KEY_PRESS_DOWN(QIN_MBUTTON)*/)		//Sidestep Right
			{
			...
			if(((player[g_Player]->Mposestate != 25 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 9 && player[g_Player]->Mposestate != 6 && !KEY_DOWN_w() && !KEY_DOWN_s()) || (player[g_Player]->Mposestate != 6 && KEY_PRESS_DOWN(QIN_LSHIFT))) && (player[g_Player]->Mposestate != 10 && player[g_Player]->Mposestate != 11 && player[g_Player]->Mposestate != 12 && player[g_Player]->Mposestate != 13 && player[g_Player]->Mposestate != 14 && player[g_Player]->Mposestate != 15 && player[g_Player]->Mposestate != 16) && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->Mposestate != 8)	//Dependent Movement mode switch
				{
		...
		if (KEY_DOWN_w() /*&& !KEY_PRESS_DOWN(QIN_MBUTTON)*/)
			{
			...
			if(player[g_Player]->Ninja.SpeedACT == 1)
				{
				Qpo->Vec3d_AddScaled(&playerAV[g_Player]->OldPos, player[g_Player]->ForwardSpeed*4, &playerAV[g_Player]->Up, &playerAV[g_Player]->NewPos);
				}
			else
				{
				Qpo->Vec3d_AddScaled(&playerAV[g_Player]->OldPos, player[g_Player]->ForwardSpeed, &playerAV[g_Player]->Up, &playerAV[g_Player]->NewPos);
				}
		...
		if(KEY_DOWN_s() /*&& !KEY_PRESS_DOWN(QIN_MBUTTON)*/)
			{
			if(KEY_PRESS_DOWN(QIN_LSHIFT) || ((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].ItemDATA[player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].CurrWEAPON].ItemREF == 3 || player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].ItemDATA[player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].CurrWEAPON].ItemREF == 4) && player[g_Player]->WeaponStatus != 0) || ((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].Class == 2) && (player[g_Player]->Cleric.ShieldOn == 1)))
				{
				player[g_Player]->ForwardSpeed = -(player[g_Player]->PlayerSpeed/4)*_->TimeScale;  //Walk Backwards

				if(player[g_Player]->Mposestate != 25 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 9 && player[g_Player]->Mposestate != 2 && !KEY_DOWN_a() && !KEY_DOWN_d() && player[g_Player]->Mposestate != 8 && (player[g_Player]->Mposestate != 10 && player[g_Player]->Mposestate != 11 && player[g_Player]->Mposestate != 12 && player[g_Player]->Mposestate != 13 && player[g_Player]->Mposestate != 14 && player[g_Player]->Mposestate != 15 && player[g_Player]->Mposestate != 16) && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->Mposestate != 8)	//Movement mode switch
					{
		...
		//Wall run Cancel
		if((KEY_PRESS_DOWN(QIN_SPACE) || KEY_PRESS_DOWN(QIN_q) || KEY_PRESS_DOWN(QIN_e) || (KEY_PRESS_DOWN(QIN_z) && KEY_DOWN_a()) || (KEY_PRESS_DOWN(QIN_z) && KEY_DOWN_d()) || (KEY_PRESS_DOWN(QIN_z) && KEY_DOWN_w()) || (KEY_PRESS_DOWN(QIN_z) && KEY_DOWN_s())) && player[g_Player]->Mposestate == 9 && KEY_PRESS_DOWN(QIN_MBUTTON)) player[g_Player]->WallRunSWITCH = 1;

		//###--- JUMP ---###
		if(KEY_PRESS_DOWN(QIN_SPACE) /*&& player[g_Player]->Mposestate != 9*/ && player[g_Player]->Mposestate != 25 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 8 && player[g_Player]->Mposestate != 16)
			{
			player[g_Player]->SPACEKEYcnt++;

			if(KEY_DOWN_s() && !(KEY_DOWN_a() || KEY_DOWN_d()))	//### Spiral Over Enemy Jump
				{
				if(player[g_Player]->Mposestate != 10 && player[g_Player]->Mposestate != 11 && player[g_Player]->Mposestate != 12 && player[g_Player]->Mposestate != 13 && player[g_Player]->Mposestate != 14 && player[g_Player]->Mposestate != 15 && player[g_Player]->Mposestate != 16)
					...
					Qpo->Transform_GetIn(&playerAV[g_Player]->Xform,&player[g_Player]->JumpVec);
		...
		if (KEY_PRESS_DOWN(QIN_e) && KEY_PRESS_DOWN(QIN_z) /*&& player[g_Player]->Mposestate != 9*/ && player[g_Player]->Mposestate != 25 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->Mposestate != 16 && player[g_Player]->BlockHit == 0 && (player[g_Player]->Aposestate != 26 && player[g_Player]->Aposestate != 28 && player[g_Player]->Aposestate != 30))		//ROLL LOW RIGHT
			{
		...
		if (KEY_PRESS_DOWN(QIN_q) /*&& player[g_Player]->Mposestate != 9*/ && player[g_Player]->Mposestate != 25 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->Mposestate != 16 && player[g_Player]->BlockHit == 0 && (player[g_Player]->Aposestate != 26 && player[g_Player]->Aposestate != 28 && player[g_Player]->Aposestate != 30))		//CARTWHEEL LEFT
			{
		...
		if (KEY_PRESS_DOWN(QIN_e) /*&& player[g_Player]->Mposestate != 9*/ && player[g_Player]->Mposestate != 25 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->Mposestate != 16 && player[g_Player]->BlockHit == 0 && (player[g_Player]->Aposestate != 26 && player[g_Player]->Aposestate != 28 && player[g_Player]->Aposestate != 30))		//CARTWHEEL RIGHT
			{
		...
		if (KEY_PRESS_DOWN(QIN_z) && player[g_Player]->Mposestate != 25 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 16 && (player[g_Player]->Aposestate != 26 && player[g_Player]->Aposestate != 28 && player[g_Player]->Aposestate != 30))		//DODGE LEFT / ROLL FORWARD
			{
			if (KEY_DOWN_a() && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->BlockHit == 0)		//ROLL LOW LEFT
				{
				player[g_Player]->Mposestate = 28;
				player[g_Player]->runposecnt = 0;
				player[g_Player]->SideSpeed = 0;
			...
			if (KEY_DOWN_d() && player[g_Player]->Mposestate != 23 && player[g_Player]->Mposestate != 24 && player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->BlockHit == 0)		//ROLL LOW RIGHT
				{
				player[g_Player]->Mposestate = 29;
				player[g_Player]->runposecnt = 0;
				player[g_Player]->SideSpeed = 0;
			...
			if (KEY_DOWN_w() && player[g_Player]->Mposestate != 8 && player[g_Player]->BlockHit == 0)
				{
			...
			if(KEY_DOWN_s() && player[g_Player]->BlockHit == 0)	//Pressing Back So Backflip
				{
		...
		if(KEY_PRESS_DOWN(QIN_TAB) && player[g_Player]->WeaponStatus == 0 && !(player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].Class == 0 && player[g_Player]->Mposestate == 16))	//****////WEAPON\\\\****
			{
		...
		if(KEY_PRESS_DOWN(QIN_h))	//player mounts a vehicle
			{
		...
		//### Taunt
		if(KEY_PRESS_DOWN(QIN_RETURN) && player[g_Player]->Mposestate != 51)	//Taunt
			{
		...
		//###-  Blocking  -###
		if(KEY_DOWN_s() && !KEY_PRESS_DOWN(QIN_SPACE) && player[g_Player]->WeaponStatus == 0 && player[g_Player]->attackon == 0 && player[g_Player]->Mposestate != 8 && player[g_Player]->Mposestate != 16)
			{
			if(KEY_PRESS_DOWN(QIN_z))
				{
			...
			{
			if(player[g_Player]->BlockHit == 0 && (player[g_Player]->CrouchInit == 0 || (player[g_Player]->CrouchInit == 1 && !((KEY_PRESS_DOWN(QIN_z) && player[g_Player]->attackon == 0) || player[g_Player]->attackstackSPECIAL[player[g_Player]->astackrun - 1] == 1))))
				{
				player[g_Player]->Blocking = 0;
				player[g_Player]->Block = 0;
				}
		...
		if(player[g_Player]->Mposestate != 28 && player[g_Player]->Mposestate != 29 && player[g_Player]->Mposestate != 26 && player[g_Player]->Mposestate != 8 && player[g_Player]->Mposestate != 16)
			{
			if((KEY_PRESS_DOWN(QIN_z) && player[g_Player]->attackon == 0) || player[g_Player]->attackstackSPECIAL[player[g_Player]->astackrun - 1] == 1)
				{
				...
				//Qpo->Avatar_SetPosePM(playerAV[g_Player]->Avatar, playerAV[g_Player]->Motion[61], player[g_Player]->crohposecnt, NULL);
				//Qpo->Avatar_SetPosePM(playerAV[g_Player]->Avatar, playerAV[g_Player]->Motion[62], player[g_Player]->crohposecnt, NULL);
		...
		if (KEY_PRESS_DOWN(QIN_PAGEUP))
			{
			jet=0;
			while(jet < _->NETNomNetplayer)
				{
				if(Q_Netplayer->Netplayer[jet].FRLswitch == 0)
					{
					Q_Forge->Forge[Q_Netplayer->Netplayer[jet].LocalCONTROLID].NetMoveScale += 0.01;
					}
				jet++;
				}
			}

		if (KEY_PRESS_DOWN(QIN_PAGEDOWN))
			{
			jet=0;
			while(jet < _->NETNomNetplayer)
				{
