/*
	...
	*/#include "pch.h"/*- Minor Component -Avatar Muzzle Flash-
...
void Q_COREFunctions::CONTROL_AvatarMuzzleFlash(int AIcounter)
	...
	if(Q_MFlash->MFlash[AIcounter].JDamageLVL > 0)
		...
		//QpoSound3D_GetConfig(World, &player[g_Player]->ViewXForm, &Q_MFlash->MFlash[AIcounter].AlignMatrix.Translation, 600.0f, 2.0f, &Volume, &Pan, &Frequency);
		//'FIRE' SOUND
		Qpo->Sound_Play(Medias->S_RifleFire[11], _->Volume, _->Pan, _->S_TimeScale, false, &Q_MFlash->MFlash[AIcounter].AlignMatrix.Translation);
		}
	...
	if(Q_Forge->Forge[AIcounter].MFlash == 1 && Q_MFlash->MFlash[AIcounter].SkipFlag == 0)
		...
		Q_Forge->Forge[AIcounter].AV.Avatar.Scale.x = Q_Forge->Forge[AIcounter].CharDATA.AvatarSCALE + (AVSCALEINC*(((Q_Forge->Forge[AIcounter].CharDATA.BodySKILL.Weight/100)-0.5)*2));
		Q_Forge->Forge[AIcounter].AV.Avatar.Scale.y = Q_Forge->Forge[AIcounter].CharDATA.AvatarSCALE + (AVSCALEINC*(((Q_Forge->Forge[AIcounter].CharDATA.BodySKILL.Height/100)-0.5)*2));
		Q_Forge->Forge[AIcounter].AV.Avatar.Scale.z = Q_Forge->Forge[AIcounter].CharDATA.AvatarSCALE + (AVSCALEINC*(((Q_Forge->Forge[AIcounter].CharDATA.BodySKILL.Weight/100)-0.5)*2));

		if(Q_Forge->Forge[AIcounter].Weapon.Avatar == 3 || Q_Forge->Forge[AIcounter].Weapon.Avatar == 4)
			{
			Qpo->AvatarView_GetBoneTransform(&Q_Forge->Forge[AIcounter].AV, "BIP01 R HAND", &Q_Forge->Forge[AIcounter].TempXForm1, false);
			}
		else
			{
			Qpo->Avatar_GetTransformAvatar(&Q_Forge->Forge[AIcounter].AV, "BIP01 R HAND", &Q_Forge->Forge[AIcounter].TempXForm1, AIcounter, false);
			}
		...
		/*if(player[g_Player]->Weapon.Avatar == 3 || player[g_Player]->Weapon.Avatar == 4)
			...
			Qpo->Transform_RotateY(&Q_Forge->Forge[AIcounter].TempXForm1, Ange.y);
			Qpo->Transform_RotateX(&Q_Forge->Forge[AIcounter].TempXForm1, Ange.x);
			Qpo->Transform_RotateZ(&Q_Forge->Forge[AIcounter].TempXForm1, Ange.z);
			//#####
			}*/
		...
		if(Q_Forge->Forge[AIcounter].Q_Brain.TargetLOCKID == 250)
			{
			Q_Forge->Forge[AIcounter].TempVec1 = playerAV[g_Player]->Xform.Translation;
			Q_Forge->Forge[AIcounter].TempVec1.y += (4 + (30*(((player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].BodySKILL.Height/100)-0.5)*2)));
			}										//23
		else
			{
			Q_Forge->Forge[AIcounter].TempVec1 = Q_Forge->Forge[Q_Forge->Forge[AIcounter].Q_Brain.TargetLOCKID].Xform.Translation;
			Q_Forge->Forge[AIcounter].TempVec1.y += (4 + (30*(((Q_Forge->Forge[Q_Forge->Forge[AIcounter].Q_Brain.TargetLOCKID].CharDATA.BodySKILL.Height/100)-0.5)*2)));
			}										//23
		...
		else Qpo->Vec3d_CrossProduct(&qAxis[major], &Q_Forge->Forge[AIcounter].TempVec4, &TempVec1);
		...
		//Trajectory
		Qpo->Transform_AvatarGetUp(/*ORIG GetIn*/&Q_Forge->Forge[AIcounter].TempXForm1, &Q_Cartridge->Cartridge[_->NomCart].TrajVec);
		...
		Qpo->Transform_AvatarGetIn(/*ORIG GetLeft*/&Q_Forge->Forge[AIcounter].TempXForm1, &Q_Cartridge->Cartridge[_->NomCart].TrajVec);
		...
		Q_Cartridge->Cartridge[_->NomCart].AvID = AIcounter;
		...
		if((_->NetworkOption != 1) && (_->NomBullet < QAA_NET_ARRAY_MAX) && (_->NomBullet >= 0))
			{
			SCRAM = 0;
			while((SCRAM < Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].BulletCNT) && (_->NomBullet < 14999))	//=-- Maximum bullet restriction
				{
				...
				if(Q_Forge->Forge[AIcounter].Soldier.JAccuracyLVL == 0)
					{
					Q_Bullet->Bullet[_->NomBullet].TrajVec.x += (((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5)) + ((((float)rand() / 32767)-0.5) * 0.18);
					Q_Bullet->Bullet[_->NomBullet].TrajVec.y += (((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5)) + ((((float)rand() / 32767)-0.5) * 0.18);
					Q_Bullet->Bullet[_->NomBullet].TrajVec.z += (((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5)) + ((((float)rand() / 32767)-0.5) * 0.18);
					}

				if(Q_Forge->Forge[AIcounter].Soldier.JAccuracyLVL == 1)
					{
					Q_Bullet->Bullet[_->NomBullet].TrajVec.x += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 3)) + ((((float)rand() / 32767)-0.5) * 0.18);
					Q_Bullet->Bullet[_->NomBullet].TrajVec.y += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 3)) + ((((float)rand() / 32767)-0.5) * 0.18);
					Q_Bullet->Bullet[_->NomBullet].TrajVec.z += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 3)) + ((((float)rand() / 32767)-0.5) * 0.18);
					}

				if(Q_Forge->Forge[AIcounter].Soldier.JAccuracyLVL == 2)
					{
					Q_Bullet->Bullet[_->NomBullet].TrajVec.x += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 6)) + ((((float)rand() / 32767)-0.5) * 0.18);
					Q_Bullet->Bullet[_->NomBullet].TrajVec.y += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 6)) + ((((float)rand() / 32767)-0.5) * 0.18);
					Q_Bullet->Bullet[_->NomBullet].TrajVec.z += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 6)) + ((((float)rand() / 32767)-0.5) * 0.18);
					}

				if(Q_Forge->Forge[AIcounter].Soldier.JAccuracyLVL == 3)
					{
					Q_Bullet->Bullet[_->NomBullet].TrajVec.x += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 30)) + ((((float)rand() / 32767)-0.5) * 0.18);
					Q_Bullet->Bullet[_->NomBullet].TrajVec.y += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 30)) + ((((float)rand() / 32767)-0.5) * 0.18);
					Q_Bullet->Bullet[_->NomBullet].TrajVec.z += ((((((float)rand() / 32767)-0.5)/((Q_Forge->Forge[AIcounter].CharDATA.WeaponSKILL.Accuracy/5000)*5000))*((Q_Forge->Forge[AIcounter].WeaponRecoil*500)+1)*(((5000 - Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Accuracy) / 5000) * 5) / 30)) + ((((float)rand() / 32767)-0.5) * 0.18);
					}
				...
				Q_Bullet->Bullet[_->NomBullet].BulletDMG = ((float)Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Damage / 5000) * 0.025 * Q_Forge->Forge[AIcounter].JINK3_WeaponFactor;
				Q_Bullet->Bullet[_->NomBullet].BulletPWR = ((float)Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].Power / 5000) * 5 * Q_Forge->Forge[AIcounter].JINK3_WeaponFactor;
		...
		//'FIRE' SOUND
		Qpo->Sound_Play(Medias->S_RifleFire[Q_Forge->Forge[AIcounter].CharDATA.ItemDATA[Q_Forge->Forge[AIcounter].CharDATA.CurrWEAPON].QAS_Fire], _->Volume, _->Pan, _->S_TimeScale, false, &Q_Forge->Forge[AIcounter].TempXForm1.Translation);

		if(Q_Forge->Forge[AIcounter].Soldier.JDamageLVL > 0)
			{ Qpo->Sound_Play(Medias->S_RifleFire[11], _->Volume, _->Pan, _->S_TimeScale, false, &Q_Forge->Forge[AIcounter].TempVec1); }
		...
		if((_->NomBullet >= 1) && (_->NomBullet < QAA_NET_ARRAY_MAX))
			{
			...
			Q_MFlash->MFlash[AIcounter].AlignMatrix.Translation.x = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.x;
			Q_MFlash->MFlash[AIcounter].AlignMatrix.Translation.y = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.y;
			Q_MFlash->MFlash[AIcounter].AlignMatrix.Translation.z = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.z;

			Q_MFlash->MFlash[AIcounter].Pos.x = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.x;
			Q_MFlash->MFlash[AIcounter].Pos.y = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.y;
			Q_MFlash->MFlash[AIcounter].Pos.z = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.z;
			...
			Q_MFlash->MFlash[AIcounter].VecUp.x += Q_MFlash->MFlash[AIcounter].VecLeft.x * ((((float)rand() / 32767) * 0.4) - 0.2);
			Q_MFlash->MFlash[AIcounter].VecUp.y += Q_MFlash->MFlash[AIcounter].VecLeft.y * ((((float)rand() / 32767) * 0.4) - 0.2);
			Q_MFlash->MFlash[AIcounter].VecUp.z += Q_MFlash->MFlash[AIcounter].VecLeft.z * ((((float)rand() / 32767) * 0.4) - 0.2);

			Q_MFlash->MFlash[AIcounter].VecLeft.x += Q_MFlash->MFlash[AIcounter].VecUp.x * ((((float)rand() / 32767) * 0.4) - 0.2);
			Q_MFlash->MFlash[AIcounter].VecLeft.y += Q_MFlash->MFlash[AIcounter].VecUp.y * ((((float)rand() / 32767) * 0.4) - 0.2);
			Q_MFlash->MFlash[AIcounter].VecLeft.z += Q_MFlash->MFlash[AIcounter].VecUp.z * ((((float)rand() / 32767) * 0.4) - 0.2);

			Q_MFlash->MFlash[AIcounter].VecIn.x = Q_Bullet->Bullet[_->NomBullet - 1].TrajVec.x;
			Q_MFlash->MFlash[AIcounter].VecIn.y = Q_Bullet->Bullet[_->NomBullet - 1].TrajVec.y;
			Q_MFlash->MFlash[AIcounter].VecIn.z = Q_Bullet->Bullet[_->NomBullet - 1].TrajVec.z;
			...
			Q_MFlash->MFlash[AIcounter].ScaleBIT = 0.2f;

			//***** MFLASH POWER SCALE ******
			Q_MFlash->MFlash[AIcounter].Scale = 20 * (Q_Bullet->Bullet[_->NomBullet-1].BulletPWR);//20 * (Q_Bullet->Bullet[_->NomBullet-1].BulletPWR);

																	//-70
			Qpo->Vec3d_AddScaled(&Q_MFlash->MFlash[AIcounter].Pos, -15, &Q_MFlash->MFlash[AIcounter].VecIn, &Q_MFlash->MFlash[AIcounter].Pos);
			...
			Qpo->Vec3d_Inverse(&Q_MFlash->MFlash[AIcounter].VecLeft);
			...
			/*Q_MFlash->MFlash[AIcounter].Lvert[0][0].u = 0;
			Q_MFlash->MFlash[AIcounter].Lvert[0][0].v = 1;
			Q_MFlash->MFlash[AIcounter].Lvert[0][0].a = 255;
			Q_MFlash->MFlash[AIcounter].Lvert[0][0].b = 255;
			Q_MFlash->MFlash[AIcounter].Lvert[0][0].g = 255;
			Q_MFlash->MFlash[AIcounter].Lvert[0][0].r = 255;
			Q_MFlash->MFlash[AIcounter].Lvert[0][0].x = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.x;
			Q_MFlash->MFlash[AIcounter].Lvert[0][0].y = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.y;
			Q_MFlash->MFlash[AIcounter].Lvert[0][0].z = Q_Bullet->Bullet[_->NomBullet - 1].WorldPos.z;*/
			...
			Qpo->Poly_AddOnce(&Q_MFlash->MFlash[AIcounter].Lvert[0][0], 1, MFLASHbmp, Q_MFlash->MFlash[AIcounter].Type, Q_MFlash->MFlash[AIcounter].RenderFlags, Q_MFlash->MFlash[AIcounter].ScaleBIT);

			Qpo->Vec3d_AddScaled(&Q_MFlash->MFlash[AIcounter].Pos, Q_MFlash->MFlash[AIcounter].Scale, &Q_Bullet->Bullet[_->NomBullet - 1].TrajVec, &Q_MFlash->MFlash[AIcounter].PosEnd);

			Qpo->Vec3d_AddScaled(&Q_MFlash->MFlash[AIcounter].Pos, 30, &Q_Bullet->Bullet[_->NomBullet - 1].TrajVec, &Q_MFlash->MFlash[AIcounter].Pos);
		
			player[g_Player]->ViewXForm.Translation.x = g_render->camera.pos.x;
			player[g_Player]->ViewXForm.Translation.y = g_render->camera.pos.y;
			player[g_Player]->ViewXForm.Translation.z = g_render->camera.pos.z;
			Q.ShowLine(&Q_MFlash->MFlash[AIcounter].Pos, &Q_MFlash->MFlash[AIcounter].PosEnd, &player[g_Player]->ViewXForm.Translation, Q_MFlash->MFlash[AIcounter].Scale, Medias->MFlashX, 125, 255, 255, 255, 1);*/

	#if 1
			Qpo->Poly_AddOnce(&Q_MFlash->MFlash[AIcounter].Lvert[0][0], 4, Medias->MFlashX, Q_MFlash->MFlash[AIcounter].Type, Q_MFlash->MFlash[AIcounter].RenderFlags, Q_MFlash->MFlash[AIcounter].ScaleBIT);
			Qpo->Poly_AddOnce(&Q_MFlash->MFlash[AIcounter].Lvert[1][0], 4, Medias->MFlashX, Q_MFlash->MFlash[AIcounter].Type, Q_MFlash->MFlash[AIcounter].RenderFlags, Q_MFlash->MFlash[AIcounter].ScaleBIT);
			Qpo->Poly_AddOnce(&Q_MFlash->MFlash[AIcounter].Lvert[2][0], 4, Medias->MFlashS1, Q_MFlash->MFlash[AIcounter].Type, Q_MFlash->MFlash[AIcounter].RenderFlags, Q_MFlash->MFlash[AIcounter].ScaleBIT);
	#endif
		...
		if(Q_Forge->Forge[AIcounter].Backpack.Draw == 1)
			...
			Qpo->Transform_AvatarGetIn(/*ORIG GetLeft*/&Q_Forge->Forge[AIcounter].AV.Xform,&Q_Forge->Forge[AIcounter].Backpack.Leftvec);
			Qpo->Transform_AvatarGetLeft(/*ORIG GetUp*/&Q_Forge->Forge[AIcounter].AV.Xform,&Q_Forge->Forge[AIcounter].Backpack.Upvec);
			...
			Qpo->Poly_AddOnce(&Q_Forge->Forge[AIcounter].Backpack.Lvert[0][0], 4, Medias->special4[Q_Forge->Forge[AIcounter].Backpack.Matcount], QPOLY_TYPE_TRI, QPOLY_REND_DEPTH_SORT, 1.0f);
			...
			Qpo->Poly_AddOnce(&Q_Forge->Forge[AIcounter].Backpack.Lvert[0][0], 1, Medias->special4[Q_Forge->Forge[AIcounter].Backpack.Matcount], QPOLY_TYPE_PNT, QPOLY_REND_DEPTH_SORT, 0.1f);
