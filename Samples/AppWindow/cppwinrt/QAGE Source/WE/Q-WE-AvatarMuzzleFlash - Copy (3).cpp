...
void Q_COREFunctions::CONTROL_AvatarMuzzleFlash(int AIcounter)
	...
	if(Q_Forge->Forge[AIcounter].MFlash == 1 && Q_MFlash->MFlash[AIcounter].SkipFlag == 0)
		...
		if(player->Weapon.Avatar == 3 || player->Weapon.Avatar == 4)
			{
			Qpo->AvatarView_GetBoneTransform(&Q_Forge->Forge[AIcounter].AV, "BIP01 R HAND", &Q_Forge->Forge[AIcounter].TempXForm1);
			}
		else
			{
		...
		if(Q_Forge->Forge[AIcounter].Q_Brain.TargetLOCKID == 250)
			...
			Q_Forge->Forge[AIcounter].TempVec1.y += (4 + (30*(((player->CDATA.CharDATA[player->CDATA.CurrCHAR].BodySKILL.Height/100)-0.5)*2)));
			}										//23
		else
			{
		...
		Q_MFlash->MFlash[AIcounter].Type = QPOLY_TYPE_PNT;
		...
		Q_MFlash->MFlash[AIcounter].Lvert[0][0].x = Q_MFlash->MFlash[AIcounter].Pos.x;
		Q_MFlash->MFlash[AIcounter].Lvert[0][0].y = Q_MFlash->MFlash[AIcounter].Pos.y;
		Q_MFlash->MFlash[AIcounter].Lvert[0][0].z = Q_MFlash->MFlash[AIcounter].Pos.z;

		Qpo->Poly_AddOnce(&Q_MFlash->MFlash[AIcounter].Lvert[0][0], 1, Media->MFlashS, Q_MFlash->MFlash[AIcounter].Type, Q_MFlash->MFlash[AIcounter].RenderFlags, Q_MFlash->MFlash[AIcounter].ScaleBIT);

		Qpo->Vec3d_AddScaled(&Q_MFlash->MFlash[AIcounter].Pos, Q_MFlash->MFlash[AIcounter].Scale, &Q_MFlash->MFlash[AIcounter].VecIn, &Q_MFlash->MFlash[AIcounter].PosEnd);

		Q.ShowLine(&Q_MFlash->MFlash[AIcounter].Pos, &Q_MFlash->MFlash[AIcounter].PosEnd, &player->ViewXForm.Translation, Q_MFlash->MFlash[AIcounter].Scale / 5, Media->MFlashX, 125, 255, 255, 255, 1);
