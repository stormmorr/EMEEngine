...
void Q_COREFunctions::World_Event_Engine(void)
...
#if 1 //WORLD EVENT ENGINE - Point Sprite Control
	if(_->NomPntSprite > 0)
		{
	...
	if((_->TimeHole != 0) && (player[0]->JumpFG > 0))
		...
		if(!(KEY_DOWN(QIN_x) && (player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].ItemDATA[player[g_Player]->BFO.CharDATA[player[g_Player]->BFO.CurrCHAR].CurrWEAPON].ScopeTYPE > 0)))
			{
