...
VoxelGridManager::VoxelGridManager()
{
	...
	const std::string vs = STRING
        ...
        void main()
			...
			vec4 f_WorldObjectAttrPosition = uModelMatrix * f_AttrPosition;
			//f_Normal = uProjMatrix * uViewMatrix * uModelMatrix * f_Normal;
			f_Normal = uModelMatrix * f_Normal;

			f_WorldObjectAttrPosition.xyz += uPos;

			f_WorldObjectAttrPosition = uProjMatrix * uViewMatrix * f_WorldObjectAttrPosition;
	...
	const std::string fs = STRING
        ...
        void main()
			...
			f_SunLightColor = (f_SunLightColor * 0.8) + ((randElement * randElement) * 0.2); // mix in radiation perc
			...
			vec4 f_AccumulationBuffer = (Idiff * (f_SunRadius / (f_SunDistanceL / 550.553))); //750 //650 //800? //655.3
...
void WarZone::acInitialize(void)
{
	...
	for(int f_Zfact = 0; f_Zfact < f_LampCount; f_Zfact++)
		...
		f_LightStr->m_Radius = ((Cube::random() * 6500.0f) + 1000.0f);
			...
			if(m_VoxelGridsVector[f_VoxelGridCount]->m_emptyVoxelGrid == false)
				...
				if(m_VoxelGridsVector[f_VoxelGridCount]->m_created == true)
					...
					if(m_VoxelGridsVector[f_VoxelGridCount]->m_rebuild < 1)
						...
						BiVector f_PlayerGridVector(f_Position.m_X - f_ParentGrid->m_position.m_X, f_Position.m_Y - f_ParentGrid->m_position.m_Y, f_Position.m_Z - f_ParentGrid->m_position.m_Z);
						...
						if(f_DistA < VIEW_DISTANCE)
							{
#if 1
							glUniform3f(mPosUniformLocation, f_Position.m_X - f_ParentGrid->m_position.m_X, f_Position.m_Y - f_ParentGrid->m_position.m_Y, f_Position.m_Z - f_ParentGrid->m_position.m_Z);
#else
						...
						for(int f_XY = 0; f_XY < f_ParentGrid->m_Child.size(); f_XY++)
							...
							BiVector f_Child_PlayerGridVector(f_Position.m_X - f_ChildGrid->m_position.m_X, f_Position.m_Y - f_ChildGrid->m_position.m_Y, f_Position.m_Z - f_ChildGrid->m_position.m_Z);
							...
							if(f_Dist < VIEW_DISTANCE)
								{
#if 1
								glUniform3f(mPosUniformLocation, f_Position.m_X - f_ChildGrid->m_position.m_X, f_Position.m_Y - f_ChildGrid->m_position.m_Y, f_Position.m_Z - f_ChildGrid->m_position.m_Z);
#else
