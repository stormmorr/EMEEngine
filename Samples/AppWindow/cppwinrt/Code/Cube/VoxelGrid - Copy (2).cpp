...
{
	if((m_isParent == true) && (m_Child.size() == 0))
		{
		if(mVertexPositionBuffer != 0)
			{
			glDeleteBuffers(1, &mVertexPositionBuffer);
    ...
    mDrawCount(0)
{
	...
	(
        ...
        void main()
			...
			int f_CubeVert = int(aCubeVert);

			vNormalX = f_Normals[f_CubeVert].x;
			vNormalY = f_Normals[f_CubeVert].y;
			vNormalZ = f_Normals[f_CubeVert].z;
	...
	mNormalAttribLocation = glGetAttribLocation(mProgram, "aCubeVert");
...
void VoxelGridManager::DrawPlayer(BiVector f_Position, int f_Player)
{
	...
	std::shared_ptr<VoxelGrid> f_VoxelGrid = m_playerVoxelGrid[f_Player];
			...
			if(m_VoxelGridsVector[f_VoxelGridCount]->m_emptyVoxelGrid == false)
				...
				if(m_VoxelGridsVector[f_VoxelGridCount]->m_created == true)
					...
					if(m_VoxelGridsVector[f_VoxelGridCount]->m_rebuild < 1)
						...
						BiVector f_BiVector(f_ParentGrid->m_position.m_X + f_Position.m_X, f_ParentGrid->m_position.m_Y + f_Position.m_Y, f_ParentGrid->m_position.m_Z + f_Position.m_Z);

						float f_DistA = f_BiVector.acLength();

						if(f_DistA < VIEW_DISTANCE)
							{
							glUniform3f(mPosUniformLocation, f_BiVector.m_X, f_BiVector.m_Y, f_BiVector.m_Z);
						...
						for(int f_XY = 0; f_XY < f_ParentGrid->m_Child.size(); f_XY++)
							...
							BiVector f_Child_BiVector(f_ChildGrid->m_position.m_X + f_Position.m_X, f_ChildGrid->m_position.m_Y + f_Position.m_Y, f_ChildGrid->m_position.m_Z + f_Position.m_Z);

							float f_Dist = f_Child_BiVector.acLength();

							if(f_Dist < VIEW_DISTANCE)
								{
								glUniform3f(mPosUniformLocation, f_Child_BiVector.m_X, f_Child_BiVector.m_Y, f_Child_BiVector.m_Z);
	...
	for (float x = startx; x < endx; x += VoxelGrid::BLOCK_RENDER_SIZE)
	{
		for (float y = starty; y < endy; y += VoxelGrid::BLOCK_RENDER_SIZE)
		{
			for (float z = startz; z < endz; z += VoxelGrid::BLOCK_RENDER_SIZE)
			{
				...
				std::shared_ptr<VoxelGrid> pVoxelGrid = NULL;
...
void VoxelGridManager::CreatePlayerVoxelGrid(BiVector f_Position, int f_Player, float f_Scale)
{
	...
	// Create a new VoxelGrid at this grid position
	std::shared_ptr<VoxelGrid> pNewVoxelGrid = std::make_shared<VoxelGrid>();
...
#if 0 //uses async loading on engineloop thread
		RebuildMesh();
		CompleteMesh();
		SetCreated(true);
#endif
...
void VoxelGrid::Make_Alone(void)
{
	if(m_Child.size() > 0)
		...
		for(int f_Jet = 0; f_Jet < m_Child.size(); f_Jet++)
			{
