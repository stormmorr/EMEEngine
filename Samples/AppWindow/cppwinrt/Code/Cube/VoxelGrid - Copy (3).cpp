...
bool VoxelGridManager::GetActive_Global(int x, int y, int z)
{
	...
	if(f_Grid != nullptr)
		...
		unsigned int colour = f_Grid->m_colour[f_Local_X + f_Local_Y * VoxelGrid::VOXEL_GRID_SIZE + f_Local_Z * VoxelGrid::VOXEL_GRID_SIZE_SQUARED];
		unsigned int alpha = (colour & 0xFF000000) >> 24;
		unsigned int blue = (colour & 0x00FF0000) >> 16;
		unsigned int green = (colour & 0x0000FF00) >> 8;
		unsigned int red = (colour & 0x000000FF);
...
unsigned int VoxelGridManager::AddVertexToMeshNeighbourhood(std::shared_ptr<VoxelGrid> f_VoxelGrid, int str_x, int str_y, int str_z, BiVector p1, BiVector n1, float r, float g, float b, float a)
{
	 ...
	 //
	//
	f_VoxelGrid->m_vec_Vertex.push_back(new BiVector(p1.m_X + f_VoxelGrid->m_position.m_X, p1.m_Y + f_VoxelGrid->m_position.m_Y, (p1.m_Z * -1.0f) + (f_VoxelGrid->m_position.m_Z * -1.0f)));
	f_VoxelGrid->m_vec_Normal.push_back(new BiVector(n1.m_X, n1.m_Y, n1.m_Z * -1.0f));
...
VoxelGridManager::VoxelGridManager()
{
	...
	const std::string vs = STRING
        ...
        void main()
			...
			f_WorldObjectAttrPosition.xyz += uPos;

			absPosition = f_WorldObjectAttrPosition.xyz;
			vNormal = f_Normal.xyz;

			vUV = aUV;
			vColor = aColor;

			gl_Position = f_WorldObjectAttrPosition;
			}
		);
#endif
#ifdef DAYTIME_SHADER
    ...
    const std::string fs = STRING
        ...
        void main()
			{
			//initially seed the psuedo random generator
			seed(vec3(uSeed * absPosition.x, uSeed * absPosition.y, uSeed * absPosition.z));

			//vec3 f_vNormal = vNormal; // normalize(vNormal);
			...
			vec4 randElement = vec4(f_qsX * 0.3, f_qsY * 0.3, f_qsZ * 0.3, 1.0);

			vec3 f_SunLightPos = vec3(400.0, 4500.0, 1850.0);
			vec4 f_SunLightColor = vec4(0.92, 0.87, 0.79, 1.0);

			f_SunLightColor = (f_SunLightColor * 0.7) + (randElement * 0.3);

			float f_SunRadius = 695.0;
			...
			vec4 f_SunAccumulationBuffer = (Idiff * ((f_SunRadius) / (f_SunDistanceL / 115.0))) * 0.7;
			...
			for(int f_i = 0; f_i < 10; f_i++)
				...
				if(f_ProcCount < uLightNom)
					...
					if(uLightPos[f_i].w > 0.0)
						...
						vec3 to_light = (f_LightPos - vPosition) - vcPos;
						...
						f_AccumulationBuffer += (Idiffl * (f_LightRadius / 28.0)) * (1.0 - (f_constant_dist / 100.0));
						}
					}
			...
			f_Result.r += f_SunAccumulationBuffer.r * f_sunamp;
			f_Result.g += f_SunAccumulationBuffer.g * f_sunamp;
			f_Result.b += f_SunAccumulationBuffer.b * f_sunamp;
			f_Result.a += f_SunAccumulationBuffer.a * f_sunamp;

			if(uLightNom >= 1.0)
				{
				f_Result.r += (f_AccumulationBuffer.r / (uLightNom)) * f_amp;
				f_Result.g += (f_AccumulationBuffer.g / (uLightNom)) * f_amp;
				f_Result.b += (f_AccumulationBuffer.b / (uLightNom)) * f_amp;
				f_Result.a += (f_AccumulationBuffer.a / (uLightNom)) * f_amp;
				}

			f_Result.a = 1.0;

			gl_FragColor = (f_AmbientColor * 0.8) + (f_Result * 0.45);
			gl_FragColor.a = 1.0;
			}
		);
#else
	const std::string fs = STRING
        ...
        void main()
			...
			f_SunLightColor = (f_SunLightColor * 0.835) + ((randElement * randElement) * 0.25); // mix in radiation perc

			vec3 f_Ls = normalize(f_SunLightPos);

			vec4 Idiff = vec4(f_SunLightColor.rgb * max(dot(f_vNormal, f_Ls), 0.0), 1.0);
			...
			vec4 f_AccumulationBuffer = (Idiff * (f_SunRadius / (f_SunDistanceL / 450.553))); //750 //650 //800? //655.3
			...
			for(int f_i = 0; f_i < 10; f_i++)
				...
				if(f_ProcCount < uLightNom)
					...
					if(uLightPos[f_i].w > 0.0)
						...
						float f_constant_dist = sqrt(to_light.x * to_light.x + to_light.y * to_light.y + to_light.z * to_light.z);
																		 //28.0 //6.5
						f_AccumulationBuffer += (Idiffl * f_LightRadius) * (1.0 - (f_constant_dist / 654.0)); // 750.0
						}
			...
			gl_FragColor = (f_AmbientColor * 0.75)* (f_Result * 0.75);
			}
		);
#endif
...
void WarZone::acInitialize(void)
{
	...
	for(int f_Zfact = 0; f_Zfact < f_LampCount; f_Zfact++)
		...
		f_LightStr->m_RGBA.alpha = ((Cube::random() * 0.8f) + 0.2f);
		f_LightStr->m_RGBA.red = ((Cube::random() * 0.8f) + 0.2f);
		f_LightStr->m_RGBA.green = ((Cube::random() * 0.8f) + 0.2f);
		f_LightStr->m_RGBA.blue = ((Cube::random() * 0.8f) + 0.2f);

		f_LightStr->m_Radius = ((Cube::random() * 650.0f) + 100.0f);
...
void VoxelGridManager::Draw(BiVector f_Position)
{
		...
		if((f_Count < Q_Nom_Light))
			...
			if(f_Count < 5000)
				...
				if(f_LightSelect[f_CountIndex] == false)
					...
					BiVector f_VecTo = f_Pos - f_Position;
	...
	for(int f_VoxelGridCount = 0; f_VoxelGridCount < f_Size; f_VoxelGridCount++)
		{
		if(m_VoxelGridsVector[f_VoxelGridCount]->m_isParent == true)
			{
			if(m_VoxelGridsVector[f_VoxelGridCount]->m_emptyVoxelGrid == false)
				{
				if(m_VoxelGridsVector[f_VoxelGridCount]->m_created == true)
					{
					if(m_VoxelGridsVector[f_VoxelGridCount]->m_rebuild < 1)
						{
						std::shared_ptr<VoxelGrid> f_ParentGrid = m_VoxelGridsVector[f_VoxelGridCount];

						bool f_BuffersUpl = false;
						int f_FaceCount = 0;

						BiVector f_PlayerGridVector(f_ParentGrid->m_position.m_X + f_Position.m_X, f_ParentGrid->m_position.m_Y + f_Position.m_Y, f_ParentGrid->m_position.m_Z + f_Position.m_Z);

						float f_DistA = f_PlayerGridVector.acLength();

						if(f_DistA < VIEW_DISTANCE)
							{
#if 1
							glUniform3f(mPosUniformLocation, f_ParentGrid->m_position.m_X + f_Position.m_X, f_ParentGrid->m_position.m_Y + f_Position.m_Y, f_ParentGrid->m_position.m_Z + f_Position.m_Z);
#else
							glUniform3f(mPosUniformLocation, f_PlayerGridVector.m_X, f_PlayerGridVector.m_Y, f_PlayerGridVector.m_Z);
#endif
						...
						for(int f_XY = 0; f_XY < f_ParentGrid->m_Child.size(); f_XY++)
							...
							BiVector f_Child_PlayerGridVector(f_ChildGrid->m_position.m_X + f_Position.m_X, f_ChildGrid->m_position.m_Y + f_Position.m_Y, f_ChildGrid->m_position.m_Z + f_Position.m_Z);

							float f_Dist = f_Child_PlayerGridVector.acLength();

							if(f_Dist < VIEW_DISTANCE)
								{
#if 1
								glUniform3f(mPosUniformLocation, f_ChildGrid->m_position.m_X + f_Position.m_X, f_ChildGrid->m_position.m_Y + f_Position.m_Y, f_ChildGrid->m_position.m_Z + f_Position.m_Z);
#else
								glUniform3f(mPosUniformLocation, f_Child_PlayerGridVector.m_X, f_Child_PlayerGridVector.m_Y, f_Child_PlayerGridVector.m_Z);
#endif
...
void VoxelGridManager::Position_Player(BiVector f_Position)
{
	...
	float f_origin_X = f_grs_X;
	float f_origin_Y = f_grs_Y;
	float f_origin_Z = f_grs_Z;

	/*if(f_grs_X < (VOX_BY2 * -1.0f))
		{
		f_grs_X = f_Position.m_X - VOX_BY2;
		}
	else
		{
		f_grs_X = f_Position.m_X + VOX_BY2;
	...
	if(f_grs_Y < (VOX_BY2 * 1.0f))
		{
		f_grs_Y = f_Position.m_Y - VOX_BY2;
		}
	else
		{
		f_grs_Y = f_Position.m_Y + VOX_BY2;
	...
	if(f_grs_Z < (VOX_BY2 * 1.0f))
		{
		f_grs_Z = f_Position.m_Z - VOX_BY2;
		}
	else
		{
		f_grs_Z = f_Position.m_Z + VOX_BY2;
		...
		for(int f_Y = f_Ysgrid - LOAD_DISTANCE; f_Y < f_Ysgrid + LOAD_DISTANCE; f_Y++)
			...
			for(int f_Z = f_Zsgrid - LOAD_DISTANCE; f_Z < f_Zsgrid + LOAD_DISTANCE; f_Z++)
				...
				if(f_Found == false)
					...
					Cube::g_WarZoneManager->acOnPushGrid(f_VoxelGrid);
