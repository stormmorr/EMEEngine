...
VoxelGridManager::VoxelGridManager()
{
	...
	const std::string vs = STRING
        ...
        void main()
			...
			// interim translation
			f_ModelAttrPosition.xyz += uPos - uCam;
			...
			for(int f_i = 0; f_i < 10; f_i++)
				...
				if(f_ProcCount < uLightNom)
					...
					if(uLightPos[f_i].w > 0.0)
						...
						float f_LightRadius = uLightColor[f_i].w / 150.0;
			...
			if(m_VoxelGridsVector[f_VoxelGridCount]->m_emptyVoxelGrid == false)
				...
				if(m_VoxelGridsVector[f_VoxelGridCount]->m_created == true)
					...
					if(m_VoxelGridsVector[f_VoxelGridCount]->m_rebuild < 1)
						...
						BiVector f_Parent_PlayerGridVector(f_Position.m_X - (f_ParentGrid->m_position.m_X + VOX_BY2), f_Position.m_Y - (f_ParentGrid->m_position.m_Y + VOX_BY2), f_Position.m_Z - (f_ParentGrid->m_position.m_Z + VOX_BY2));
						...
						for(int f_XY = 0; f_XY < f_ParentGrid->m_Child.size(); f_XY++)
							...
							BiVector f_Child_PlayerGridVector(f_Position.m_X - (f_ChildGrid->m_position.m_X + VOX_BY2), f_Position.m_Y - (f_ChildGrid->m_position.m_Y + VOX_BY2), f_Position.m_Z - (f_ChildGrid->m_position.m_Z + VOX_BY2));
