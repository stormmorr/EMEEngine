...
VoxelGridManager::VoxelGridManager()
{
	...
	const std::string fs = STRING
        ...
        void main()
			...
			// sun effect
			vec3 f_SunLightPos = vec3(-3.3, 5.0, 2.7);
			...
			f_SunLightColor = (f_SunLightColor * 0.6) + (randElement * 0.4); //wk
			...
			//vec4 f_AccumulationBuffer = sunIdiff * 8.0; // night-

			vec4 f_AccumulationBuffer = sunIdiff * 55.0; // summer day-
				...
				if(f_i < f_LightNom)
					...
					if(uLightPos[f_i].w > 0.0)
						...
						float f_constant_dist = sqrt(to_light.x * to_light.x + to_light.y * to_light.y + to_light.z * to_light.z);
							...
							//(7%) OF (0.0 -> 1.0)
						vec4 f_CalcVec = (Idiffl * (f_LightRadius / 27.5)) * (1.0 - clamp((f_constant_dist / 1450.0), 0.0, 1.0)); // 750.0 // 654.0 // 850.0 // 55.0

						float f_MullLength = sqrt(f_CalcVec.x * f_CalcVec.x + f_CalcVec.y * f_CalcVec.y + f_CalcVec.z * f_CalcVec.z);
						
						float f_OffProcFac = clamp(f_MullLength, 0.0, 1.0) * 0.7;

						if(f_OffProcFac > 0.0)
							{
							f_AccumulationBuffer += f_CalcVec;

							f_ProcCount += f_OffProcFac;
							}
						}
					}
				}
