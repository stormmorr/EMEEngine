...
VoxelLimb::VoxelLimb(int f_ManagerIndex, int f_MinX, int f_MinY, int f_MinZ, int f_MaxX, int f_MaxY, int f_MaxZ, BlockType f_InitialBlockType)
{
	...
	mresVertexPositionBuffer = 0;
	mresVertexNormalBuffer = 0;
	mresVertexColorBuffer = 0;
	mresVertexTexCoordBuffer = 0;
	mresIndexBuffer = 0;
	...
	m_vec_BlockType.push_back(f_InitialBlockType);
...
void VoxelGrid::SetCreated(int f_ThreadID, bool created)
{
	m_created = created;
	...
	if(m_repeatbuild > 0)
		...
		RebuildMesh(f_ThreadID);
		CompleteMesh(f_ThreadID);
		SetCreated(f_ThreadID, true);
		}
}

// Creation and destruction
void VoxelLimb::SetCreated(int f_ThreadID, bool created)
{
	m_created = created;
	...
	if(m_setup == false)
		...
		for(int x = 0; x < VoxelGrid::VOXEL_GRID_SIZE; x++)
			...
			for(int z = 0; z < VoxelGrid::VOXEL_GRID_SIZE; z++)
				{
				float xPosition = m_position.m_X + ((float)x * BLOCK_RENDER);
				float zPosition = m_position.m_Z + ((float)z * BLOCK_RENDER);
				...
				for(int y = 0; y < VoxelGrid::VOXEL_GRID_SIZE; y++)
					...
					if(yPosition < noiseHeight)
						{
						if((y + (m_gridY * VoxelGrid::VOXEL_GRID_SIZE) < f_Height) || 1)
							{
...
bool VoxelGridManager::GetActive_Global(int x, int y, int z)
{
	...
	if(f_Grid->m_Has_Core == false)
		{
		if(f_Grid->m_full == true)
			{
			return true;
			}
		else if(f_Grid->m_emptyVoxelGrid == true)
			{
			return false;
			}
#ifdef ECOIN_DEBUG_BREAKS
...
void VoxelGrid::SetColour(int x, int y, int z, float r, float g, float b, float a, bool setBlockType)
{
	...
	if(m_Has_Core == false)
		...
		m_VX_Core = std::make_shared<VoxelGridCore>((std::shared_ptr<VoxelGrid>)this);

		SetNeedsRebuild(true, false, false);
		}
...
unsigned int VoxelGrid::GetColour(int x, int y, int z)
{
	...
	if(m_Has_Core == false)
		...
		if(m_full == true)
			...
			unsigned int blue = (int)(1.0f * 255) << 16;
			unsigned int green = (int)(1.0f * 255) << 8;
			unsigned int red = (int)(1.0f * 255);
			...
			{
			unsigned int alpha = (int)(0.0f * 255) << 24;
			unsigned int blue = (int)(0.0f * 255) << 16;
			unsigned int green = (int)(0.0f * 255) << 8;
			unsigned int red = (int)(0.0f * 255);
...
void VoxelGrid::GenerateBuffers(int f_ThreadID)
{
	...
	if(m_ReserveReady == true)
		...
		m_resFaceCount = m_vec_Index.size() * 3;
		}
	else
		{
...
void VoxelLimb::GenerateBuffers(int f_ThreadID)
{
	const unsigned int f_VertNom = m_vec_Vertex.size() * 3;
	const unsigned int f_ColorNom = m_vec_Color.size() * 4;
	const unsigned int f_UVNom = m_vec_Tex.size() * 2;
...
void VoxelGrid::GenerateBuffersColour(void)
{
	...
	// Create mesh
void VoxelGrid::CreateMesh(int f_ThreadID)
{
...
void VoxelGrid::CreateMeshCubeColour(int f_ThreadID)
{
	const float BLOCK_RENDER = VoxelGrid::BLOCK_RENDER_SIZE * 2.0f;
...
std::string VoxelGrid::HashMeshColourCube(void)
{
	...
	for(int x = 0; x < VOXEL_GRID_SIZE; x++)
		{
		for(int y = 0; y < 3; y++) //// VOXEL_GRID_SIZE; y++)
			{
			for(int z = 0; z < VOXEL_GRID_SIZE; z++)
				{
...
std::string VoxelLimb::HashMeshColourCube(void)
{
	...
	CubeHASH_originstring* f_HashPad =
		new CubeHASH_originstring();
...
void VoxelGrid::CreateMeshCubeDetail(int f_ThreadID, float f_Scale)
{
	float r = 1.0f;
	float g = 1.0f;
	float b = 1.0f;
	...
	if(m_build_count == 0)
		...
		if(m_emptyVoxelGrid == false)
			{
			if((m_full == true) && (m_surroundedVoxelGrid == true))
				{
				m_Hash = "FullGridSurrounded"; //alone surrounded parent
				}
			else
				{
				if(m_gridY == 0) //base production layer
					{
					m_Hash = "MakeGridBase";
					}
				else
					...
					__debugbreak();
#endif
					m_Hash = HashMeshColourCube(); // escapegrid hashed
					}
				}
			}
		else
			{
			m_Hash = "EmptyFlightGrid"; //alone space parent
			}

		int f_ParentGridID = g_VoxelGridManager->acSearch_WithHash(m_Hash, m_UID);
...
void VoxelLimb::CreateMeshCubeDetail(int f_ThreadID, float f_Scale)
{
	const float BLOCK_RENDER = VoxelGrid::BLOCK_RENDER_SIZE * 2.0f;
	const float GRIDSIZE = (VoxelGrid::VOXEL_GRID_SIZE * BLOCK_RENDER);
	...
	if(m_build_count == 0)
		...
		if(m_emptyVoxelGrid == false)
			...
			if(m_full == true)
				{
				m_Hash = "FullLimbParent"; //alone parent limb

				f_ParentLimbID = acSearch_WithExtents_Limb(true);
				}
			else
				{
				std::string f_HashStr = HashMeshColourCube();

				f_ParentLimbID = m_GridRef->acSearch_WithHash_Limb(f_HashStr, m_UID);
				}
			}
		else
			{
			m_Hash = "EmptyLimbParentAlone"; //alone parent limb

			return;
			}

#ifdef VOXEL_LIMB_FULL_HASH
		if(f_ParentLimbID < 0)
			{
			std::string f_HashStr = HashMeshColourCube();
...
void VoxelGrid::CompleteMesh(int f_ThreadID)
{
	UpdateEmptyFlag();
...
void VoxelLimb::CompleteMesh(int f_ThreadID)
{
	UpdateEmptyFlag();
...
// Rebuild
void VoxelGrid::RebuildMesh(int f_ThreadID, float f_Scale)
{
	m_rebuild = 1;

#ifdef VOXEL_REBUILDING_NEIGHBOURS
	if((m_full == false) || (m_surroundedVoxelGrid == false))
		{
		if(f_Scale == 1.0f)
			{
	...
	CreateMeshCubeDetail(f_ThreadID, f_Scale);
...
// Rebuild
void VoxelLimb::RebuildMesh(int f_ThreadID, float f_Scale)
{
	m_rebuild = 1;
...
void VoxelGrid::RebuildMeshColour(int f_ThreadID)
{
	m_rebuild = 1;
	...
	CreateMeshCubeColour(f_ThreadID);
...
VoxelGridManager::VoxelGridManager()
{
	...
	const std::string fs = STRING
		...
		vec4 al_QBuild(vec3 axis, float angle)
			{
			vec4 qr;
		...
		vec3 rotate_vertex_position(vec3 position, vec3 axis, float angle)
			{ 
			vec4 f_q = al_QBuild(axis, angle);

			return position + 2.0 * cross(f_q.xyz, cross(f_q.xyz, position) + f_q.w * position);
			}

		vec3 al_OrbitalSunPos(float f_TimeAbs, vec3 f_Initial_Position)
			{
			vec3 f_ResultPosition = rotate_vertex_position(f_Initial_Position, vec3(0.0, 0.0, 1.0), f_TimeAbs);

			return f_ResultPosition;
			}

		vec3 al_OrbitalMoonPos(float f_TimeAbs, vec3 f_Initial_Position)
			{
			vec3 f_ResultPosition = rotate_vertex_position(f_Initial_Position, vec3(1.0, 0.0, 0.0), -f_TimeAbs);
        ...
        void main()
			...
			// call for fragment
			float f_Frl = rand() * 1.0;
			float f_Byl = rand() * f_Frl;
			float f_qsX = ((rand() - 0.2) * 2.0) * f_Byl;
			float f_qsY = ((rand() - 0.2) * 2.0) * f_Byl;
			float f_qsZ = ((rand() - 0.2) * 2.0) * f_Byl;
			...
			//sun factor
			float f_TimeBohSun = min(f_TimeBoh + 0.707, 0.0);

			//moon factor
			float f_TimeBohMoon = abs(max(f_TimeBoh - 0.707, 0.0));

			// accumulation Buffer
			vec4 f_AccumulationBuffer = (f_AmbientColor * 1.0);

			//- quantum sun effect -//
			//vec3 f_SunLightPos = al_OrbitalPos(f_TimeBoh, vec4(-15000000000000.0, 0.0, 0.0, 0.0)).xyz;

			vec3 f_SunLightPos = al_OrbitalSunPos(f_TimeBohSun * (M_PI * 2.0), vec3(-10000.0, 0.0, 0.0));
			vec4 f_SunLightColor = vec4(0.96, 0.90, 0.79, 1.0);

			vec3 f_MoonLightPos = al_OrbitalMoonPos(f_TimeBohMoon * (M_PI * 2.0), vec3(0.0, 0.0, 10000.0));
			vec4 f_MoonLightColor = vec4(0.94, 0.96, 1.0, 1.0);

			vec4 randElementSun = (randElement * (f_TimeBohSun * 1.75)); //swk !!! 1.45
			vec4 randElementMoon = (randElement * (f_TimeBohMoon * 1.1)); //swk !!! 2.85

			vec3 f_to_Sun = f_SunLightPos - absPosition;
			...
			vec4 f_sunLight = sunIdiff * 0.07; // 1.795 summer day-8

			//rad boost
			f_sunLight += randElementSun; //swk!!!

			// Scale the power of this fragment based on its distance to the light.
			float f_sun_constant_dist = sqrt(f_to_Sun.x * f_to_Sun.x + f_to_Sun.y * f_to_Sun.y + f_to_Sun.z * f_to_Sun.z);

			//(7%) OF (0.0 -> 1.0)                //33.0
			vec4 f_sun_CalcVec = (sunIdiff * (f_sunLight / 24.4)) * (1.0 - clamp((f_sun_constant_dist / 1450.0), 0.0, 1.0)); // 750.0 // 654.0 // 850.0 // 55.0 - 1450.0

			vec4 f_sun_lightCalcVec = (f_SunLightColor * (f_sun_CalcVec * 0.877)) + (f_sun_CalcVec * 0.125);
			...
			//f_ProcCount += (1.0 - (f_sunOffFac * 1.0)); //swk 0.9775

			f_ProcCount += 1.0;

				//- final pixel sun -//
			f_AccumulationBuffer = f_sun_lightCalcVec * 5.0; // 3.0 // summer day-

			//- quantum moon effect -//
			vec3 f_to_Mn = f_MoonLightPos - absPosition;
			vec3 f_Lr = normalize(f_MoonLightPos);

			vec4 MoonIdiff = vec4(f_MoonLightColor.rgb * max(dot(f_vNormal, f_Lr), 0.0), 1.0);

			MoonIdiff = clamp(MoonIdiff, 0.0, 1.0);

			vec4 f_MoonLight = MoonIdiff * 0.07; // *= 0.03

			//rad boost
			f_MoonLight += randElementMoon;

			// Scale the power of this fragment based on its distance to the light.
			float f_mn_constant_dist = sqrt(f_to_Mn.x * f_to_Mn.x + f_to_Mn.y * f_to_Mn.y + f_to_Mn.z * f_to_Mn.z);

			//(7%) OF (0.0 -> 1.0)                //33.0
			vec4 f_mn_CalcVec = (sunIdiff * (f_MoonLight / 24.4)) * (1.0 - clamp((f_mn_constant_dist / 1450.0), 0.0, 1.0)); // 750.0 // 654.0 // 850.0 // 55.0 - 1450.0

			vec4 f_mn_lightCalcVec = (f_MoonLightColor * (f_mn_CalcVec * 0.877)) + (f_mn_CalcVec * 0.125);
			...
			//f_ProcCount += (1.0 - (f_MoonOffFac * 1.0)); //swk 0.89

			f_ProcCount += 1.0;

				//- final pixel Moon -//
			f_AccumulationBuffer = f_mn_lightCalcVec * 2.75; // 2.75 // summer night-
			...
			for(int f_i = 0; f_i < 10; f_i++)
				...
				if(f_i < f_LightNom)
					...
					if(uLightPos[f_i].w > 0.0)
						...
						//f_ProcCount += (1.0 - (f_OffFac * 1.0)); // 0.965 swk
			...
			vec4 f_Result = (f_AccumulationBuffer / 2.0) / f_ProcCount; //swk!!!

			// [zero] blend
			f_Result.a = 1.0;

			// set pixel color
			gl_FragColor = (f_AmbientColor * 0.707) * f_Result; // = f_Result //swk???
			//gl_FragColor = ((f_AmbientColor * 0.5) + (randElementSun * 0.5) + (randElementMoon * 0.5)) * f_Result; // = f_Result //swk???
			//gl_FragColor = f_AmbientColor * f_Result;
			//gl_FragColor = f_Result; // = (f_Result * 0.88) + (f_AmbientColor * 0.33) //swk???
			}
		);
...
#ifdef ECOIN_DEBUG_BREAKS
	m_VX_Stride = sizeof(unsigned int) + sizeof(char) + sizeof(char) + sizeof(float);
	m_VX_Size_UINT = sizeof(unsigned int);
	m_VX_Size_char = sizeof(char);
	m_VX_Size_float = sizeof(float);
#endif
...
bool WarZone_Manager::acPushGridOrigin(BiVector f_NewGridOrigin, std::shared_ptr<Cube::VoxelGrid> f_Grid)
{
	...
	if(f_NewGridOrigin.m_X > m_current_Zone_Origin.m_X + (f_Extent + 400.0f))
		...
		std::shared_ptr<WarZone> f_current_WarZone = std::make_shared<WarZone>(f_NewWarzoneOrigin);

		f_current_WarZone->m_Zone_X = f_X_IntStr + 1;
		f_current_WarZone->m_Zone_Y = f_Y_IntStr;
		f_current_WarZone->m_Zone_Z = f_Z_IntStr;
		...
		f_current_WarZone->acInitialize();
	...
	if(f_NewGridOrigin.m_Y > m_current_Zone_Origin.m_Y + (f_Extent + 400.0f))
		...
		f_current_WarZone->m_Zone_X = f_X_IntStr;
		f_current_WarZone->m_Zone_Y = f_Y_IntStr + 1;
		f_current_WarZone->m_Zone_Z = f_Z_IntStr;
	...
	if(f_NewGridOrigin.m_Z > m_current_Zone_Origin.m_Z + (f_Extent + 400.0f))
		...
		f_current_WarZone->m_Zone_X = f_X_IntStr;
		f_current_WarZone->m_Zone_Y = f_Y_IntStr;
		f_current_WarZone->m_Zone_Z = f_Z_IntStr + 1;
	...
	if(m_current_WarZone == nullptr)
		{
		std::shared_ptr<WarZone> f_current_WarZone = std::make_shared<WarZone>(f_NewGridOrigin);

		f_current_WarZone->m_Zone_X = 0;
		f_current_WarZone->m_Zone_Y = 0;
		f_current_WarZone->m_Zone_Z = 0;
...
void WarZone::acInitialize(void)
{
...
#else
	float f_LampCount = (WAR_ZONE_LAMP_MIN + (Cube::randomFac() % (WAR_ZONE_LAMP_MAX - WAR_ZONE_LAMP_MIN)));

#if 0
	f_LampCount *= 1.5; // 1.6 //// 1.5
#else
	f_LampCount *= 1.6; //2
#endif
#endif
	...
	for(int f_Zfact = 0; f_Zfact < f_SmallCount; f_Zfact++)
		...
		f_Building->m_Width = (10 + ((Cube::randomFac() % 15) + (Cube::randomFac() % 10))) * (10 / BLOCK_RENDER);
		f_Building->m_Length = (5 + ((Cube::randomFac() % 5) + (Cube::randomFac() % 8))) * (10 / BLOCK_RENDER);
		f_Building->m_Height = (6 + ((Cube::randomFac() % 8) + (Cube::randomFac() % 4))) * (10 / BLOCK_RENDER);

		//building max headroom
		if(f_Building->m_Height >= VoxelGrid::VOXEL_GRID_SIZE)
			{
			f_Building->m_Height = VoxelGrid::VOXEL_GRID_SIZE - 1;
			}

		f_Building->m_Pos.m_X = m_position.m_X + (((Cube::random() - 0.5f) * 2.0f) * m_AxisExtent);
		f_Building->m_Pos.m_Y = (BLOCK_RENDER * 0.5f) + (BLOCK_RENDER * 2.0f);
...
#ifdef BUILDING_LIMITLESS_FIXME
	...
	for(int f_Zfact = 0; f_Zfact < f_MediumCount; f_Zfact++)
		...
		f_Building->m_Width = (10 + ((Cube::randomFac() % 12) + (Cube::randomFac() % 12))) * (10 / BLOCK_RENDER);
		f_Building->m_Length = (10 + ((Cube::randomFac() % 12) + (Cube::randomFac() % 12))) * (10 / BLOCK_RENDER);
		f_Building->m_Height = (13 + ((Cube::randomFac() % 15) + (Cube::randomFac() % 10))) * (10 / BLOCK_RENDER);
	...
	for(int f_Zfact = 0; f_Zfact < f_LargeCount; f_Zfact++)
		...
		f_Building->m_Width = (12 + ((Cube::randomFac() % 25) + (Cube::randomFac() % 30))) * (10 / BLOCK_RENDER);
		f_Building->m_Length = (12 + ((Cube::randomFac() % 20) + (Cube::randomFac() % 25))) * (10 / BLOCK_RENDER);
		f_Building->m_Height = (10 + ((Cube::randomFac() % 7) + (Cube::randomFac() % 8))) * (10 / BLOCK_RENDER);
	...
	for(int f_Zfact = 0; f_Zfact < f_ObjectCount; f_Zfact++)
		...
		f_Building->m_Width = (16 + ((Cube::randomFac() % 25) + (Cube::randomFac() % 17))) * (10 / BLOCK_RENDER);
		f_Building->m_Length = (16 + ((Cube::randomFac() % 25) + (Cube::randomFac() % 15))) * (10 / BLOCK_RENDER);
		f_Building->m_Height = (14 + ((Cube::randomFac() % 20) + (Cube::randomFac() % 17))) * (10 / BLOCK_RENDER);
	 ...
	 // LIGHT ////////
	//
	for(int f_Zfact = 0; f_Zfact < (int)f_LampCount; f_Zfact++)
		{
		std::shared_ptr<Building> f_Building = std::make_shared<Building>();

		f_Building->m_Width = (1 + (Cube::randomFac() % 4)) * (10 / BLOCK_RENDER);
		...
		f_Building->m_Height = (5 + (Cube::randomFac() % 14)) * (10 / BLOCK_RENDER);
		...
		f_LightStr->m_RGBA.red = ((Cube::random() * 0.75f) + 0.35f) * 255.0f;
		f_LightStr->m_RGBA.green = ((Cube::random() * 0.75f) + 0.35f) * 255.0f;
		f_LightStr->m_RGBA.blue = ((Cube::random() * 0.75f) + 0.35f) * 255.0f;
	...
	//'0' all work comp if threads stable in position
	if(g_WarZoneManager)
		{
		for(int f_XLM = 0; f_XLM < g_WarZoneManager->m_vec_WorkThread.size(); f_XLM++)
			{
			g_WarZoneManager->m_vec_WorkThread[f_XLM]->ac_reset_Clear();
			}

		g_WarZoneManager->m_current_WarZone = (shared_ptr<WarZone>)this;
		}

	//unlock building
	m_BuildingMuti.unlock();

	//unlock light building
...
void VoxelGridManager::Draw(BiVector f_Position)
{
	...
	for(int i = 0; i < f_i; i++)
		{
		//Main Valid Proof
		if(m_VoxelGridsVector[i])
			{
			if(m_VoxelGridsVector[i]->m_isParent == true)
				{
				if(m_VoxelGridsVector[i]->m_created == true)
					{
					if((m_VoxelGridsVector[i]->m_full == false) || (m_VoxelGridsVector[i]->m_surroundedVoxelGrid == false))
						{
						if((m_VoxelGridsVector[i]->m_rebuild == 0) || ((m_VoxelGridsVector[i]->m_rebuild > 0) && (m_VoxelGridsVector[i]->m_ReserveReady == true)))
							...
							float f_DistP = FLT_MAX;

							if(f_ParentGrid->m_emptyVoxelGrid == false)
								{
								...
								BiVector f_Parent_PlayerGridVector(f_Position.m_X - (f_ParentGrid->m_midpoint.m_X), f_Position.m_Y - (f_ParentGrid->m_midpoint.m_Y), f_Position.m_Z - (f_ParentGrid->m_midpoint.m_Z));

								f_DistP = f_Parent_PlayerGridVector.acLength();

								if((f_DistP < VIEW_DISTANCE) && (f_ParentGrid->m_ShowOff == false))
									{
									if(f_ParentGrid->m_Has_Core == true)
										{
										f_ParentGrid->m_VX_Core->m_Core_Muti.lock(); //as grid locked in position
										...
										f_ParentGrid->m_VX_Core->m_Core_Muti.unlock(); //as grid locked in position
										}
									}
								}
							...
							for(int f_XY = 0; f_XY < f_ParentGrid->m_Child.size(); f_XY++)
								{
								if(f_ParentGrid->m_Child[f_XY])
									{
									if(f_ParentGrid->m_Child[f_XY]->m_created == true)
										{
										if((f_ParentGrid->m_Child[f_XY]->m_full == false) || (f_ParentGrid->m_Child[f_XY]->m_surroundedVoxelGrid == false))
											{
											if(f_ParentGrid->m_Child[f_XY]->m_emptyVoxelGrid == false)
												{
												if(f_ParentGrid->m_Child[f_XY]->m_rebuild == 0)
													{
													std::shared_ptr<VoxelGrid> f_ChildGrid = f_ParentGrid->m_Child[f_XY];
			
													BiVector f_Child_PlayerGridVector(f_Position.m_X - f_ChildGrid->m_midpoint.m_X, f_Position.m_Y - f_ChildGrid->m_midpoint.m_Y, f_Position.m_Z - f_ChildGrid->m_position.m_Z);

													float f_DistC = f_Child_PlayerGridVector.acLength();

													if((f_DistC < VIEW_DISTANCE) && (f_ChildGrid->m_ShowOff == false))
														{
														if(f_ChildGrid->m_Has_Core == true)
															{
															f_ChildGrid->m_VX_Core->m_Core_Muti.lock(); //as childgrid locked in position
															...
															f_ChildGrid->m_VX_Core->m_Core_Muti.unlock(); //as childgrid locked in position
															}

														// of Grid Child // Parent Limbs
														for(int f_LYX = 0; f_LYX < f_ChildGrid->m_vec_VoxelLimb.size(); f_LYX++)
															...
															if(f_Limb)
																{
																if(f_Limb->m_isParent == true)
																	{
																	if(f_Limb->m_created == true)
																		{
																		if((f_Limb->m_rebuild == 0) || ((f_Limb->m_rebuild > 0) && (f_Limb->m_ReserveReady == true)))
																			{
																			...
																			if(f_Limb->m_ShowOff == false)
																				{
																				f_Limb->m_VX_Core->m_Core_Muti.lock();
																				...
																				//MathHelper::Matrix4 modelMatrix = MathHelper::SimpleModelMatrix((float)((float)M_PI * (float)2.0f) * (float)+0.0f);
																				//glUniformMatrix4fv(mModelUniformLocation, 1, GL_FALSE, &(modelMatrix.m[0][0]));
																				//glUniform3f(mPosUniformLocation, f_Limb->m_position.m_X, f_Limb->m_position.m_Y, f_Limb->m_position.m_Z);
																				glUniform3f(mPosUniformLocation, f_ChildGrid->m_position.m_X, f_ChildGrid->m_position.m_Y, f_ChildGrid->m_position.m_Z);
#endif
																				...
																				f_Limb->m_VX_Core->m_Core_Muti.unlock(); //as childgrid locked in position
																				}
																					...
																					if(f_ChildLimb->m_created == true)
																						...
																						if(f_ChildLimb->m_rebuild == 0)
																							...
																							if(f_ChildLimb->m_ShowOff == false)
																								{
																								f_ChildLimb->m_VX_Core->m_Core_Muti.lock();
																								...
																								f_ChildLimb->m_VX_Core->m_Core_Muti.unlock();
																								}
																							}
																						}
																					}
											...
											if(f_Limb->m_created == true)
												...
												if((f_Limb->m_rebuild == 0) || ((f_Limb->m_rebuild > 0) && (f_Limb->m_ReserveReady == true)))
													...
													if(f_Limb->m_ShowOff == false)
														...
														f_Limb->m_VX_Core->m_Core_Muti.unlock();
														}

													// Limb-/-Limb Playground
...
void VoxelGrid::SetPosition(BiVector pos)
{
	...
	m_midpoint += BiVector(VOX_BY2, 0.0f, VOX_BY2);
...
void VoxelGridManager::CreateNewVoxelGrid(int x, int y, int z)
{
	...
	if(it != m_VoxelCoresMap.end())
		...
		pNewVoxelGrid->m_Has_Core = true;
		pNewVoxelGrid->m_full = false;
		pNewVoxelGrid->m_emptyVoxelGrid = false;
		pNewVoxelGrid->m_setup = true;
		pNewVoxelGrid->SetNeedsRebuild(true, true, false);

		//Calc if requires grid core atm
		if(pNewVoxelGrid->m_gridY < 0)
			{
			pNewVoxelGrid->m_full = true;
			pNewVoxelGrid->m_emptyVoxelGrid = false;
			pNewVoxelGrid->m_surroundedVoxelGrid = true;
			}
		else if(pNewVoxelGrid->m_gridY > 0)
			{
			pNewVoxelGrid->m_full = false;
			pNewVoxelGrid->m_emptyVoxelGrid = true;
			pNewVoxelGrid->m_surroundedVoxelGrid = false;
			}
		else
			{ // Win! Create a new Voxel Core for this grid
			pNewVoxelGrid->m_full = false;
			pNewVoxelGrid->m_emptyVoxelGrid = false;
			pNewVoxelGrid->m_surroundedVoxelGrid = false;
			}
		...
		if(pNewVoxelGrid->m_gridY < 0)
			{
			pNewVoxelGrid->m_Has_Core = false;
			pNewVoxelGrid->m_full = true;
			pNewVoxelGrid->m_emptyVoxelGrid = false;
			pNewVoxelGrid->m_setup = true;
			pNewVoxelGrid->m_surroundedVoxelGrid = true;
			pNewVoxelGrid->SetCreated(ENGINE_MANAGER_THREAD_ID, true);
			pNewVoxelGrid->m_rebuild = 0;
			pNewVoxelGrid->m_VX_Core = nullptr;
			}
		else if(pNewVoxelGrid->m_gridY > 0)
			{
			pNewVoxelGrid->m_Has_Core = false;
	...
	if(pNewVoxelGrid->m_Has_Core == true)
		{
...
std::shared_ptr<VoxelGridCore> VoxelGridManager::GetVoxelGridCore(int aX, int aY, int aZ)
{
	std::shared_ptr<VoxelGridCore> f_VX_Core = GetVoxelGrid(aX, aY, aZ)->m_VX_Core;

	//acquire core
	if(f_VX_Core != nullptr)
		{
		if(f_VX_Core->m_GridRef->m_Has_Core == false)
			{
			f_VX_Core->m_GridRef->m_Has_Core = true;

			f_VX_Core->m_GridRef->m_VX_Core = std::make_shared<VoxelGridCore>(f_VX_Core->m_GridRef);

			f_VX_Core->m_GridRef->SetNeedsRebuild(true, false, false);
			}
		}
...
int VoxelGridManager::acSearch_WithHash(std::string f_Hash, int f_UID)
{
...
int VoxelGridManager::acSearch_WithHash_Limb(std::string f_Hash, int f_UID)
{
...
int VoxelLimb::acSearch_WithExtents_Limb(bool f_Full)
{
	for(int f_Int = 0; f_Int < g_VoxelGridManager->m_VoxelLimbVector.size(); f_Int++)
		{
		std::shared_ptr<VoxelLimb> f_Limb = g_VoxelGridManager->m_VoxelLimbVector[f_Int];

		if(f_Limb)
			{
			if(f_Limb->m_UID != m_UID)
				{
				if(f_Limb->m_full == f_Full)
					{
					if(f_Limb->m_vec_BlockType[0] == m_vec_BlockType[0])
						{
						if(f_Limb->m_Extents_Width == m_Extents_Width)
							{
							if(f_Limb->m_Extents_Height == m_Extents_Height)
								{
								if(f_Limb->m_Extents_Length == m_Extents_Length)
									{
									return f_Int;
									}
								}
...
int VoxelGrid::acSearch_WithHash_Limb(std::string f_Hash, int f_UID)
{
...
bool VoxelGridManager::Make_Gen_Cuboid(BlockType f_BlockType, float f_MinX, float f_MinY, float f_MinZ, float f_MaxX, float f_MaxY, float f_MaxZ, int VColorRed, int VColorGreen, int VColorBlue, int VColorAlpha, std::shared_ptr<VoxelGrid> f_VoxelGrid, std::shared_ptr<VoxelLimb> f_CurrentLimb, bool f_NewLimb)
{
	...
	if((f_MinX < (((float)f_VoxelGrid->m_gridX + 1) * GRIDSIZE)) &&
		...
		int f_Building_Local_MinX = ag_max(ag_min(f_Building_MinX - f_VoxelGrid->m_Xpos, 0), VoxelGrid::VOXEL_GRID_SIZE);
		int f_Building_Local_MinY = ag_max(ag_min(f_Building_MinY - f_VoxelGrid->m_Ypos, 0), VoxelGrid::VOXEL_GRID_SIZE);
		int f_Building_Local_MinZ = ag_max(ag_min(f_Building_MinZ - f_VoxelGrid->m_Zpos, 0), VoxelGrid::VOXEL_GRID_SIZE);

		int f_Building_Local_MaxX = ag_max(ag_min(f_Building_MaxX - f_VoxelGrid->m_Xpos, 0), VoxelGrid::VOXEL_GRID_SIZE);
		int f_Building_Local_MaxY = ag_max(ag_min(f_Building_MaxY - f_VoxelGrid->m_Ypos, 0), VoxelGrid::VOXEL_GRID_SIZE);
		int f_Building_Local_MaxZ = ag_max(ag_min(f_Building_MaxX - f_VoxelGrid->m_Zpos, 0), VoxelGrid::VOXEL_GRID_SIZE);

		  //////////
		 // Grid //
		//std::shared_ptr<VoxelLimb> f_NewLimb
		for(int vx = f_Building_Local_MinX; vx < f_Building_Local_MaxX; vx++)
			{
			for(int vy = f_Building_Local_MinY; vy < f_Building_Local_MaxY; vy++)
				{
				for(int vz = f_Building_Local_MinZ; vz < f_Building_Local_MaxZ; vz++)
					{		
					 ///////////////////////////////////
					...
					if(f_Collide == false)
						...
						if(f_NewLimb == true)
							{
							f_CurrentLimb = f_VoxelGrid->acMake_PushLimb(f_Building_Local_MinX, f_Building_Local_MinY, f_Building_Local_MinZ, f_Building_Local_MaxX, f_Building_Local_MaxY, f_Building_Local_MaxZ, f_BlockType);
					...
					//acquire core
					if(f_VoxelGrid->m_Has_Core == false)
						{
						f_VoxelGrid->m_Has_Core = true;

						f_VoxelGrid->m_VX_Core = std::make_shared<VoxelGridCore>(f_VoxelGrid);

						f_CurrentLimb->m_VX_Core = f_VoxelGrid->m_VX_Core;
						}

					f_CurrentLimb->m_VX_Core->SetColourType(vx, vy, vz, VColorRed, VColorGreen, VColorBlue, VColorAlpha, f_BlockType);

					f_CurrentLimb->m_VX_Core->SetStrength(vx, vy, vz, ag_BlockType_Varg(f_BlockType, BlockStat_Argument::BlockArg_Strength));
...
bool VoxelGridManager::Make_Gen_Point(BlockType f_BlockType, int f_PointX, int f_PointY, int f_PointZ, int VColorRed, int VColorGreen, int VColorBlue, int VColorAlpha, std::shared_ptr<VoxelLimb> f_CurrentLimb)
{
	...
	if(f_CurrentLimb != nullptr)
		{
		//acquire core
		if(f_CurrentLimb->m_GridRef->m_Has_Core == false)
			{
			f_CurrentLimb->m_GridRef->m_Has_Core = true;

			f_CurrentLimb->m_GridRef->m_VX_Core = std::make_shared<VoxelGridCore>(f_CurrentLimb->m_GridRef);

			f_CurrentLimb->m_GridRef->SetNeedsRebuild(true, false, false);
			}
	...
	if(f_CurrentTime > m_StorageTime + 10)
		...
		for(int f_XY = 0; f_XY < m_VoxelGridsVector.size(); f_XY++)
			...
			if( (f_Grid->m_gridX < f_grids_X - LOAD_DISTANCE) ||
				...
				//if(f_Grid->m_TimedExit > 350) // One Minute Passed 6
				//	{
					if(f_Grid != nullptr)
						{
						f_Grid->Make_Alone();
...
void VoxelLimb::Make_Alone(void)
{
	if(m_Child.size() > 0)
		{
		std::shared_ptr<VoxelLimb> f_NewParentCh = m_Child[0];

		if(f_NewParentCh)
			{
			f_NewParentCh->m_isParent = true;

			for(int f_Jet = 1; f_Jet < m_Child.size(); f_Jet++)
				{
				std::shared_ptr<VoxelLimb> f_Child = m_Child[f_Jet];

				if(f_Child)
					{
					f_Child->m_Parent = f_NewParentCh;

					f_NewParentCh->m_Child.push_back(f_Child);

					//array re-order
					for(int f_XY = f_Jet; f_XY < m_Child.size() - 1; f_XY++)
						{
						m_Child[f_XY] = m_Child[f_XY + 1];
						}
...
void VoxelLimb::acPreCalc_Cache(void)
{
	if(m_rebuildCache == 1)
		...
		for(int f_X = m_MinX; f_X < m_local_MaxX; f_X++)
			{
			for(int f_Y = m_MinY; f_Y < m_local_MaxY; f_Y++)
				{
				for(int f_Z = m_MinZ; f_Z < m_local_MaxZ; f_Z++)
					{
...
void VoxelLimb::acCoreCalc(float f_TimeScale)
{
 ...
 /////////////////
// 'PushLimb'
std::shared_ptr<VoxelLimb> VoxelGrid::acMake_PushLimb(int f_MinX, int f_MinY, int f_MinZ, int f_MaxX, int f_MaxY, int f_MaxZ, BlockType f_InitialBlockType)
{
	// Make New Limb Class
	std::shared_ptr<VoxelLimb> f_Limb = std::make_shared<VoxelLimb>(m_vec_VoxelLimb.size(), f_MinX, f_MinY, f_MinZ, f_MaxX, f_MaxY, f_MaxZ, f_InitialBlockType);

	f_Limb->SetPosition(m_position);
	f_Limb->SetGrid(m_gridX, m_gridY, m_gridZ);
	f_Limb->m_full = true;

	f_Limb->m_Extents_Width = f_Limb->m_MaxX - f_Limb->m_MinX;
	f_Limb->m_Extents_Height = f_Limb->m_MaxY - f_Limb->m_MinY;
	f_Limb->m_Extents_Length = f_Limb->m_MaxZ - f_Limb->m_MinZ;
...
std::shared_ptr<VoxelLimb> VoxelGrid::OnVoxelLimb(int f_X, int f_Y, int f_Z)
{
	...
	if(m_VoxelLimb_MeM != nullptr)
		{
		if( (f_X >= m_VoxelLimb_MeM->m_MinX) &&
			(f_X < m_VoxelLimb_MeM->m_MaxX) &&
			(f_Y >= m_VoxelLimb_MeM->m_MinY) &&
			(f_Y < m_VoxelLimb_MeM->m_MaxY) &&
			(f_Z >= m_VoxelLimb_MeM->m_MinZ) &&
			(f_Z < m_VoxelLimb_MeM->m_MaxZ))
			{
	...
	for(int f_Cycl = 0; f_Cycl < m_vec_VoxelLimb.size(); f_Cycl++)
		...
		if (f_X < f_Limb->m_MinX)
			continue;

		if (f_X >= f_Limb->m_MaxX)
			continue;

		if (f_Y < f_Limb->m_MinY)
			continue;

		if (f_Y >= f_Limb->m_MaxY)
			continue;

		if (f_Z < f_Limb->m_MinZ)
			continue;

		if (f_Z >= f_Limb->m_MaxZ)
			continue;
