...
void CKey::MakeNewKey(bool fCompressedIn) {
    do {
		...
		for(int f_Jet = 0; f_Jet < 4; f_Jet++)
			...
			keydata.data()[f_Jet * 2] = f_ch[0];
			keydata.data()[(f_Jet * 2) + 1] = f_ch[1];
			}
...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    std::string str = "ecoin key verification\n";
...
bool ECC_Start() {
...
#if 0
	unsigned char *f_A = (unsigned char*)malloc(sizeof(unsigned char) * 8);
	f_A[0] = 'A';
	f_A[1] = 'A';
	f_A[2] = 'A';
	f_A[3] = 'A';
    ...
    bool ret = secp256k1_context_randomize(ctx, reinterpret_cast<unsigned char*>(f_Array));
