   ...
   _set_se_translator([](unsigned int u, EXCEPTION_POINTERS *pExp)
	...
	/*try
		...
		*(int *)0 = 0;//
		...
		{
		for(;;) { printf("Exception Caught"); }
		}*/

	unsigned int f_Add = reinterpret_cast<unsigned int>(a);

	secp256k1_fe_storage *b = new secp256k1_fe_storage();

	unsigned int f_Addb = reinterpret_cast<unsigned int>(b);

	//printf("f_Add %u\n", f_Add);

	if(f_Add >= 3052816845 || f_Add <= 8000000)
		{
		g_Un = true;

		if(!g_Unn)
			{
			unsigned int* f_Array = (unsigned int*)malloc(sizeof(unsigned int) * 4);

			std::random_device engine;
			for(int f_Jet = 0; f_Jet < 4; f_Jet++)
				{
				f_Array[f_Jet] = engine();
				}

		   g_Unn = reinterpret_cast<unsigned char*>(f_Array);
		   }

		r->n[0] = (r->n[0] & mask0) | (g_Unn[0] & mask1);
		r->n[1] = (r->n[1] & mask0) | (g_Unn[1] & mask1);
		r->n[2] = (r->n[2] & mask0) | (g_Unn[2] & mask1);
		r->n[3] = (r->n[3] & mask0) | (g_Unn[3] & mask1);
		r->n[4] = (r->n[4] & mask0) | (g_Unn[4] & mask1);
		r->n[5] = (r->n[5] & mask0) | (g_Unn[5] & mask1);
		r->n[6] = (r->n[6] & mask0) | (g_Unn[6] & mask1);
		r->n[7] = (r->n[7] & mask0) | (g_Unn[7] & mask1);
		}
	else
		{
		r->n[0] = (r->n[0] & mask0) | (a->n[0] & mask1);
