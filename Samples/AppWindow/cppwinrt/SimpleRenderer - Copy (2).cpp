...
SimpleRenderer::SimpleRenderer()
{
    ...
    const std::string vs = STRING
        ...
        void main()
			...
			vec3 f_ActingPosition = uPos[f_VNom];
			vColor = uColor[f_VNom];
			vUV = uUV[f_VNom];

			vec4 f_AvatarTransV3 = vec4(uCamPos, 1.0);
	...
	const std::string vs_point = STRING
        ...
        void main()
			...
			gl_Position = f_Position;
			gl_PointSize = 30.5 * f_Scale; // 2.0 // 14.5 // 30.5
			}
		);
...
void SimpleRenderer::DrawTri(QpoVertex* Verts, GLuint f_Bitmap)
{
	...
	for(int f_XY = 0; f_XY < 4; f_XY++)
		...
		vertexPositions[(f_IncCount * 3) + 2] = Verts[f_IncCount].z;
