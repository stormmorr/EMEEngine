...
SecondaryAppWindowPageOGL::SecondaryAppWindowPageOGL() : mRenderSurface(EGL_NO_SURFACE), m_FirstAll(true)
{
	...
	m_ShareShop = ::std::make_shared_reflect<eshop>();
	...
	if(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[0].at(0) == '0')
		...
		{
		f_eventLCount = g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->size();
		}

	if(f_eventLCount > 0)
		{
		g_EventID = atoi(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[0].c_str());
		}
	...
	std::shared_ptr<Cube::ShareContainerExt> f_ShareContainer = ag_SelectPeerGraphExt(g_JobID, 0, 0, g_CurrentShareOffset, g_PEER, true);
		...
		if(f_Grid->m_rebuild == 1)
			...
			for(int f_Jet = 0; f_Jet < f_Grid->m_vec_VoxelLimb.size(); f_Jet++)
				...
				if(f_Limb->m_rebuild == 1)
					...
					f_Limb->RebuildMesh();
					f_Limb->CompleteMesh();
					f_Limb->SetCreated(true);
					}
				}

			f_Grid->SetCreated(true);
...
void SecondaryAppWindowPageOGL::EngineLoopManager(void)
{
	...
	while(m_Engine_ThreadON == true)
		...
		for(int f_Count = 0; f_Count < g_VoxelGridManager->m_VoxelGridsVector.size(); f_Count++)
			...
			if((f_Grid->m_rebuild != 0) && (f_Grid->m_Has_Core == true))
				{
				bool f_Dupe = false;
					...
					if(f_Limb->m_rebuild == 1)
						...
						for(int f_Grf = 0; f_Grf < g_VoxelGridManager->m_vec_EngineWork_Limb.size(); f_Grf++)
							...
							if(f_ChkLimb->m_UID == f_Limb->m_UID)
								...
								f_Jet = f_Grid->m_vec_VoxelLimb.size();
								}
							}
		...
		if(g_QamStatus < QAM_STATUS_LOADED)
			...
			if(f_Ready == true)
				...
				for(int f_Jeh = 0; f_Jeh < Cube::g_WarZoneManager->m_vec_WorkComp.size(); f_Jeh++)
					{
					std::shared_ptr<std::vector<int>> f_Vector = Cube::g_WarZoneManager->m_vec_WorkComp[f_Jeh];

					for(int f_JB = 0; f_JB < f_Vector->size(); f_JB++)
						{
						if(f_Vector->at(f_JB) < 1)
							{
							f_Ready = false;

							f_Jeh = Cube::g_WarZoneManager->m_vec_WorkComp.size();
							f_JB = f_Vector->size();
							}
						}
					}
				...
				if(f_Ready == true)
					...
					for(int f_Jeh = 0; f_Jeh < Cube::g_WarZoneManager->m_vec_WorkComp.size(); f_Jeh++)
						...
						if(f_Vector->size() > 0)
							{
							if(f_Vector->at(f_Vector->size() - 1) != 5)
								{
								f_Ready = false;
...
void SecondaryAppWindowPageOGL::EngineFrame(void)
{
	...
	for(int f_JI = 0; f_JI < Cube::g_WarZoneManager->m_vec_WorkComp.size(); f_JI++)
		{
		if(Cube::g_WarZoneManager->m_vec_WorkComp[f_JI]->size() <= 0)
			{
			f_Engine_ThreadIndex = Cube::g_VoxelGridManager->m_vec_running_EngineThread_IDX[f_JI];

			f_vec_WorkStatus = Cube::g_WarZoneManager->m_vec_WorkComp[f_JI];

			f_vec_WorkStatus->push_back(0);

			f_JI = Cube::g_WarZoneManager->m_vec_WorkComp.size();
			}
		}
	...
	if(f_Engine_ThreadIndex < 0)
		...
		Cube::g_WarZoneManager->m_vec_WorkComp.push_back(f_vec_WorkStatus);
		}
		...
		if(f_Grid_WorkSize > 0)
			...
			if((f_WorkGrid->m_rebuild == 1) && (f_WorkGrid->m_Has_Core == true))
				...
				if(f_WorkGrid->m_build_count == 0)
					...
					f_Cnt = f_Engine_ThreadIndex * (Cube::g_WarZoneManager->m_current_WarZone->m_vec_Building.size() / VOXEL_ENGINE_THREAD_COUNT);

					if(f_Cnt >= Cube::g_WarZoneManager->m_current_WarZone->m_vec_Building.size())
						{
						f_Cnt -= Cube::g_WarZoneManager->m_current_WarZone->m_vec_Building.size();
						}

					Cube::g_VoxelGridManager->m_Muti_EngineWork.lock();

					f_vec_WorkStatus->clear();

					while(f_vec_WorkStatus->size() < f_Cnt)
						{
						f_vec_WorkStatus->push_back(0);
						}
					...
					if(f_WorkGrid->m_full == false)
						...
						while(f_ScanBuilding == true)
							...
							if(f_Cnt < f_vec_WorkStatus->size())
								{
								if(f_vec_WorkStatus->at(f_Cnt) == 0)
									{
									Cube::g_VoxelGridManager->m_Muti_EngineWork.unlock();
							
									Cube::g_WarZoneManager->m_current_WarZone->m_BuildingMuti.lock();

									if(Cube::g_WarZoneManager->m_current_WarZone->m_vec_WorkStatus_Building[f_Cnt] == 0)
										{
										Cube::g_WarZoneManager->m_current_WarZone->m_vec_WorkStatus_Building[f_Cnt] = 1;
										...
										if(f_Building->acPush_Grid(f_WorkGrid) == false)
											...
											for(int f_J = 0; f_J < Cube::g_WarZoneManager->m_vec_WorkComp.size(); f_J++)
												{
												std::shared_ptr<std::vector<int>> f_Vector = Cube::g_WarZoneManager->m_vec_WorkComp[f_J];

												//all sizes 1 off
												if(f_Cnt < f_Vector->size())
													{
													for(int f_XY = f_Cnt; f_XY < f_Vector->size() - 1; f_XY++)
														{
											...
											if(f_Cnt < Cube::g_WarZoneManager->m_current_WarZone->m_vec_WorkStatus_Building.size())
												{
												Cube::g_WarZoneManager->m_current_WarZone->m_vec_WorkStatus_Building[f_Cnt] = 0;
												}
							...
							if(f_WorkSig != 2)
								...
								if(f_Cnt < f_vec_WorkStatus->size())
									{
									f_vec_WorkStatus->at(f_Cnt) = f_WorkSig;
									}
								else
									{
						...
						//Generate Proper Count Start Position // size() reentrant????
						f_Cnt = f_Engine_ThreadIndex * (Cube::g_WarZoneManager->m_current_WarZone->m_vec_Light_Building.size() / VOXEL_ENGINE_THREAD_COUNT);

						if(f_Cnt >= Cube::g_WarZoneManager->m_current_WarZone->m_vec_Light_Building.size())
							{
							f_Cnt -= Cube::g_WarZoneManager->m_current_WarZone->m_vec_Light_Building.size();
							}
						...
						while(f_ScanBuilding == true)
							...
							if(f_Cnt < f_vec_WorkStatus->size())
								...
								if(f_vec_WorkStatus->at(f_Cnt) == 0)
									...
									if(Cube::g_WarZoneManager->m_current_WarZone->m_vec_WorkStatus_Light_Building[f_Cnt] == 0)
										{
										Cube::g_WarZoneManager->m_current_WarZone->m_vec_WorkStatus_Light_Building[f_Cnt] = 1;
										...
										if(f_Light_Building->acPush_Grid(f_WorkGrid) == false)
											...
											if(f_Cnt < Cube::g_WarZoneManager->m_current_WarZone->m_vec_WorkStatus_Light_Building.size())
												{
												Cube::g_WarZoneManager->m_current_WarZone->m_vec_WorkStatus_Light_Building[f_Cnt] = 0;
												}
				...
				f_WorkGrid->RebuildMesh();
				f_WorkGrid->CompleteMesh();
		...
		if(f_Limb_WorkSize > 0)
			...
			if(Cube::g_VoxelGridManager->m_vec_EngineWork_Limb.size() > 0)
				...
				if(f_WorkLimb->m_rebuild == 1)
					{
					f_WorkLimb->RebuildMesh();
					f_WorkLimb->CompleteMesh();
					f_WorkLimb->SetCreated(true);
					}
	...
	void SecondaryAppWindowPageOGL::Frame_OnCount(winrt::Windows::Foundation::IInspectable const& sender, winrt::Windows::Foundation::IInspectable e)
	{
		...
		if(g_Schedular[1]->m_Scan == true)
			...
			if(g_WalletOpen == true)
				...
				if((g_Viewing == 0) || (g_Compressing == 1))
					{
					g_StorageCounter++;
