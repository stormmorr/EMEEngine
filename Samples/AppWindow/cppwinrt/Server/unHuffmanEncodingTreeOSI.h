...
#ifndef UN_HUFFMAN_ENCODING_TREE
#define UN_HUFFMAN_ENCODING_TREE

#include "RakMemoryOverride.h"
#include "DS_HuffmanEncodingTreeNode.h"
#include "BitTransferOSI.h"
#include "Export.h"
...
class RAK_DLL_EXPORT unHuffmanEncodingTree
{

public:
	unHuffmanEncodingTree();
	~unHuffmanEncodingTree();
	...
	/// \param [out] output The BitTransfer to write to
	void EncodeArray( unsigned char *input, size_t sizeInBytes, BitTransfer * output );

	// Decodes an array encoded by EncodeArray()
	unsigned DecodeArray( BitTransfer * input, BitSize_t sizeInBits, size_t maxCharsToWrite, unsigned char *output );
	void DecodeArray( unsigned char *input, BitSize_t sizeInBits, BitTransfer * output );
