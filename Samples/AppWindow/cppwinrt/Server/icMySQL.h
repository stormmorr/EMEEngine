...
#ifdef _WIN32    //mysql fails without winsock
  #include <windows.h>
  #include <winsock.h>
  #include <mysql.h>
#else
...
public:
   ...
   * Constructor.
   *
   * @param resultSet The MySQL result set to clone
   */
  icMySQLResultSet (st_mysql_res* resultSet);
...
public:
   ...
   */
  icMySQLResultSet* acSelectCommand (const char* cmd);
   ...
   */
  unsigned int Insert (const char *table, const char **fields, const char **values, int fieldcount);
   ...
   * @return FIXME: a bool??
   */
  bool Update (const char *table, const char **fields, const char **values, int fieldcount, const char *whereclause);
...
private:
  ...
  MYSQL_RES* IssueSelectCommand (const char* cmd);
};